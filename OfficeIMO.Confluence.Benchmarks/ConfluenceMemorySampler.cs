...
internal sealed class ConfluenceMemorySampler : IDisposable {
    ...
    internal ConfluenceMemorySampler(Process process) {
        _process = process;
        _peakManagedHeapBytes = GC.GetTotalMemory(forceFullCollection: false);
        _process.Refresh();
        _peakWorkingSetBytes = _process.WorkingSet64;
        _thread = new Thread(SampleUntilStopped) { IsBackground = true, Name = "OfficeIMO.Confluence memory sampler" };
        _thread.Start();
    }

    internal ConfluenceMemoryPeak Stop() {
        if (Interlocked.Exchange(ref _stopped, 1) == 0) {
            _stop.Set();
            _thread.Join();
            RecordCurrentMemory();
        }
        return new ConfluenceMemoryPeak(Interlocked.Read(ref _peakManagedHeapBytes), Interlocked.Read(ref _peakWorkingSetBytes));
    }
