...
<article class="ocx-capability-card"><span class="ocx-chip ocx-chip--warn">Next browser route</span><h3>HTML to DOCX</h3><p>Complete the web-content bridge after style and image handling are validated.</p><code>html.ToWordDocument().SaveAsByteArray()</code></article>
<article class="ocx-capability-card"><span class="ocx-chip ocx-chip--warn">Next browser route</span><h3>Markdown to PDF</h3><p>Generate Word from Markdown first, then call the PDF engine with diagnostics.</p><code>markdown.LoadFromMarkdown().SaveAsPdf(options)</code></article>
