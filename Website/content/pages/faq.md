...
meta.faq.questions:
  ...
  - "Does OfficeIMO support DOC, XLS, XLSB, and PPT as well as DOCX, XLSX, and PPTX?|Current repository source classifies and reads modern and legacy Word, Excel, and PowerPoint families, writes documented native subsets, and reports conversion coverage by operation and fidelity. Check the installed package version because released packages can lag the source documentation."
  - "What .NET versions are supported?|Current repository source targets .NET 8.0, .NET 10.0, .NET Standard 2.0, and .NET Framework 4.7.2. Check the selected package on NuGet for its published target frameworks."
  ...
  - "What is PSWriteOffice?|PSWriteOffice is the first-party PowerShell surface over OfficeIMO. This site derives its command catalog from a pinned source snapshot; the PowerShell Gallery is the authority for the published module version."
  ...
  - "Is NativeAOT compilation supported?|The current source validation matrix covers 92 of 93 production projects: 90 fully rooted libraries, one bounded Google APIs workflow, one native command-line tool, and one managed-only WPF/WebView2 renderer. Check the matrix for the exact package and runtime path."
