...
meta.faq.questions:
  ...
  - "Does OfficeIMO support DOC, XLS, XLSB, and PPT as well as DOCX, XLSX, and PPTX?|OfficeIMO 3.2 classifies and reads modern and legacy Word, Excel, and PowerPoint families, writes documented native subsets, and reports conversion coverage by operation and fidelity."
  - "What .NET versions are supported?|OfficeIMO 3.2 targets .NET 8.0, .NET 10.0, .NET Standard 2.0, and .NET Framework 4.7.2."
  ...
  - "Is NativeAOT compilation supported?|The OfficeIMO 3.2 validation matrix covers 99 of 101 production projects: 97 fully rooted libraries, one bounded Google APIs workflow, one native command-line tool, one managed cross-platform Chromium bridge, and one managed-only WPF/WebView2 renderer. Check the matrix for the exact package and runtime path."
