...
The [machine-readable project matrix](/data/aot-compatibility.json) names all 118 production projects and records which proof applies to each one. This distinction matters to customers: a green native workflow is useful evidence, but it is not permission to assume that every optional third-party API has been executed.
...
| Managed cross-platform applications and integrations | 3 | `OfficeIMO.Html.Pdf.Browser` and the local HTML/PDF workbench use the managed HtmlTinkerX and Playwright runtime; OfficeIMO Studio uses its managed Avalonia desktop distribution profiles. | Use managed .NET deployment for these optional applications and integrations; do not advertise them as NativeAOT-compatible without separate native-publish proof. |
