...
The [machine-readable project matrix](/data/aot-compatibility.json) names all 115 production projects and records which proof applies to each one. This distinction matters to customers: a green native workflow is useful evidence, but it is not permission to assume that every optional third-party API has been executed.
