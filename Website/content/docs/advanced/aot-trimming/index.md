...
- **87 production libraries** are fully rooted as complete assemblies in one native compile graph; the resulting executable must start successfully.
...
|---|---:|---|---|
| Fully rooted libraries | 87 | The complete assembly surfaces compile into one NativeAOT executable on Windows and Linux, and that executable starts. | These packages are suitable NativeAOT building blocks; still test the exact documents and options your application uses. |
