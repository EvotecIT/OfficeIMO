...
The current matrix validates **114 of 121 production projects** for NativeAOT: **111 libraries are fully rooted** across the main native host and dedicated hosts for `OfficeIMO.Security` and `OfficeIMO.Provenance.C2pa`, the optional Google APIs adapter runs a bounded token-store workflow, the production CLI tool publishes and starts natively, and the row-mapping source generator emits code exercised by the native host. Seven applications and integrations use managed deployment: the Chromium browser-PDF bridge, local HTML/PDF workbench, and Avalonia-based OfficeIMO Studio run cross-platform; the document-AI engine, IntelligenceX adapter and headless example target managed .NET 10 without a NativeAOT claim. The WPF/WebView2 renderer stays on Windows because the .NET SDK rejects trimming for WPF executables.
