...
| Ami Pro SAM 4 | Structured | `[tag]` style definitions, `[edoc]` paragraphs, basic character styles, half-point font sizes, fonts, RGB color, alignment, spacing, page-break and keep properties, and applied source style names | the current structured profile is ASCII-only and rejects undeclared extended bytes; code-page support, frames, equations, images, tables, and additional inline tags remain open |
