...
public sealed partial class EmailStorePstMutationTransaction {
    ...
    private EmailStorePstMutationReport CreateCommittedReport(string? committedBackupPath,
        EmailStorePstMutationPlan plan, EmailStorePstWriteReport writeReport,
        EmailStorePstMutationVerificationReport? verification,
        Dictionary<string, string> folderMap, Dictionary<string, string> itemMap) {
        ...
        int copiedItems = _items.Values.Count(item => item.IsCreated && !item.Deleted && item.CopiedFromId != null);
        int replacedItems = _items.Values.Count(item => !item.IsCreated && !item.Deleted && item.Replaced);
        int patchedItems = _items.Values.Count(item => !item.Deleted &&
            (item.PropertyPatchChanges > 0 || item.AttachmentPatchChanges > 0));
        ...
        IReadOnlyList<EmailStorePstMutationOperationResult> operationResults =
            BuildOperationResults(plan, folderMap, itemMap);
        var report = new EmailStorePstMutationReport(_sourcePath, committedBackupPath, plan,
            writeReport, verification, createdFolders, renamedFolders, movedFolders,
            deletedFolders, addedItems, copiedItems, replacedItems, patchedItems, movedItems, deletedItems,
    ...
    private static IReadOnlyList<EmailStorePstMutationOperationResult> BuildOperationResults(
        EmailStorePstMutationPlan plan, IReadOnlyDictionary<string, string> folderMap,
        IReadOnlyDictionary<string, string> itemMap) {
        var results = new List<EmailStorePstMutationOperationResult>(plan.Operations.Count);
        foreach (EmailStorePstMutationPlanOperation operation in plan.Operations) {
            string? destination;
            switch (operation.Kind) {
                case EmailStorePstMutationOperationKind.CreateFolder:
                case EmailStorePstMutationOperationKind.RenameFolder:
                case EmailStorePstMutationOperationKind.MoveFolder:
                    folderMap.TryGetValue(operation.EntityId, out destination);
                    break;
                case EmailStorePstMutationOperationKind.DeleteFolder:
                case EmailStorePstMutationOperationKind.DeleteItem:
                    destination = null;
                    break;
                default:
                    itemMap.TryGetValue(operation.EntityId, out destination);
                    break;
            }
            results.Add(new EmailStorePstMutationOperationResult(operation, destination));
        }
