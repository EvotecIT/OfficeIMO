...
internal sealed class PstNamedPropertyWriter {
    ...
    private PstNamedPropertyWriter(bool seedDefaults) {
        ...
        GetOrAdd(MapiKnownProperties.PidName.Keywords.Name!);
        GetOrAdd(MapiKnownProperties.PidLid.ReminderDelta.Name!);
    }
    ...
    internal IReadOnlyList<MapiProperty> BuildProperties() {
        ...
        var properties = new List<MapiProperty> {
            Property(MapiKnownProperties.PidTag.NameidBucketCount, BucketCount),
            Property(MapiKnownProperties.PidTag.NameidStreamGuid, guidStream.ToArray()),
            Property(MapiKnownProperties.PidTag.NameidStreamEntry, entryStream),
            Property(MapiKnownProperties.PidTag.NameidStreamString, stringStream.ToArray())
        };
    ...
    private static MapiProperty Property<T>(MapiPropertyKey<T> key, T value) =>
        new MapiProperty(key.PropertyId!.Value, key.PreferredType, value);
    ...
    private static int GetGuidIndex(Guid guid, bool stringNamed,
        ...
        if (guid == MapiPropertySets.Mapi) return 1;
        if (guid == MapiPropertySets.PublicStrings) return 2;
