...
internal static class PstTableContextWriter {
    ...
    private static void AssignLayout(Column[] columns, out int dataEnd,
        ...
        Column rowId = columns.Single(item => item.PropertyId == MapiKnownProperties.PidTag.LtpRowId.PropertyId);
        Column rowVersion = columns.Single(item => item.PropertyId == MapiKnownProperties.PidTag.LtpRowVer.PropertyId);
        rowId.DataOffset = 0;
