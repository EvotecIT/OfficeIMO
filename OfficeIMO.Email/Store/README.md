...
`OfficeIMO.Email.Store` is the fully managed mailbox-store API area included in the `OfficeIMO.Email` package for PST, OST, OLM, EMLX, Mbox, Apple Mail,
...
## Query typed store tables

Use the typed query API when an application needs composable filters, selected columns, deterministic sorting, and
stable keyset paging instead of a runtime string filter:

```csharp
var query = new EmailStoreTableQuery(
    folderId: session.FolderCatalog.FindSpecialFolder(EmailStoreSpecialFolderKind.Inbox)!.Key,
    filter: EmailStoreFields.Subject.Contains("invoice") &
            EmailStoreFields.HasAttachments.EqualTo(true),
    sorts: new[] {
        EmailStoreFields.ReceivedAt.Descending(),
        EmailStoreFields.Subject.Ascending()
    },
    projection: new EmailStoreProjection(
...
while (page.NextToken != null) {
    page = session.SearchPage(query.ContinueFrom(page.NextToken));
}
```

Continuation tokens include the query signature and stable folder/item tie-breakers; a token cannot be reused with a
different scope, filter, or sort. `Explain()` reports whether execution reads complete items. The current store
backends evaluate lightweight summaries offline and materialize only matching rows for sorting.

`EnumerateItemsAsync` supplies the `await foreach` pattern on every target without an async-interface compatibility
package. It observes cancellation per item and does not hide a whole-store scan in `Task.Run`; individual backend
reads remain ordered and synchronous.
...
EmailStorePstMutationReport mutationReport = await mutation.CommitAsync();
...
copy items and folder subtrees; apply typed or raw MAPI property patches; add, replace, or remove attachments; apply
...
them with a worker-thread wrapper. Use `Commit()` when a fully synchronous workflow is preferable.
...
## Read Outlook folder-associated data and reminders

`ReadAssociatedData` scans only folder-associated information tables and returns one bounded catalog. It includes
lossless master-category lists, roaming XML/dictionaries, view envelopes, opaque rule-organizer streams with stable
fingerprints, search-folder definitions and folder flags, and folder user-property definitions:

```csharp
EmailStoreAssociatedDataCatalog data = session.ReadAssociatedData(
    new EmailStoreAssociatedDataOptions(maxItems: 10_000));

EmailStoreCategoryList categories = data.CategoryLists.Single().CategoryList!;
categories.Set("Customer", color: 7, keyboardShortcut: 1);

EmailStoreAssociatedItem? calendarOptions =
    data.FindEffectiveConfiguration("IPM.Configuration.Calendar");
```

Typed editors preserve XML elements and attributes they do not understand. Rule, view, and search-folder bytes stay
available when OfficeIMO cannot decode their complete grammar. Folder `Mapi` bags are detached read snapshots; writes
still require an explicit mutation transaction.

`GetReminders` builds a bounded offline reminder queue at an explicit `AsOf` time. It reports whether each signal came
from `PidLidReminderSignalTime`, the reminder time, or an appointment start-minus-delta calculation. Deleted, Junk,
Drafts, Outbox, conflict, failure, and sync folders are excluded by default and can be included explicitly.

`BuildConversationGraph` combines Internet message IDs/references, Outlook conversation IDs/index roots, and meeting
or task lifecycle identifiers. Subject-only edges are marked heuristic rather than silently treated as authoritative.

## Recover, compact, and split PST data

`ExportRecoverableItemsToDirectory` exports each readable indexed or orphaned item independently and writes a
provenance manifest, so one corrupt item does not invalidate earlier output when continuation is enabled.

Compaction and split are plan-first operations:

```csharp
EmailStorePstCompactionPlan compact = session.PlanPstCompaction("archive.compact.pst");
if (compact.IsExecutable) {
    EmailStorePstCompactionReport report = session.CompactToPst("archive.compact.pst");
}

EmailStorePstSplitPlan split = session.PlanPstSplit(
    "archive-part.pst",
    new EmailStorePstSplitOptions(maxEstimatedBytesPerPart: 10L * 1024 * 1024 * 1024));
if (split.IsExecutable) {
    EmailStorePstSplitReport report = session.ExecutePstSplit(split);
}
```

Compaction always writes a distinct PST, reopens it, and semantically verifies every selected item. Split selection can
reuse the typed query AST; each part is staged and verified before the complete output set is committed, with rollback
protection for replaced destinations. Part sizes are estimates until serialization completes.
...
See the [OfficeIMO.Email package overview](../README.md) for related formats and Reader adapters.
