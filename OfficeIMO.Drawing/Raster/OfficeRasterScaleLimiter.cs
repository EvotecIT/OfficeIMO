...
public static class OfficeRasterScaleLimiter {
    ...
    public static OfficeRasterScaleLimit Resolve(double width, double height, double requestedScale, long maximumPixels) {
        ...
        if (Fits(width, height, requestedScale, maximumPixels, out int requestedWidth, out int requestedHeight)) {
        ...
        if (!Fits(width, height, limited, maximumPixels, out _, out _)) {
            double lower = 0D;
            for (int iteration = 0; iteration < 80; iteration++) {
                double candidate = lower + (upper - lower) / 2D;
                if (Fits(width, height, candidate, maximumPixels, out _, out _)) lower = candidate;
                else upper = candidate;
            }
            limited = lower > 0D ? lower : double.Epsilon;
        }

        if (!Fits(width, height, limited, maximumPixels, out int pixelWidth, out int pixelHeight)) {
            throw new InvalidOperationException("The raster scale could not be reduced to the requested decoded-pixel limit.");
        }
        return new OfficeRasterScaleLimit(limited, pixelWidth, pixelHeight, true);
    }

    private static bool Fits(double width, double height, double scale, long maximumPixels, out int pixelWidth, out int pixelHeight) {
        pixelWidth = 0;
        pixelHeight = 0;
        ...
        return pixelWidth <= maximumPixels / pixelHeight;
    }
