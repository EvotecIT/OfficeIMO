...
public static partial class OfficeDrawingSvgExporter {
    private static void AppendEffectGroup(StringBuilder sb, OfficeDrawingEffectGroup effectGroup, IOfficeRasterImageCodec? imageCodec, string idPrefix, ref int gradientId, ref int clipPathId) {
        string? maskId = null;
        if (effectGroup.SoftMask != null) {
            maskId = idPrefix + "officeimo-mask-" + (++clipPathId).ToString(CultureInfo.InvariantCulture);
            AppendSoftMaskDefinition(sb, maskId, effectGroup.SoftMask, imageCodec, idPrefix, ref gradientId, ref clipPathId);
        }
        ...
        sb.Append('>');
        AppendElements(sb, effectGroup.InnerDrawing.Elements, imageCodec, idPrefix, ref gradientId, ref clipPathId);
        sb.Append("</g>");
    }

    private static void AppendSoftMaskDefinition(StringBuilder sb, string id, OfficeDrawingSoftMask mask, IOfficeRasterImageCodec? imageCodec, string idPrefix, ref int gradientId, ref int clipPathId) {
        ...
        AppendElements(sb, mask.InnerDrawing.Elements, imageCodec, idPrefix, ref gradientId, ref clipPathId);
