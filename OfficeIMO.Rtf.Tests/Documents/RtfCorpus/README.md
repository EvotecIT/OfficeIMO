...
- a deterministic Outlook 16 in-memory MAPI `RTFBody` fixture with genuine `fromhtml`/`htmltag` encapsulation;
- an immutable external Microsoft WordPad sample with byte/hash, self-identifying text, and native RichEdit generator evidence;
...
Do not relabel synthetic grammar coverage as producer evidence. `corpus-manifest.json` records external paths for WordPad, Google Docs, macOS TextEdit/RTFD, a redacted Epic EHI export, a Salesforce configuration-report workflow, and a helpdesk RichEdit artifact. The CRM and helpdesk artifacts are workflow evidence, not vendor-native exports. The checked GemBox.Document fixture is reproducibly generated from the exact package version under `Build/ProducerCorpus`.
