...
- Simple worksheet header/footer text zones, first-page and even-page text variants, and supported header/footer images, including page number, total page count, sheet-name, date, time, workbook file-name, and workbook path tokens. Simple line-level header/footer font family/style, font size, and RGB text color are mapped when the styled text can be represented by one first-party PDF header/footer line style.
