...
`--allow-remote` authorizes the selected source evidence to leave the machine. `--images` additionally sends selected page images and requires `--model-supports-images`, an explicit declaration that the chosen model accepts images. Without `--codex-session`, the native route uses IX's default authentication selection. Existing output files are never overwritten.
...
Add `--images --model-supports-images` for scanned pages or visual tables. Use `"operation": "Parse"` with an instruction and no fields to produce proposed blocks/tables and `proposed-reader.json`. `Ask`, `Explain`, and `Summarize` likewise take an instruction without field definitions.
...
```powershell
dotnet run --project Examples/OfficeIMO.AI.Example/OfficeIMO.AI.Example.csproj -c Release --no-build -- --evaluate --allow-remote --codex-session --model-supports-images --output output/evaluation
```
...
XLSX cells support at most 32,767 UTF-16 code units and require XML-compatible characters. Supplementary Unicode characters use two units each, so an otherwise valid AI result can exceed that export limit. The example prepares all artifacts in memory and verifies the workbook before creating output files. It rejects oversized or XML-invalid cells and values that change during workbook readback, including carriage returns normalized by the current Excel writer. Applications that need these values can serialize the result with `OfficeAiArtifacts.SerializeReport`.
...
For native Copilot, use `--copilot --allow-remote --model <available-model>`. `OFFICEIMO_AI_API_KEY` can supply a GitHub credential, or the SDK can read `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `GITHUB_TOKEN`. No CLI is required. Select a vision-capable model for image cases; `--text-only` explicitly excludes those cases and records their identifiers in the report. Omit `--prompted-json` for models that enforce JSON Schema. Model availability and subscription access must be verified for the account.
