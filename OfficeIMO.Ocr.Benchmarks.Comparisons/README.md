...
OfficeIMO supports Tesseract as the default local engine. Caller-owned neural, native, or cloud providers implement the format-neutral `IOcrEngine` contract; `OfficeIMO.Reader.Ocr` and `OfficeIMO.Pdf.Ocr` consume that same engine without provider-specific bridges.
...
```powershell
.\OfficeIMO.Ocr.Benchmarks.Comparisons\Invoke-OcrEngineComparison.ps1 -Plan
.\OfficeIMO.Ocr.Benchmarks.Comparisons\Invoke-OcrEngineComparison.ps1 -IterationCount 1 -Case clean-english
.\OfficeIMO.Ocr.Benchmarks.Comparisons\Invoke-OcrEngineComparison.ps1 -IterationCount 3
```
