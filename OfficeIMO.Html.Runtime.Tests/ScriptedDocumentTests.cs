...
public sealed class ScriptedDocumentTests {
    ...
    public async Task TerminatesRunawayExecutionAtTheDeadline(string stage) {
        ...
        if (stage == "frame-module") {
            var frame = new Uri(request.DocumentUrl, "runaway-module-frame.html");
            request.Html = "<iframe src='/runaway-module-frame.html'></iframe>";
            request.Resources = new[] { HtmlRuntimeResource.FromText(frame,
                "<script type='module'>while(true){}</script>", "text/html; charset=utf-8") };
        }
