...
namespace OfficeIMO.Excel.Fluent {
    ...
    public sealed partial class SheetComposer {
        ...
        public SheetComposer(ExcelDocument workbook, string sheetName, SheetTheme? theme = null) {
            ...
            _sheet = _workbook.AddWorksheet(sheetName, ExcelSheetNameValidationMode.Sanitize);
            _row = 1;
            ...
            static IEnumerable<string> Humanize(string segment) {
                ...
                foreach (var token in raw.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)) {
                    ...
                    for (int i = 0; i < token.Length; i++) {
                        char c = token[i];
                        if (i > 0 && char.IsUpper(c) && (char.IsLower(token[i - 1]) || (i + 1 < token.Length && char.IsLower(token[i + 1])))) { parts.Add(sb.ToString()); sb.Clear(); }
                        sb.Append(c);
                    }
