...
- The same 2026-05-13 Release read comparison shows the strongest OfficeIMO shape on bounded reads: first-100-row range read averaged `4.5 ms` versus `95.6 ms` for ClosedXML, `30.2 ms` for EPPlus, and `27.3 ms` for EPPlus 4.5.3.3; first-100-row streaming averaged `3.9 ms` versus `78.3 ms`, `25.4 ms`, and `27.1 ms`.
- Typed `ReadObjects<T>()` still leads ClosedXML in the refreshed comparison (`55.4 ms` versus `90.3 ms`), but current and legacy EPPlus lead this scenario at about `31.0 ms`; this remains a priority parity target.
- Full 2,500-row dense reads are now honest parity targets rather than solved claims: `ReadRange()` averaged `67.6 ms` versus `103.4 ms` for ClosedXML, `51.0 ms` for EPPlus, and `33.4 ms` for EPPlus 4.5.3.3; `ReadRangeStream()` averaged `56.5 ms` versus `107.6 ms`, `42.1 ms`, and `32.8 ms`.
