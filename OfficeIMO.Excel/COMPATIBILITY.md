...
- The same 2026-05-13 Release read comparison shows the strongest OfficeIMO shape on bounded reads: first-100-row range read averaged `4.1 ms` versus `114.9 ms` for ClosedXML, `37.9 ms` for EPPlus, and `28.5 ms` for EPPlus 4.5.3.3; first-100-row streaming averaged `4.8 ms` versus `116.9 ms`, `38.9 ms`, and `28.0 ms`.
- Typed `ReadObjects<T>()` still leads ClosedXML in the refreshed comparison (`91.5 ms` versus `114.1 ms`), but current and legacy EPPlus lead this scenario at `30.0 ms` and `35.5 ms`; this remains a priority parity target.
- Full 2,500-row dense reads are now honest parity targets rather than solved claims: `ReadRange()` averaged `67.8 ms` versus `129.8 ms` for ClosedXML, `50.7 ms` for EPPlus, and `36.6 ms` for EPPlus 4.5.3.3; `ReadRangeStream()` averaged `70.4 ms` versus `116.5 ms`, `50.1 ms`, and `33.9 ms`.
