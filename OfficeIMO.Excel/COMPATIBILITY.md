...
- The same 2026-05-13 Release read comparison shows the strongest OfficeIMO shape on bounded and typed reads: first-100-row range read averaged `5.1 ms` versus `114.7 ms` for ClosedXML, `34.6 ms` for EPPlus, and `29.5 ms` for EPPlus 4.5.3.3; first-100-row streaming averaged `2.9 ms` versus `59.4 ms`, `35.1 ms`, and `28.1 ms`; typed `ReadObjects<T>()` averaged `26.1 ms` versus `43.3 ms`, `43.2 ms`, and `41.0 ms`.
- Full 2,500-row dense reads are now honest parity targets rather than solved claims: `ReadRange()` averaged `77.8 ms` versus `114.8 ms` for ClosedXML, `44.3 ms` for EPPlus, and `34.4 ms` for EPPlus 4.5.3.3; `ReadRangeStream()` averaged `82.7 ms` versus `96.8 ms`, `44.1 ms`, and `37.6 ms`.
