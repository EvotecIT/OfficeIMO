...
- The 2026-05-13 local Release read comparison records build configuration, medians, and raw samples against `ClosedXML`, current `EPPlus`, and isolated `EPPlus 4.5.3.3`. `OfficeIMO.Excel` is ahead of `ClosedXML` on every covered read scenario in that snapshot, but current and legacy EPPlus still lead the full dense range scans.
- The same 2026-05-13 Release read comparison shows the strongest OfficeIMO shape on bounded reads: first-100-row range read averaged `3.5 ms` versus `73.4 ms` for ClosedXML, `31.7 ms` for EPPlus, and `26.9 ms` for EPPlus 4.5.3.3; first-100-row streaming averaged `3.4 ms` versus `69.0 ms`, `30.9 ms`, and `26.4 ms`.
- Typed `ReadObjects<T>()` still leads ClosedXML in the refreshed comparison (`67.4 ms` versus `100.4 ms`), but current and legacy EPPlus lead this scenario at about `30.3 ms` and `30.6 ms`; this remains a priority parity target.
- Full 2,500-row dense reads are now honest parity targets rather than solved claims: `ReadRange()` averaged `54.5 ms` versus `78.6 ms` for ClosedXML, `41.5 ms` for EPPlus, and `32.8 ms` for EPPlus 4.5.3.3; `ReadRangeStream()` averaged `46.3 ms` versus `72.9 ms`, `37.4 ms`, and `31.8 ms`.
