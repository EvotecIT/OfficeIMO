...
| Charts | Partial | Common chart authoring is present, including dashboard recipe helpers, KPI/contribution/variance-bridge presets, axis titles/formats/scale/gridlines, series styling, trendlines, secondary-axis combo scenarios, and pivot-source chart metadata, but breadth and round-trip parity are still behind top-tier competitors. |
| Pivot tables | Partial | Source-range pivot creation supports row/column/page/data fields, layouts, styles, display flags, captions, data-field number formats, show-values-as data field options, field sort/subtotal/display options, field item visibility filters, and selected page-field items. Date/number grouping and advanced value/label filters remain parity targets. |
...
| Comments and notes | Partial | Legacy worksheet comments can be authored, inspected, found, updated, and removed by author, text, and A1 range. Rich-text comment authoring and threaded-comment editing remain roadmap items. |
