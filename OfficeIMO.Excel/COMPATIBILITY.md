...
| Feature inspection | Partial | `InspectFeatures()` reports editable, partially editable, preserved, and unsupported workbook features, with detail entries for preservation-sensitive package parts and external relationships. Targeted fail-fast guards can reject named features or support levels before edit-heavy workflows. Broader corpus-backed preservation proof remains a target. |
...
- A 2026-05-18 local Release comparison covers the automatic direct DataSet export path through normal `InsertDataSet(...)` plus normal `Save(...)`. At 25,000 rows, `OfficeIMO.Excel` averaged `42.9 ms` for styled DataSet tables, `45.6 ms` with AutoFit, and `35.2 ms` for sparse tables. In the same run, ClosedXML averaged `414.2 ms`, `759.6 ms`, and `530.8 ms`, while current EPPlus averaged `265.8 ms`, `344.9 ms`, and `406.6 ms`. This is a covered export-shaped workload, not a blanket claim for feature-rich edited workbooks.
...
- Formula calculation can now be invoked directly with `doc.Calculate()` or scoped to a single save with `ExcelSaveOptions.EvaluateFormulasBeforeSave`; supported same-sheet formula dependencies, numeric cross-sheet references, A1-backed named ranges, simple table structured references, common text helpers, and exact-match lookup formulas returning text are evaluated in-memory, while unsupported formulas stay intact, get clearer inspection diagnostics, and can be paired with `ForceFullCalculationOnOpen` when Excel should finish the workbook calculation.
