...
The inspector reports nearly transparent, tiny, and low-contrast OCR regions using bounded geometry and conservative pixel evidence. Redaction is deliberately disabled by default. When enabled, only sufficiently confident, currently matching findings can be selected; the workflow covers their bounded regions with the configured opaque color, emits a single-frame PNG derivative, reopens it, verifies every output pixel, and reruns OCR inspection under the same captured engine identity and capabilities. A selected region, including its configured padding, must not overlap any recognized span that was not also selected. Cumulative limits bound both region-pixel work and OCR-region intersection comparisons. This is destructive rectangular coverage, not semantic image editing. Multi-frame images, unsupported geometry, oversized provider output, input or analysis work, provider errors or non-recoverable diagnostics, color-rendering metadata that the managed decoder cannot normalize to sRGB, unsupported embedded orientation, and outputs where OCR still recognizes any text in a changed region all fail closed.
