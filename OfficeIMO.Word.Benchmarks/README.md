...
The suite covers package load, field refresh, mail merge, structured comparison, bounded VBA signature inspection over 100 KiB and 1,000 KiB projects, Word-to-HTML including package load, and Word-to-HTML over an already loaded document. Global setup executes each workflow once and rejects unexpected field, merge, paragraph, HTML-output, macro-project length, or comparison results before BenchmarkDotNet starts timing. Temporary packages are created under the operating-system temporary folder and removed by benchmark cleanup.
