        ...
        public async Task<GoogleDriveResumableUploadResult> UploadResumableStreamAsync(Stream content, long length,
            ...
            if (checkpoint == null) {
                ...
                GoogleWorkspaceHttpResponse initiation = await Transport.SendRawAsync(token, HttpMethod.Post, initUri,
                    ...
                    request => { request.Headers.TryAddWithoutValidation("X-Upload-Content-Type", options.ContentType); request.Headers.TryAddWithoutValidation("X-Upload-Content-Length", length.ToString(System.Globalization.CultureInfo.InvariantCulture)); }).ConfigureAwait(false);
            ...
            while (offset < length) {
                ...
                if (status.StatusCode == HttpStatusCode.OK || status.StatusCode == HttpStatusCode.Created) offset = length;
                else {
                ...
                if (offset == length && (status.StatusCode == HttpStatusCode.OK || status.StatusCode == HttpStatusCode.Created)) {
                    EnsureSourceUnchanged(content, length, sourceFingerprint, cancellationToken);
                    return new GoogleDriveResumableUploadResult(status.DeserializeJson(GoogleDriveJsonSerializerContext.Default.GoogleDriveFile), state);
                }
            }
