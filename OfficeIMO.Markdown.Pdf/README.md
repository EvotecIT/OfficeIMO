...
MarkdownDoc document = MarkdownDoc.Parse("""
# Status
...
File-based conversion uses the native Markdown lifecycle so a string path is not confused with Markdown text:

```csharp
MarkdownDoc.Load("README.md").SaveAsPdf("README.pdf");
```
...
markdown.SaveAsPdfFromMarkdown("incident-review.pdf", new MarkdownToPdfOptions {
...
MarkdownDoc.Load("Docs/status.md").SaveAsPdf("Artifacts/status.pdf", options);
```
...
MarkdownDoc document = MarkdownDoc.Parse(markdown);

document.SaveAsHtml("report.html", new HtmlOptions { Theme = theme });
document.SaveAsPdf("report.pdf", new MarkdownToPdfOptions { Theme = theme });
...
var result = document.SaveAsPdfResult("README.pdf", options);
if (!result.Succeeded) {
...
- Profile presets through `MarkdownToPdfOptions.UseProfile(...)`, plus shared `TextFallbacks` and `ResourcePolicy` controls. Profiles may disable images for lightweight/text-only output but never grant local, remote, or host-font access.
...
MarkdownDoc.Load("README.md").SaveAsPdf("README.pdf", options);
```
