...
PDF reading now accepts at most 4,096 text-show clipping paths across one page content tree, including nested Form XObjects. The limit counts one path for each non-empty shown-string run from `Tj`, `'`, `"`, and each string entry in a `TJ` array, not individual glyphs. Older versions reset the count at each text object and could perform unbounded path intersections across consecutive objects; current versions throw `PdfReadLimitException` with `Kind == PdfReadLimitKind.TextClippingPaths` when the aggregate path ceiling is exceeded, or `PdfReadLimitKind.TextClippingIntersectionWork` when estimated contour-intersection work exceeds the fixed page budget. These safety ceilings are not configurable through `PdfReadLimits`. Applications that accept external PDFs should handle either exception as an unsupported or over-complex input. Trusted producers must simplify clipping text or reduce consecutive clipping-mode text objects before OfficeIMO reads the file.
