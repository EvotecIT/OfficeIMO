...
internal static class PdfActionPayloadFingerprint {
    ...
    internal static string Create(
        ...
        int nodes = 0;
        AppendDictionary(builder, action, objects, pageNumbers, activeReferences, depth: 0, ref nodes, isActionRoot: true);
    ...
    private static void AppendObject(
        ...
        switch (value) {
            ...
            case PdfReference reference:
                AppendReference(builder, reference, objects, pageNumbers, activeReferences, depth, ref nodes);
                return;
            case PdfArray array:
                builder.Append('[');
                for (int i = 0; i < array.Items.Count; i++) {
                    AppendObject(builder, array.Items[i], objects, pageNumbers, activeReferences, depth + 1, ref nodes);
                    builder.Append(';');
                }
                builder.Append(']');
                return;
            case PdfDictionary dictionary:
                AppendDictionary(builder, dictionary, objects, pageNumbers, activeReferences, depth + 1, ref nodes, isActionRoot: false);
                return;
            case PdfStream stream:
                builder.Append("stream:");
                AppendDictionary(builder, stream.Dictionary, objects, pageNumbers, activeReferences, depth + 1, ref nodes, isActionRoot: false);
    ...
    private static void AppendReference(
        ...
        try {
            ...
            AppendObject(builder, indirect.Value, objects, pageNumbers, activeReferences, depth + 1, ref nodes);
        } finally {
    ...
    private static void AppendDictionary(
        ...
        foreach (string key in dictionary.Items.Keys.OrderBy(static key => key, StringComparer.Ordinal)) {
            ...
            AppendObject(builder, dictionary.Items[key], objects, pageNumbers, activeReferences, depth + 1, ref nodes);
            builder.Append(';');
        }
        builder.Append('}');
    }
