...
public sealed partial class PdfReadPage {
    ...
    private Dictionary<string, PdfPageGraphicsStateResource> GetGraphicsStateResources(PdfDictionary? resources) {
        ...
        foreach (KeyValuePair<string, PdfObject> entry in extGStates.Items) {
            ...
            bool unsupportedBlendMode = state.Items.ContainsKey("BM") && !blendMode.HasValue;
            PdfObject? resolvedSoftMask = ResolveObject(state.Items.TryGetValue("SMask", out PdfObject? softMaskObject) ? softMaskObject : null);
            bool unsupportedSoftMask = hasSoftMask && resolvedSoftMask is not PdfName { Name: "None" } && softMask == null;
            if (fillOpacity.HasValue ||
                ...
                unsupportedSoftMask ||
                hasSoftMask) {
                result[entry.Key] = new PdfPageGraphicsStateResource(fillOpacity, strokeOpacity, strokeWidth, strokeDashStyle, strokeLineCap, strokeLineJoin, blendMode, hasSoftMask, softMask, unsupportedBlendMode, unsupportedSoftMask);
            }
        }
