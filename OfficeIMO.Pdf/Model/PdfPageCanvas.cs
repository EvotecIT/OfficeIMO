...
public sealed partial class PdfPageCanvas {
    ...
    internal PdfPageCanvas ForegroundImage(byte[] bytes, double x, double y, double width, double height,
        bool horizontalFlip = false, bool verticalFlip = false) => ImageShared(PdfCanvasImageResource.Create(bytes),
            x, y, width, height, horizontalFlip: horizontalFlip, verticalFlip: verticalFlip, foreground: true);

    internal PdfPageCanvas ImageShared(PdfCanvasImageResource imageResource, double x, double y, double width, double height, PdfImageStyle? style = null, string? linkUri = null, string? linkContents = null, string? alternativeText = null, double rotationAngle = 0D, bool horizontalFlip = false, bool verticalFlip = false, bool foreground = false) {
        ...
        _items.Add(new PdfCanvasImageItem(new ImageBlock(imageResource.Bytes, width, height, imageInfo, imageStyle, linkUri, linkContents, useDataSnapshot: true), x, y, rotationAngle, horizontalFlip, verticalFlip, foreground));
        return this;
    }
...
internal sealed class PdfCanvasImageItem : PdfCanvasItem {
    public PdfCanvasImageItem(ImageBlock block, double x, double y, double rotationAngle, bool horizontalFlip, bool verticalFlip, bool foreground = false)
        : base(x, y) {
        Block = block;
        RotationAngle = rotationAngle;
