...
internal static partial class PdfTextEditor {
    ...
    private static PositionedTextFragment[] BuildPositionedFragments(
        ...
        for (int index = 0; index < ordered.Length; index++) {
            ...
            if (edit.Replacement.Length > 0) AddPositionedFragment(fragments, edit.Replacement, edit.Style ?? replacementStyle);
    ...
    private readonly struct SpanTextEdit {
        internal SpanTextEdit(int start, int length, string replacement, PdfResolvedTextStyle? style = null) { Start = start; Length = length; Replacement = replacement; Style = style; }
