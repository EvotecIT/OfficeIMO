...
internal static partial class PdfFormFiller {
    private static void ValidateFlattenFieldNames(IReadOnlyCollection<string> fieldNames, System.Threading.CancellationToken cancellationToken = default) {
    ...
    private static void CollectSelectedFlattenFields(
        ...
        for (int i = owner.Items.Count - 1; i >= 0; i--) {
            ...
            CollectSelectedFlattenFields(objects, kids, requested, matched, fieldType, fieldFlags, fieldMaxLength, defaultResources, defaultAppearance, displayValue, richRuns, fullName, choiceOptions, options, widgets, removableObjects, ref nextObjectNumber, cancellationToken);
    ...
    private static void FilterCalculationOrder(Dictionary<int, PdfIndirectObject> objects, PdfDictionary acroForm, HashSet<int> removedObjects, System.Threading.CancellationToken cancellationToken = default) {
        cancellationToken.ThrowIfCancellationRequested();
