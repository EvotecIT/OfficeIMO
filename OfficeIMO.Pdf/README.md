...
The PDF engine is useful and broad, but it is still evolving. It has strong first-party coverage for common generated business documents, reusable Unicode line breaking and Latin ligatures, bounded built-in core-Arabic shaping plus an optional HarfBuzz adapter for full GSUB/GPOS shaping, bounded Type 3 vector, normalized raster-image, colored tiling/shading-pattern, and outer colored, tinted, DeviceGray/DeviceRGB intersecting axial-shading fill, or increasing concentric radial-shading fill pattern paint for vector and image-mask PaintType 2 glyph content, standard blend-mode, supported alpha/luminosity soft-mask, isolated non-knockout transparency-group glyph-program rendering with explicit DeviceRGB blending color spaces including inherited pattern paint through vector and image-mask group content, and named OCG plus inline OCMD visibility inside glyph programs and nested Forms. It also supports authored and bounded-synthesized annotation appearances in page images, conservative read/manipulation workflows, password security, optional provider-backed certificate signing/validation, standards-compliant Fast Web View output, and bounded-payload stream saves with runtime serialization evidence. Shading BBoxes, nonlinear or out-of-range shading functions, non-intersecting axial placements, shading-pattern strokes, nonconcentric or decreasing radial families, implicit/non-isolated/knockout/non-RGB Type 3 transparency groups, ordinary color-image PaintType 2 glyph content, advanced or recursively nested patterns, and advanced optional-content usage-application or intent behavior retain diagnosed limits; unsupported content-paint color spaces, difficult producer-specific preservation, broader transparency edge cases, and genuinely forward-only layout remain deeper current-state areas.
