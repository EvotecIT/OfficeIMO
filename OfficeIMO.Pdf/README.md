...
The PDF owner recognizes the standards-defined embedded file with media type `application/c2pa` and `/AFRelationship /C2PA_Manifest`. Removal uses the bounded attachment rewrite and validates attachment readback, preserving unrelated attachments. Malformed candidates remain untouched unless the caller disables `RequireStructurallyValidCarrier`. A signed PDF is never silently rewritten or stripped; handle its signature through an explicit PDF signature workflow first. Optional cryptographic C2PA verification is provided by `OfficeIMO.Security`.
