...
public static class PdfRenderCapabilities {
    ...
    internal const string Type3FontSubstitutionId = "render.resource.font-type3-substitution";
    ...
    internal const string IccMabColorSpaceId = "render.colorspace.icc-mab";
    internal const string IccOutputIntentId = "render.colorspace.icc-output-intent";
    internal const string UnsupportedIccOutputIntentId = "render.colorspace.icc-output-intent-unsupported";
    internal const string OutputIntentTransparencyId = "render.colorspace.icc-output-intent-transparency-simplified";
    ...
    internal const string UnsupportedShadingId = "render.resource.shading-unsupported";
    ...
    private static System.Collections.ObjectModel.ReadOnlyCollection<PdfRenderCapability> BuildRegistry() {
        var entries = new[] {
            ...
            Entry(IccOutputIntentId, "color", "ICC destination output intents", PdfRenderSupportLevel.Supported, "Catalog destination profiles with bounded RGB matrix/TRC or RGB/CMYK B2A0/B2A1/B2A2 mBA output transforms are soft-proofed through the shared managed color engine for content without explicit PDF transparency composition."),
            Entry(UnsupportedIccOutputIntentId, "color", "Unsupported ICC destination output intent", PdfRenderSupportLevel.Simplified, "The catalog output intent is retained for inspection, but its destination profile is malformed or outside the managed output-transform contract, so page colors are rendered without output-profile soft proofing."),
            Entry(OutputIntentTransparencyId, "color", "Output intent with explicit PDF transparency", PdfRenderSupportLevel.Simplified, "The page uses blend modes, opacity, soft masks, transparency groups, or image masks. Colors remain in their authored space because output-profile conversion before transparency composition would be inaccurate."),
            ...
            Entry("render.colorspace.alternate", "color", "Separation, DeviceN, and NChannel alternate color spaces", PdfRenderSupportLevel.Supported, "Bounded exponential, linear and cubic sampled, stitching, and Type 4 calculator tint transforms are projected through their declared alternate color space."),
            Entry(ColorSpaceId, "color", "Unsupported or malformed content-paint color space", PdfRenderSupportLevel.Unsupported, "The selected content-paint color space cannot be projected within the managed renderer's bounded color contract."),
            ...
            Entry(RenderingIntentId, "operator", "ri and image rendering intents", PdfRenderSupportLevel.Supported, "Perceptual, relative colorimetric, saturation, and absolute colorimetric intents select authored ICC transforms and propagate through graphics state, nested forms, text, color-managed DCT/JPEG and other normalized images, indexed palettes, output-intent soft proofing, shadings, tiling patterns, and soft-mask groups."),
            ...
            Entry("render.resource.font-type3", "resource", "Type 3 vector, image, pattern, and isolated-group glyph programs", PdfRenderSupportLevel.Supported, "Bounded Type 3 vector, normalized raster-image, colored pattern, standard blend/soft-mask, and isolated non-knockout transparency-group programs with explicit DeviceRGB blending color spaces are projected with their font matrix, text transform, clipping, paint type, resources, and page paint order."),
            Entry(CffFontSubstitutionId, "resource", "Embedded CFF, Type1C, and CIDFontType0C font programs", PdfRenderSupportLevel.Simplified, "Text is decoded and positioned, but the embedded CFF outlines are not executed and glyphs are rendered through a mapped system font."),
            ...
            Entry(Type3FontSubstitutionId, "resource", "Malformed or unsupported Type 3 font glyph programs", PdfRenderSupportLevel.Simplified, "Text is decoded and positioned through a mapped system font when the Type 3 font dictionary or requested glyph program cannot be projected safely."),
            ...
            Entry("render.resource.shading-pattern", "resource", "Axial and radial shading patterns", PdfRenderSupportLevel.Simplified, "Supported axial and radial shadings, including function arrays and bounded sampled or stitching functions, retain authored knots and discontinuities through a finite shared Drawing gradient projection."),
            Entry(UnsupportedShadingId, "resource", "Unsupported axial or radial shading", PdfRenderSupportLevel.Unsupported, "The invoked shading is malformed or uses a color or function contract outside the bounded managed renderer."),
            Entry(TilingPatternId, "resource", "Tiling patterns", PdfRenderSupportLevel.Supported, "Colored and bounded uncolored tiling patterns are projected for fill, stroke, and text painting through shared Drawing vector patterns."),
