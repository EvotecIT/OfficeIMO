...
internal static partial class PdfWriter {
    ...
    // Fills a rectangle whose selected corners are rounded to <paramref name="radius"/> points. Used for
    // the outer box of a rounded table row or a rounded panel background.
    ...
    // Strokes a rectangle whose selected corners are rounded. Replaces the square DrawRowRect perimeter
    // for a rounded table row / panel border.
    ...
    // Pushes a graphics state and intersects the clip with a corner-rounded rectangle, so any drawing
    // that follows (a cell fill) is trimmed to the rounded box. Balance with <see cref="EndRoundedClip"/>.
    ...
    // Emits a rectangle path (m/l/c operators, not painted) with only the requested corners rounded to
    // <paramref name="cornerRadius"/>; the remaining corners stay sharp. Wound the same direction as
    // <see cref="AppendRoundedPath"/> and reduces to it when all four corners are requested.
    private static void AppendCornerRoundedPath(ContentStreamBuilder content, double x, double y, double w, double h, double cornerRadius, bool tl, bool tr, bool br, bool bl) {
        ...
        if (br) content.CubicTo(x2 - r + c, y, x2, y + r - c, x2, y + r);
        content.LineTo(x2, tr ? y2 - r : y2);
        if (tr) content.CubicTo(x2, y2 - r + c, x2 - r + c, y2, x2 - r, y2);
        content.LineTo(tl ? x + r : x, y2);
        if (tl) content.CubicTo(x + r - c, y2, x, y2 - r + c, x, y2 - r);
        content.LineTo(x, bl ? y + r : y);
        if (bl) content.CubicTo(x, y + r - c, x + r - c, y, x + r, y);
    }
    ...
    // Draws a cell's per-side borders against a rounded outer box (e.g. a left accent stripe or a top
    // accent bar). Each present side is drawn as a rounded-rectangle STROKE along the box's own rounded
    // path, at the side's width, clipped to that side plus its two corners (down to the corner tangents).
    // This gives a constant-thickness border that shares the box's exact rounded corners and meets the
    // neighbouring (thin) border cleanly at the tangents - the CSS border-radius behaviour - instead of a
    // straight stroke or inset band whose corner diverges from the box and lets the neighbour show through.
    private static void DrawRoundedCellBorderStrokes(StringBuilder sb, PdfCellBorder border, double x, double y, double w, double h, double radius, double outerBorderWidth, bool tl, bool tr, bool br, bool bl, bool artifact = false) {
        // Only explicit per-side overrides are drawn as accents. The uniform PdfCellBorder colour (grey by
        // default, with all sides enabled) is deliberately skipped here: on a rounded box it just
        // duplicates the table's own perimeter border, and drawing it would lay a second stroke over the
        // accent at the shared corner. A cell border that is ONLY a uniform box (no per-side override) is
        // drawn as one rounded rectangle instead.
        PdfCellBorderSide? top = border.Top ? border.TopBorderSnapshot : null;
        PdfCellBorderSide? right = border.Right ? border.RightBorderSnapshot : null;
        PdfCellBorderSide? bottom = border.Bottom ? border.BottomBorderSnapshot : null;
        PdfCellBorderSide? left = border.Left ? border.LeftBorderSnapshot : null;
        bool anySide = IsRenderableCellBorderSide(top) || IsRenderableCellBorderSide(right) ||
                       IsRenderableCellBorderSide(bottom) || IsRenderableCellBorderSide(left);
        if (anySide) {
            if (IsRenderableCellBorderSide(top)) DrawRoundedSideStroke(sb, top!.Color!.Value, top.Width, outerBorderWidth, RoundedRectSide.Top, x, y, w, h, radius, tl, tr, br, bl, artifact);
            if (IsRenderableCellBorderSide(right)) DrawRoundedSideStroke(sb, right!.Color!.Value, right.Width, outerBorderWidth, RoundedRectSide.Right, x, y, w, h, radius, tl, tr, br, bl, artifact);
            if (IsRenderableCellBorderSide(bottom)) DrawRoundedSideStroke(sb, bottom!.Color!.Value, bottom.Width, outerBorderWidth, RoundedRectSide.Bottom, x, y, w, h, radius, tl, tr, br, bl, artifact);
            if (IsRenderableCellBorderSide(left)) DrawRoundedSideStroke(sb, left!.Color!.Value, left.Width, outerBorderWidth, RoundedRectSide.Left, x, y, w, h, radius, tl, tr, br, bl, artifact);
        } else if (border.Color.HasValue && border.Width > 0 && border.Top && border.Right && border.Bottom && border.Left) {
            DrawRoundedRowRect(sb, border.Color.Value, border.Width, x, y, w, h, radius, tl, tr, br, bl, artifact);
        }
    }

    // Draws one side of a rounded box's accent border. The accent follows the box's rounded path at
    // constant thickness <paramref name="width"/> and is clipped to that side plus its two corners (down
    // to the tangents), so it shares the box's exact rounded corners and meets the neighbouring border at
    // the tangents. The stroked path is inset by half the width it exceeds the surrounding
    // <paramref name="outerBorderWidth"/>, so the accent's OUTER edge lands exactly on that thin border's
    // outer edge: it covers the thin border on this side with no overhang past the corner and no sliver
    // of it left showing. Pass outerBorderWidth == width (or 0) for a standalone border that should sit
    // centred on the path.
    private static void DrawRoundedSideStroke(StringBuilder sb, PdfColor color, double width, double outerBorderWidth, RoundedRectSide side, double x, double y, double w, double h, double radius, bool tl, bool tr, bool br, bool bl, bool artifact = false) {
        ...
        if (r <= 0) {
            // No rounding: fall back to a straight stroke along the edge.
            double ex0 = x + w, ey0 = y + h;
            switch (side) {
                case RoundedRectSide.Top: DrawStyledHLine(sb, color, width, OfficeIMO.Drawing.OfficeStrokeDashStyle.Solid, x, ex0, ey0, artifact); break;
                case RoundedRectSide.Bottom: DrawStyledHLine(sb, color, width, OfficeIMO.Drawing.OfficeStrokeDashStyle.Solid, x, ex0, y, artifact); break;
                case RoundedRectSide.Left: DrawStyledVLine(sb, color, width, OfficeIMO.Drawing.OfficeStrokeDashStyle.Solid, x, ey0, y, artifact); break;
                case RoundedRectSide.Right: DrawStyledVLine(sb, color, width, OfficeIMO.Drawing.OfficeStrokeDashStyle.Solid, ex0, ey0, y, artifact); break;
            }
            return;
        }

        // Inset the stroked path so the accent's outer edge covers the surrounding border's outer edge
        // (path + outerBorderWidth/2) plus a hair, so no anti-aliased sliver of that border shows at the
        // corner. This leaves the accent's outer edge only ~half the thin border past the box silhouette.
        double inset = Math.Max(0D, width / 2D - outerBorderWidth);
        inset = Math.Min(inset, r); // keep the inset box valid
        double px = x + inset, py = y + inset, pw = w - inset * 2D, ph = h - inset * 2D, pr = r - inset;

        double x2 = x + w;
        double y2 = y + h;
        double cx, cy, cw, ch;
        switch (side) {
            case RoundedRectSide.Top: cx = x - width; cy = y2 - r; cw = w + width * 2D; ch = r + width; break;
            case RoundedRectSide.Bottom: cx = x - width; cy = y - width; cw = w + width * 2D; ch = r + width; break;
            case RoundedRectSide.Left: cx = x - width; cy = y - width; cw = r + width; ch = h + width * 2D; break;
            default: cx = x2 - r; cy = y - width; cw = r + width; ch = h + width * 2D; break; // Right
        }

        AppendArtifactBegin(sb, artifact);
        BeginRoundedClip(sb, cx, cy, cw, ch, 0D, false, false, false, false); // plain rectangular clip window
        DrawRoundedRowRect(sb, color, width, px, py, pw, ph, pr, tl, tr, br, bl);
        EndRoundedClip(sb);
        AppendArtifactEnd(sb, artifact);
    }
