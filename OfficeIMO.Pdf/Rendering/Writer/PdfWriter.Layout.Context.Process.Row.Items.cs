        ...
        for (int i = 0; i < ncols; i++) {
            ...
            foreach (var cb in rb.Columns[i].Blocks) {
                if (cb is HeadingBlock hb2) {
                    ...
                    var wrap = WrapRichRunsCore(rpb2.Runs, textFrame.Width, size, ChooseNormal(currentOpts.DefaultFont), leading, textFrame.FirstLineWidth, GetParagraphTabStopWidth(paragraphStyle), currentOpts, paragraphStyle?.TabStops.ToArray());
