    ...
    public partial class Word {
        ...
        public void Test_AddingComboBox() {
            ...
            using (WordprocessingDocument wordDoc = WordprocessingDocument.Open(filePath, false)) {
                ...
                var runText = sdtRun.SdtContentRun!.Descendants<Text>().SingleOrDefault()?.Text;
                Assert.Equal("Two", runText);

                var combo = sdtRun.SdtProperties!.Elements<SdtContentComboBox>().Single();
                Assert.Equal("Two", combo.LastValue?.Value);
            }
        ...
        [Fact]
        public void Test_ComboBoxSelectedValueUpdatesLastValue() {
            string filePath = Path.Combine(_directoryWithFiles, "DocumentWithComboBoxUpdated.docx");
            using (WordDocument document = WordDocument.Create(filePath)) {
            ...
            using (WordprocessingDocument wordDoc = WordprocessingDocument.Open(filePath, false)) {
                var sdtRun = wordDoc.MainDocumentPart!.Document.Body!.Descendants<SdtRun>().Single();
                var combo = sdtRun.SdtProperties!.Elements<SdtContentComboBox>().Single();
                Assert.Equal("Blue", combo.LastValue?.Value);

                var runText = sdtRun.SdtContentRun!.Descendants<Text>().Single().Text;
                Assert.Equal("Blue", runText);
            }
        }

        [Fact]
        public void Test_ComboBoxSelectedValueMustMatchItems() {
            string filePath = Path.Combine(_directoryWithFiles, "DocumentWithComboBoxInvalidSelection.docx");
            using (WordDocument document = WordDocument.Create(filePath)) {
                var items = new List<string> { "Yes", "No" };
                var combo = document.AddParagraph("Select:").AddComboBox(items, "Answer", "AnswerTag");

                var exception = Assert.Throws<ArgumentException>(() => combo.SelectedValue = "Maybe");
                Assert.Contains("must match one of the provided items", exception.Message);
            }
        }
    }
}
