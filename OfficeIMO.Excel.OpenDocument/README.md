...
The ODS-to-Excel route maps one numeric `cell-content()` comparison or `cell-content-is-between` / `cell-content-is-not-between` condition with literal numeric bounds on a cell style to an Excel cell-value rule when the applied common table-cell style has an explicit background fill. Range bounds must be in ascending order. It covers up to 4,096 styled cells per source style on each sheet when basic styles are included. The report marks the rule as approximate because Excel receives a differential fill rather than the full ODF applied style. Multiple maps on one style, formula bounds, other conditions, maps without a direct fill, and rules above that cell limit remain unsupported. Excel-to-ODS conditional formatting is still reported as unsupported; neither direction silently treats those rules as basic cell styles.
