...
The Excel-to-ODS route maps up to 16 numeric cell-value rules with direct-RGB solid differential fills and exactly representable plain decimal bounds on one shared contiguous range per sheet. It preserves priority and first-match behavior when every rule except the last uses stop-if-true, and applies the maps to at most 4,096 cells that have no basic cell style. The report marks these mappings as approximate because Excel differential formatting and ODF applied styles have different evaluation models. Styled target cells, overlapping merges, theme or indexed fills, exponent or higher-precision bounds, mixed differential styles, multiple ranges, other conditions, and rules beyond the limits remain explicit conversion loss; unsupported chains are not partially applied.
