...
The route covers up to 4,096 styled cells per source style on each sheet when basic styles are included. The report marks mapped rules as approximate because other ODF applied-style properties, font-family fallback lists, relative font sizes, and evaluation details are not transferred. A style with more than 16 maps or any unsupported map remains wholly unsupported. Formula bounds, other conditions, maps without a supported direct style property, and rules above the cell limit remain unsupported. Excel-to-ODS conditional formatting is still reported as unsupported; neither direction silently treats those rules as basic cell styles.
