...
internal sealed class EpubMemorySampler : IDisposable {
    ...
    internal EpubMemorySampler(Process process) {
        _process = process;
        ...
        _thread = new Thread(SampleUntilStopped) {
            IsBackground = true,
            Name = "OfficeIMO.Epub memory sampler"
        };
        _thread.Start();
    }

    internal EpubMemoryPeak Stop() {
        if (Interlocked.Exchange(ref _stopped, 1) == 0) {
            _stop.Set();
            _thread.Join();
            RecordCurrentMemory();
        }
        return new EpubMemoryPeak(
