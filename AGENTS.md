...
- Follow the [Microsoft Project milestones](Docs/ROADMAP.md#microsoft-project-document-library) as the single product scope. Start with P00; P01 may overlap its bounded feasibility experiments as specified. Use the default milestone order thereafter unless a concrete dependency or user direction justifies a recorded change.
- At each implementation task start, identify the active milestone, remaining acceptance criteria, existing evidence, and the next bounded deliverable. Inspect current source and prior task history before repeating discovery. Keep one milestone in active focus apart from the documented P00/P01 overlap or necessary shared-owner fixes.
- Preserve the adopted baseline: one typed model, thin fluent wrappers, no new external runtime dependencies, explicit scheduling, separate native-generation codecs, and operation-level preservation/conversion reports. Exact method names remain proposals until P01 settles executable API examples; do not propagate a later breaking redesign without identifying its effects.
