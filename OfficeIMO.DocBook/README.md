...
`ToOfficeDocumentModel()` and `FromOfficeDocumentModel()` use `OfficeDocumentModel.Structure` and deterministic loss diagnostics. `OfficeIMO.Reader.DocBook` adds dedicated `.dbk` and `.docbook` ingestion without taking over generic `.xml` routing.
