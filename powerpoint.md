...
Last updated: 2025-11-26. Goal: make OfficeIMO.PowerPoint generate PPTX files that are structurally identical to the PowerPoint-generated templates in `Assets/PowerPointTemplates` (blank, title, tables+charts) and ship a no-repair experience across net472/net8/net9.
...
- Chart packaging in templates lives under `ppt/charts` with paired style/color parts and Excel embeddings named `Microsoft_Excel_Worksheet*.xlsx`; our `AddChart` builds a minimal chart and relies on `RebaseChartParts`, with hard-coded part names that may not scale past two charts.
...
- [ ] Unblock tests on all TFMs: guard the Examples namespace import for net472 and ensure TemplateParity tests run where Examples are available.
- [ ] Baseline diff harness: generate decks with the current API and diff against `PowerPointBlank.pptx`, `PowerPointWithTitle.pptx`, and `PowerPointWithTablesAndCharts.pptx` (manifest + targeted XML nodes) to turn template output into verifiable acceptance data.
...
- [ ] Chart packaging parity: base chart part creation on the template structure (chartSpace, axes, externalData, style/color parts, embeddings, media) and make `RebaseChartParts` robust for N charts with deterministic naming aligned to `ppt/charts/chart{n}.xml` and `ppt/embeddings/Microsoft_Excel_Worksheet{n}.xlsx`.
