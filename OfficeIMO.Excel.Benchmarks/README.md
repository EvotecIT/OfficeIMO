...
- real-world workbook features such as freeze panes, AutoFilter, conditional formatting, charts, and pivot tables
...
Real-world feature tuning can run the combined report and the individual feature lanes together. The all-in-one scenario includes a table, AutoFit, freeze panes, AutoFilter, conditional formatting, data validation, a pivot table, and a chart. The core report scenario keeps to the cross-library report features that ClosedXML can also create. The specialized lanes isolate the expensive pieces:

```powershell
dotnet run -c Release --framework net8.0 --project .\OfficeIMO.Excel.Benchmarks\OfficeIMO.Excel.Benchmarks.csproj -- compare --rows 2500 --warmup 1 --iterations 3 --scenario realworld-report-all-in-one --scenario realworld-report-core --scenario realworld-freeze-panes --scenario realworld-autofilter --scenario realworld-conditional-formatting --scenario realworld-data-validation --scenario realworld-charts --scenario realworld-pivot-table
```
