    ...
    public async Task RecoveryConfirmationAndResultStayVisibleAtSupportedSizes(int width, int height, bool dark) {
        ...
        await app.Dispatch(async () => {
            ...
            try {
                ...
                var clearHistory = view.GetVisualDescendants().OfType<Button>().Single(button => ReferenceEquals(button.Command, settings.ClearHistoryCommand));
                clearHistory.BringIntoView();
                Layout(window, width, height);
                VisibleBounds(clearHistory, view);
                Capture(window, width, dark, "history-confirm");
                settings.ClearHistoryCommand.Execute(null);
                Assert.False(settings.ConfirmHistoryClear);
                ...
                Layout(window, width, height);
                var historyStatus = view.GetVisualDescendants().OfType<TextBlock>().Single(text => text.Text == settings.HistoryStatus);
                historyStatus.BringIntoView();
                Layout(window, width, height);
                VisibleBounds(historyStatus, view);
                Capture(window, width, dark, "history-cleared");
