...
public sealed partial class BibliographyDocument {
    ...
    internal BibliographyDocument(BibliographyFormat format, IList<BibliographyItem> items, IList<BibliographyNativeEntry> nativeEntries, string originalText, byte[]? originalBytes, IReadOnlyList<BibliographyDiagnostic> diagnostics) {
        ValidateFormat(format);
        SourceFormat = format;
    ...
    public static BibliographyReadResult Parse(string source, BibliographyReadOptions? options = null, CancellationToken cancellationToken = default) {
        ...
        return Parse(source, BibliographyFormatDetector.Detect(source), options, cancellationToken);
    }
