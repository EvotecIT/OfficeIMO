    ...
    public partial class Excel
    {
        ...
        public void Load_FromMemoryStream_WithSaveOnDispose_PersistsChanges()
        {
            ...
            try
            {
                ...
                using (var document = ExcelDocument.Load(memory, new OfficeIMO.Excel.ExcelLoadOptions { PersistenceMode = OfficeIMO.DocumentPersistenceMode.SaveOnDispose }))
                {
        ...
        public async Task LoadAsync_FromMemoryStream_WithSaveOnDispose_PersistsChanges()
        {
            ...
            try
            {
                ...
                await using (var document = await ExcelDocument.LoadAsync(memory, new OfficeIMO.Excel.ExcelLoadOptions { PersistenceMode = OfficeIMO.DocumentPersistenceMode.SaveOnDispose }))
                {
                    var sheet = document.Sheets[0];
        ...
        public void Load_FromReadOnlyStream_WithSaveOnDispose_Throws()
        {
            ...
            try
            {
                ...
                var ex = Assert.Throws<ArgumentException>(() => ExcelDocument.Load(readOnlyStream, new OfficeIMO.Excel.ExcelLoadOptions { PersistenceMode = OfficeIMO.DocumentPersistenceMode.SaveOnDispose }));
        ...
        public async Task LoadAsync_FromReadOnlyStream_WithSaveOnDispose_Throws()
        {
            ...
            try
            {
                ...
                using var readOnlyStream = new MemoryStream(File.ReadAllBytes(filePath), writable: false);
                var ex = await Assert.ThrowsAsync<ArgumentException>(() => ExcelDocument.LoadAsync(readOnlyStream, new OfficeIMO.Excel.ExcelLoadOptions { PersistenceMode = OfficeIMO.DocumentPersistenceMode.SaveOnDispose }));
                Assert.Equal("stream", ex.ParamName);
            }
            finally
            {
