...
namespace OfficeIMO.Tests {
    public partial class Excel {
        private static partial class LegacyXlsTestWorkbookBuilder {
            ...
            internal static byte[] CreateUnreadDefinedNameWorkbookStream(bool shortRecord) {
                using var stream = new MemoryStream();
                WriteRecord(stream, 0x0809, new byte[] { 0x00, 0x06, 0x05, 0x00, 0xdb, 0x0b, 0xcc, 0x07 });
                long boundSheetPosition = stream.Position;
                WriteRecord(stream, 0x0085, BuildBoundSheetPayload(0, "Names"));
                WriteRecord(stream, 0x0018, shortRecord
                    ? new byte[4]
                    : BuildDefinedNamePayload("UnrepresentedName", new byte[] { 0xff }, 0, hidden: false, builtIn: false));
                WriteRecord(stream, 0x000a, Array.Empty<byte>());
            ...
            internal static byte[] CreatePhase4AutoFilterAndCriteriaWorkbookStream() {
                using var stream = new MemoryStream();
                WriteRecord(stream, 0x0809, new byte[] { 0x00, 0x06, 0x05, 0x00, 0xdb, 0x0b, 0xcc, 0x07 });
                long boundSheetPosition = stream.Position;
                WriteRecord(stream, 0x0085, BuildBoundSheetPayload(0, "Between"));
                WriteRecord(stream, 0x0017, BuildExternSheetPayload((0, 0, 0)));
                ...
                int sheetOffset = checked((int)stream.Position);
                WriteRecord(stream, 0x0809, new byte[] { 0x00, 0x06, 0x10, 0x00, 0xdb, 0x0b, 0xcc, 0x07 });
                WriteRecord(stream, 0x0204, BuildLabelPayload(0, 0, "Amount"));
                WriteRecord(stream, 0x027e, BuildRkPayload(1, 0, 0, EncodeRkInteger(5)));
                WriteRecord(stream, 0x027e, BuildRkPayload(2, 0, 0, EncodeRkInteger(10)));
                WriteRecord(stream, 0x027e, BuildRkPayload(3, 0, 0, EncodeRkInteger(15)));
                WriteRecord(stream, 0x027e, BuildRkPayload(4, 0, 0, EncodeRkInteger(25)));
                WriteRecord(stream, 0x009d, BuildAutoFilterInfoPayload(1));
                WriteRecord(stream, 0x009b, Array.Empty<byte>());
                WriteRecord(stream, 0x009e, BuildAutoFilterNumberBetweenPayload(0, 10d, 20d));
                WriteRecord(stream, 0x000a, Array.Empty<byte>());
            ...
            internal static byte[] CreatePhase5ChartSheetSubstreamWorkbookStream() {
                ...
                WriteRecord(stream, 0x1033, Array.Empty<byte>());
                WriteRecord(stream, 0x0850, Array.Empty<byte>());
                ...
                WriteRecord(stream, 0x0852, Array.Empty<byte>());
                ...
                WriteRecord(stream, 0x100b, BuildUInt16Payload(25));
            ...
            internal static byte[] CreatePhase5DdeOleExternalNameFlagsWorkbookStream() {
                using var stream = new MemoryStream();
                WriteRecord(stream, 0x0809, new byte[] { 0x00, 0x06, 0x05, 0x00, 0xdb, 0x0b, 0xcc, 0x07 });
                long dataBoundSheetPosition = stream.Position;
                WriteRecord(stream, 0x0085, BuildBoundSheetPayload(0, "Links"));
                WriteRecord(stream, 0x01ae, BuildSupBookExternalWorkbookPayload("Excel|Topic"));
                WriteRecord(stream, 0x0023, BuildExternalNamePayload(
                    "DdePicture",
                    wantsAdvise: true,
                    wantsPicture: true,
                    cachedClipboardFormat: 9,
                    icon: true));
                WriteRecord(stream, 0x0023, BuildExternalNamePayload("OleNoOper", ole: true));
                WriteRecord(stream, 0x0023, BuildExternalNamePayload("OleData", oleLink: true));
                WriteRecord(stream, 0x000a, Array.Empty<byte>());
            ...
            internal static byte[] CreatePhase5PivotTableMetadataWorkbookStream() {
                ...
                WriteRecord(stream, 0x00b5, BuildSxliDataPayload());
                WriteRecord(stream, 0x00b5, BuildSxliGrandTotalPayload());
            ...
            internal static byte[] CreatePhase5ConditionalFormattingExtensionWithDxfWorkbookStream() {
                using var stream = new MemoryStream();
                WriteRecord(stream, 0x0809, new byte[] { 0x00, 0x06, 0x05, 0x00, 0xdb, 0x0b, 0xcc, 0x07 });
                long formattingBoundSheetPosition = stream.Position;
                ...
                WriteRecord(stream, 0x01b0, BuildConditionalFormattingRangePayload(0, 0, 2, 0, 1, headerId: 2));
                WriteRecord(stream, 0x01b1, BuildCellIsGreaterThanConditionalFormattingRulePayload(10));
                WriteRecord(stream, 0x087b, BuildConditionalFormattingExtensionPayload(headerId: 1, priority: 1, stopIfTrue: true, hasDxf: true));
                WriteRecord(stream, 0x000a, Array.Empty<byte>());
            ...
            internal static byte[] CreatePhase5ConditionalFormattingExtensionWithFontDxfWorkbookStream() {
                using var stream = new MemoryStream();
                WriteRecord(stream, 0x0809, new byte[] { 0x00, 0x06, 0x05, 0x00, 0xdb, 0x0b, 0xcc, 0x07 });
                long formattingBoundSheetPosition = stream.Position;
                ...
                WriteRecord(stream, 0x01b0, BuildConditionalFormattingRangePayload(0, 0, 2, 0, 1, headerId: 2));
                WriteRecord(stream, 0x01b1, BuildCellIsGreaterThanConditionalFormattingRulePayload(10));
                WriteRecord(stream, 0x087b, BuildConditionalFormattingExtensionPayload(headerId: 1, priority: 2, stopIfTrue: false, hasDxf: true));
                WriteRecord(stream, 0x000a, Array.Empty<byte>());
            ...
            internal static byte[] CreatePhase5ConditionalFormattingExtensionWithNumberFormatDxfWorkbookStream() {
                using var stream = new MemoryStream();
                WriteRecord(stream, 0x0809, new byte[] { 0x00, 0x06, 0x05, 0x00, 0xdb, 0x0b, 0xcc, 0x07 });
                long formattingBoundSheetPosition = stream.Position;
                ...
                WriteRecord(stream, 0x01b0, BuildConditionalFormattingRangePayload(0, 0, 2, 0, 1, headerId: 2));
                WriteRecord(stream, 0x01b1, BuildCellIsGreaterThanConditionalFormattingRulePayload(10));
                WriteRecord(stream, 0x087b, BuildConditionalFormattingExtensionPayload(headerId: 1, priority: 4, stopIfTrue: false, hasDxf: true));
                WriteRecord(stream, 0x000a, Array.Empty<byte>());
            ...
            private static byte[] BuildDifferentialFormatNumberFormatPayload() {
                ...
                WriteUInt16(stream, 0);
                WriteUInt16(stream, 1);
                WriteUInt16(stream, 0x0029);
                WriteUInt16(stream, 6);
                WriteUInt16(stream, 164);
                return stream.ToArray();
            }
            ...
            private static byte[] BuildConditionalFormattingExtensionPayload(ushort headerId, ushort priority, bool stopIfTrue, bool hasDxf = false) {
                using var stream = new MemoryStream();
            ...
            private const byte LegacyAutoFilterComparisonLessThanOrEqual = 0x03;
            ...
            private static byte[] BuildChartFontBasisPayload(ushort widthTwipsBasis, ushort heightTwipsBasis, ushort fontHeightTwips, ushort scaleBasis, ushort fontIndex) {
                using var stream = new MemoryStream();
                WriteUInt16(stream, widthTwipsBasis);
                WriteUInt16(stream, heightTwipsBasis);
            ...
            private static byte[] BuildChartPositionPayload() {
                using var stream = new MemoryStream();
                WriteUInt16(stream, 0x0005);
                WriteUInt16(stream, 0x0001);
                WriteUInt16(stream, 15);
                WriteUInt16(stream, 0);
                WriteUInt16(stream, 25);
                WriteUInt16(stream, 0);
            ...
            private static byte[] BuildChartPlotGrowthPayload() {
                ...
                WriteFixedPoint(stream, 2, 0x8000);
                return stream.ToArray();
            }
            ...
            private static byte[] BuildSxIvdPayload() {
                using var stream = new MemoryStream();
                WriteUInt16(stream, 0);
                WriteUInt16(stream, 2);
                return stream.ToArray();
            ...
            private static byte[] BuildSxliDataPayload() {
                using var stream = new MemoryStream();
                WriteUInt16(stream, 0);
                WriteUInt16(stream, 0);
                WriteUInt16(stream, 2);
            ...
            private static byte[] BuildSxliGrandTotalPayload() {
                using var stream = new MemoryStream();
                WriteUInt16(stream, 0);
                WriteUInt16(stream, 0x000d);
                WriteUInt16(stream, 1);
            ...
            private static byte[] BuildSxAddlEndPayload() {
                using var stream = new MemoryStream();
                WriteUInt16(stream, 0x0864);
                WriteUInt16(stream, 0);
                stream.WriteByte(0x03);
