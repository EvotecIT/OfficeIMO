...
- **Incident sequence:** first-party sequence-stencil actor/system/service participants, activation bars, combined fragment frame, collision-aware note placement, self-message placement, notes API coverage, error path, and remediation path; still needs richer guarded/partitioned fragment layouts.
...
2. Keep replacing anonymous basic geometry with stencil-backed premium symbols where the domain supports it; all eight current premium baselines now carry first-party stencil provenance for their domain shapes, with sequence activations, combined fragments, and collision-aware notes now promoted while richer guarded/partitioned fragments still need stronger layout.
