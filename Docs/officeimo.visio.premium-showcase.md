...
1. Extend obstacle-aware routing from the current zone/container/group/crossing-aware options into deeper whole-page route minimization; connector labels now avoid unrelated connector paths and normal sequence message labels have lifeline-aware and fragment-label-aware placement, so the next label work is denser graph, whole-page, and deeper sequence edge-case cleanup.
