...
| Word ↔ ODT, Excel ↔ ODS, PowerPoint ↔ ODP | Exact native mappings where available, with explicit approximation or omission diagnostics for target gaps; PowerPoint fields retain displayed text and representable run formatting in ODP while dynamic update semantics are flattened |
...
| PowerPoint ↔ Google Slides | Native per-run family, size, color, bold, italic, single underline/strike, small caps, superscript, and subscript in text boxes and table cells; all-caps is materialized with run-language casing, richer DrawingML variants use their closest supported appearance, and checkpoints fingerprint Google-projected text, run boundaries, and synchronized styles |
