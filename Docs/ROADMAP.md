...
- [ ] Deepen HTML-to-DOCX, HTML-to-XLSX, HTML-to-PPTX, and HTML-to-RTF beyond the established semantic mappings for tables, images, links, lists, and breaks. Add destination-native handling for the bounded positioned or floating regions, advanced backgrounds and effects, and print or slide layout constructs that have an editable representation, with stable per-construct simplify-or-omit diagnostics for the rest.
- [ ] Deepen the named Word, Excel, PowerPoint, and RTF HTML review profiles for floating objects, tracked-change and field policy, charts, pivots, masters, notes, SmartArt fallbacks, media poster frames, and advanced effects. Keep semantic, editable round-trip, positioned-review, and print-review contracts distinct from a full Office renderer.
