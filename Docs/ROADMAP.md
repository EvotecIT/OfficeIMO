...
- [ ] Complete managed rendering for remaining difficult Type 3 glyph programs that depend on unsupported transparency effects, remaining ICC and content-paint color spaces, advanced or recursively nested pattern resources, transparency groups, masks, and optional-content interactions. Require producer fixtures, per-page diagnostics, and raster plus semantic proof before removing a limitation.
