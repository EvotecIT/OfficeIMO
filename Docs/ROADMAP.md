...
- [ ] Add codec-level `Stream` and `IBufferWriter<byte>` encoding so callers can avoid a second materialized output array, then add bounded stream and span decode entry points where practical. Profile remaining whole-image copies and pool scratch buffers only where ownership remains safe; consider SIMD or parallel row work only after measurement identifies a material hotspot.
