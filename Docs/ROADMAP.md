...
- [ ] Reduce the LaTeX lossless model's retained graph beyond the current validated baseline of about 57 MiB for a 701 KiB, 256,036-token source. Preserve exact token text, source positions, syntax parent navigation, semantic editing, and byte-identical writing; add Linux/macOS evidence and a competitor only when it can build the same public model.
