...
- [ ] Complete managed rendering for remaining difficult Type 3 glyph programs that depend on implicit, non-isolated, knockout, or non-RGB transparency groups, recursively nested masks, remaining ICC and content-paint color spaces, advanced or recursively nested pattern resources, and advanced optional-content usage-application or intent interactions. Require producer fixtures, per-page diagnostics, and raster plus semantic proof before removing a limitation.
