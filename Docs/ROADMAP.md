...
- [ ] Complete managed rendering for difficult Type 3 glyph programs, ICC mBA output transforms and higher-channel/intent variants beyond the supported matrix/TRC and bounded A2B0 LUT8/LUT16/mAB paths, cubic sampled and general calculator color functions beyond the bounded linear Type 0/Type 3 paths, higher-dimensional alternate-color transforms, advanced pattern resources, transparency groups, masks, and optional-content interactions. Require producer fixtures, per-page diagnostics, and raster plus semantic proof before removing a limitation.
