...
- [ ] Complete managed rendering for difficult Type 3 glyph programs, ICC mBA output transforms and higher-channel/intent variants beyond the supported matrix/TRC and bounded A2B0 LUT8/LUT16/mAB paths, higher-dimensional alternate-color transforms beyond the bounded Type 0/2/3/4 function pipeline, advanced pattern resources, transparency groups, masks, and optional-content interactions. Require producer fixtures, per-page diagnostics, and raster plus semantic proof before removing a limitation.
