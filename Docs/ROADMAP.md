...
- [ ] Deepen PDF-to-XLSX and PDF-to-ODS table recovery beyond the shared continuation, repeated-header, typed-value, and confidence evidence now consumed by XLSX. Add merged and spanning cells, richer row and column geometry, border and fill evidence, and extend the shared profiles into ODS while keeping free-form page layout and invented formulas outside the spreadsheet contract.
...
- [ ] Extend the cross-producer reverse-conversion gate beyond the current DOCX, XLSX, PPTX, and HTML digital-PDF matrix into ODT, ODS, ODP, and image outputs. Add native-editability, text, table, page-geometry, visual-distance, and deterministic-loss assertions for scanned, malformed, encrypted, and mixed-content PDFs.
