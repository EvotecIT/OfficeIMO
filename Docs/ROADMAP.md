...
- [ ] Complete managed rendering for remaining difficult Type 3 glyph programs that depend on unsupported pattern or transparency effects, remaining ICC and content-paint color spaces, advanced pattern resources, transparency groups, masks, and optional-content interactions. Require producer fixtures, per-page diagnostics, and raster plus semantic proof before removing a limitation.
