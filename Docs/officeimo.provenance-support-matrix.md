...
`OfficeIMO.Workflows` exposes `IOfficeProvenanceWorkflowRunner` and `OfficeProvenanceWorkflowCatalog` for desktop, PowerShell, command-line, and service hosts. The catalog is the canonical extension-to-structural-format-to-owner contract; its generated [provenance capability table](Compatibility/generated/provenance.md) also identifies which formats are qualified for memory-only and browser hosts. Inspect and assess are report-only operations. Removal preserves the source format, calls the package-owned adapter, stages the result beside its destination, reopens it through the same owner, compares the reopened structural evidence with the removal result, and publishes it only after that check succeeds. Batches are sequential and have an explicit item limit.

The workflow accepts only an extension registered to a named OfficeIMO format owner, and the inspected structural format must match that registration. Unknown extensions, generic ZIP packages, and renamed content are not advertised or accepted by this orchestration layer. Callers that already own an unregistered format context can use the low-level `OfficeProvenanceInspector` API directly for signature-based structural inspection. Batch removal rejects duplicate effective output paths before processing any item, including collisions caused by equal basenames in one output directory.
...
- Resource limits are configurable through `OfficeProvenanceOptions` and `OfficeProvenanceRemovalOptions`; inputs that exceed them are rejected instead of partially inspected. The expanded-data limit counts decompressed container and stream bytes plus the scanline working set of each PNG decoded to validate its provenance carriers; a PNG without carriers is not decoded. Embedded assets draw from their container's remaining budget.
