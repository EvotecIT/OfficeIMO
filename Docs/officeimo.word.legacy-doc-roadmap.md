...
- [x] Add formatting projection for run/paragraph styles only when each mapping
  ...
  - [x] Project stylesheet-level paragraph tab-stop changes, including clear
    ...
    legacy DOC fixture proof.
  - [x] Project direct PAPX outline levels from DOC `sprmPOutLvl` into normal
    OpenXML paragraph outline properties and write them back through native DOC
    save/reload.
  ...
  - [x] Merge built-in stylesheet-level DOC language identifiers from
    ...
    with synthetic legacy DOC fixture proof.
  - [x] Merge built-in stylesheet-level DOC font-table references from
    `sprmCRgFtc0` into built-in DOCX style `w:rFonts` values with synthetic
    legacy DOC fixture proof.
...
- [x] Add common table projection after paragraph/run projection is stable.
  ...
  - [x] Add native DOC nested table save/reload coverage before calling nested
...
- [x] Expand native writer slices for formatting, supported table shapes, and
  ...
  - [x] Preserve supported built-in Heading paragraph style pagination flags
    through native DOC save/reload, covering keep-lines, keep-next,
    page-break-before, and widow-control formatting.
  - [x] Preserve supported built-in Heading paragraph style layout flags through
    native DOC save/reload, covering line-number suppression, auto-hyphenation
    suppression, contextual spacing, mirror indents, bidirectional layout, and
  ...
  - [x] Ignore OpenXML proofing error boundary markers inside simple body,
    ...
    during native DOC save/reload.
  - [x] Ignore OpenXML proofing error boundary markers between supported complex
    `PAGE`/`NUMPAGES` field runs in body, table-cell, header/footer, footnote,
    and endnote stories during native DOC save/reload.
  ...
  - [x] Write custom table styles based on `TableGrid` by inheriting the
    ...
    values.
  - [x] Write custom table styles based on another custom border/shading-only
  ...
  - [x] Write supported conditional table-style run formatting by flattening
    ...
    override.
  - [x] Write supported conditional table-style paragraph formatting by
  ...
  - [x] Read and write simple zero-length, same-paragraph, and cross-paragraph
    ...
    simple header/footer paragraph bookmark ranges, and simple footnote/endnote
    paragraph bookmark ranges through native DOC `SttbfBkmk`, `Plcfbkf`, and
    `Plcfbkl` records, then use those records to round-trip authored internal
    bookmark hyperlinks.
  ...
  - [x] Add bookmark locations outside body/table-cell/header/footer/footnote/
    ...
    - [x] Write whole-table body bookmark start/end markers around native DOC
      ...
      the legacy reader.
    - [x] Write zero-length body bookmark markers at the start edge of native
      DOC table blocks, then reload them as table-sibling bookmark markers
      through the legacy reader.
    ...
    - [x] Write conditional table-style run formatting when the style uses the
      Word-native `w:rPr`/`RunPropertiesBaseStyle` shape, then reload the first
      row formatting through the legacy reader.
    ...
    - [x] Write own and inherited conditional table-style row height, header,
      and no-split formatting through native DOC save/reload for supported
      row-scoped regions.
    - [x] Block unsupported conditional table-style row regions and row
      properties before native DOC bytes are committed, so richer row-level
