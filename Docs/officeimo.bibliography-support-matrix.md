...
| Names | Given, family, literal, suffix, dropping-particle, and non-dropping-particle components; family-only multiword names and literal names ending in commas reopen without changing shape; surrounding whitespace that a tagged or EndNote name parser would trim is diagnosed before strict output |
...
Stream decoding enforces the decoded-character limit incrementally before a full source string is materialized. EndNote line offsets are resolved by a constant-memory, cancellation-aware scan rather than a per-line index. Parsing, automatic content detection, conversion inspection, and writing observe cancellation, including EndNote post-DOM projection, per-record contributor, identifier, keyword, note, native-field, and native-entry collections, and final output encoding or preserved-byte cloning. The library does not execute BibTeX/TeX, fetch URLs, load XML external entities, invoke native citation tools, or inspect local files named by citation data.
