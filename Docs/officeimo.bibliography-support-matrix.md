...
Stream decoding enforces the decoded-character limit incrementally before a full source string is materialized. Parsing, automatic content detection, conversion inspection, and writing observe cancellation. The library does not execute BibTeX/TeX, fetch URLs, load XML external entities, invoke native citation tools, or inspect local files named by citation data.
