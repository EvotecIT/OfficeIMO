...
Latest Excel page-setup image checkpoint: page-sliced worksheet PNG/SVG export now composes rendered worksheet-page content onto a physical page canvas through shared `OfficeImageComposer` layers. The Excel adapter owns page semantics and now applies orientation, margins, and manual scale for manual-page-sliced multi-output image exports while `OfficeIMO.Drawing` owns the actual PNG/SVG page composition. Because paper-size-specific geometry is not implemented yet, the exporter emits `ExcelPageSetupPaperSizeDefaulted` when it uses the current default Letter canvas. Fit-to-width/fit-to-height remains explicitly diagnosed with `ExcelPageSetupUnsupported` until automatic fit pagination is implemented instead of being faked by silently shrinking content.
...
- Page-sliced image export now composes repeated print-title rows/columns through existing range snapshots, renders plain first/even/odd text header/footer chrome with supported page/sheet/workbook file/date/time fields in clipped and ellipsized left/center/right zones for multi-output PNG/SVG exports, and applies physical page orientation, margins, and manual scale through shared image-layer composition. It still emits source-referenced `ExcelPageSetupUnsupported` diagnostics for fit-to-width/fit-to-height pagination and `ExcelHeaderFooterUnsupported` diagnostics for richer header/footer images that are not rendered yet.
