...
- Page-sliced image export now composes repeated print-title rows/columns through existing range snapshots and renders plain odd-page text header/footer chrome for multi-output PNG/SVG exports, while still emitting source-referenced `ExcelPageSetupUnsupported` and `ExcelHeaderFooterUnsupported` diagnostics for physical page setup orientation/scaling and richer header/footer fields, formatting, images, or first/even-page variants that are not rendered yet.
