...
Latest image MIME policy checkpoint: `OfficeIMO.Drawing.OfficeImageInfo` now owns MIME-to-`OfficeImageFormat` normalization, including parameter stripping and common JPEG/SVG/metafile aliases. Excel worksheet/header-footer image insertion and Visio package asset/content-type defaults consume that shared format policy, while Excel and Visio still own OpenXML/VSDX-specific package part creation and relationship wiring.
