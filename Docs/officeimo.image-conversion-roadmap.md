...
- Page-sliced image export now composes repeated print-title rows/columns through existing range snapshots and renders plain odd-page text header/footer chrome with supported page/sheet fields for multi-output PNG/SVG exports, while still emitting source-referenced `ExcelPageSetupUnsupported` and `ExcelHeaderFooterUnsupported` diagnostics for physical page setup orientation/scaling and richer header/footer formatting, date/file/path fields, images, or first/even-page variants that are not rendered yet.
...
147. Done Excel page-sliced header/footer field slice: pass page number and page count through multi-image worksheet export, render plain header/footer text with supported `&P`, `&N`, `&A`, `&&`, `&[Page]`, `&[Pages]`, and `&[Tab]` fields in PNG/SVG page slices, and keep unsupported formatting/date/file/path/image/variant fields source-diagnosed instead of silently approximated.
