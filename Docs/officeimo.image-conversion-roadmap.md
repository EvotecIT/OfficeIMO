...
Current consolidation checkpoint: Visio SVG and PNG text rendering share `OfficeTextBlockRenderPlan` for fitted center-based text boxes, aligned placement, text background bounds, and rotated background corners, and Excel PNG/SVG plain cell text now shares the same Drawing-owned render plan for left/top cell rectangles, wrapped/stacked layout, shrink-to-fit sizing, and vertical placement. Visio still owns VSDX coordinate/style semantics and Excel still owns workbook style/rotation/diagnostic semantics, while `OfficeIMO.Drawing` owns the reusable text-block placement math both adapters need.
