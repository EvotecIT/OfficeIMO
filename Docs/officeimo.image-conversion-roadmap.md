...
- Page-sliced image export now composes repeated print-title rows/columns through existing range snapshots and renders plain first/even/odd text header/footer chrome with supported page/sheet/workbook file/date/time fields for multi-output PNG/SVG exports, while still emitting source-referenced `ExcelPageSetupUnsupported` and `ExcelHeaderFooterUnsupported` diagnostics for physical page setup orientation/scaling and richer header/footer formatting or images that are not rendered yet.
