...
| Excel to PDF | `OfficeIMO.Excel.Pdf` over `OfficeIMO.Excel` and `OfficeIMO.Pdf` | Thin adapter should reuse Excel visual/style/snapshot work and PDF/Drawing primitives rather than owning a second spreadsheet renderer. | PDF pagination/table mapping, print-area/page setup policy, warnings, PDF-specific output shape. | Reduce duplication between Excel image snapshots and Excel PDF internal rendering where the shared snapshot lowers risk. |
...
- Page-sliced image export now composes repeated print-title rows/columns through existing range snapshots and renders plain first/even/odd text header/footer chrome with supported page/sheet/workbook file/date/time fields in clipped and ellipsized left/center/right zones for multi-output PNG/SVG exports, while still emitting source-referenced `ExcelPageSetupUnsupported` and `ExcelHeaderFooterUnsupported` diagnostics for physical page setup orientation/scaling and richer header/footer formatting or images that are not rendered yet.
