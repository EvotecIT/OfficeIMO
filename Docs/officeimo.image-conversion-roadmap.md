...
Latest header/footer image checkpoint: page-sliced worksheet PNG/SVG export now renders supported `&G` header/footer PNG images through the shared Drawing composition path instead of treating every picture placeholder as unsupported page chrome. SVG output embeds Drawing-approved image MIME types through `OfficeSvgImageRenderer`, PNG output raster-composes decodable PNG header/footer images through `OfficePngReader` and `OfficeRasterCanvas`, and unsupported raster image formats now emit `ExcelHeaderFooterImageUnsupported` instead of silently disappearing. The layout remains an approximation with stable `ExcelHeaderFooterImageApproximation` diagnostics; Excel-exact header/footer image placement, scaling-with-document semantics, and non-PNG raster decoding remain premium work.
