...
- Page-sliced image export now composes repeated print-title rows/columns through existing range snapshots for multi-output PNG/SVG exports, while still emitting source-referenced `ExcelPageSetupUnsupported` and `ExcelHeaderFooterUnsupported` diagnostics for physical page setup orientation/scaling and header/footer chrome that are not rendered yet.
