...
- Page-sliced image export now composes repeated print-title rows/columns through existing range snapshots and renders plain first/even/odd text header/footer chrome with supported page/sheet/workbook file fields for multi-output PNG/SVG exports, while still emitting source-referenced `ExcelPageSetupUnsupported` and `ExcelHeaderFooterUnsupported` diagnostics for physical page setup orientation/scaling and richer header/footer formatting, date/time fields, or images that are not rendered yet.
...
147. Done Excel page-sliced header/footer field slice: pass page number and page count through multi-image worksheet export, render plain header/footer text with supported `&P`, `&N`, `&A`, `&F`, `&Z`, `&&`, `&[Page]`, `&[Pages]`, `&[Tab]`, `&[File]`, and `&[Path]` fields in PNG/SVG page slices, and keep unsupported formatting/date/time/image fields source-diagnosed instead of silently approximated.
