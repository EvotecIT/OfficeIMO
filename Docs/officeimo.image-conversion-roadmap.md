...
Latest PDF image-placement consolidation checkpoint: `OfficeIMO.Drawing.OfficeImageRenderPlan` now owns shared target-box, fit, and source-crop placement math for image rendering, with top-left and bottom-left coordinate-system entrypoints. `OfficeIMO.Pdf` consumes that plan for flow, table, header/footer, and canvas image placement instead of carrying a private crop-plus-fit algorithm in the PDF writer. PDF still owns PDF clip paths, image XObject streams, tagging, annotations, and compression, but the reusable visual placement math now lives in Drawing for Excel, Visio, PDF, and later adapters to share.
