...
- Page-sliced image export now emits source-referenced `ExcelPrintTitlesUnsupported`, `ExcelPageSetupUnsupported`, and `ExcelHeaderFooterUnsupported` diagnostics for configured print titles, physical page setup orientation/scaling, and header/footer chrome that are not rendered yet.
