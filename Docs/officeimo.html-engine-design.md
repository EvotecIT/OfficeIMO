...
The scripted-document workflow now reaches an explicitly ready document, owned interaction and independent rendered or semantic output. `WebApplicationV1` adds bounded cross-document navigation, reload and history restoration while retaining session storage, resource authority, limits and locator handles. Complete the named application's remaining lifecycle, form and isolation requirements, then add the layout/input behavior its workflows need. Retain effective HTML, CSS and JavaScript providers while establishing those contracts. Expand CSS and layout where application fixtures expose missing behavior; owned parser replacement and full static-engine qualification are separate tracks. HtmlTinkerX can reuse the engine later, but its current helper APIs, package graph and workflows do not define the runtime's boundary or acceptance criteria.
...
For runtime work, widen `WebApplicationV1` from its bounded navigation and existing application foundation to the remaining named form, lifecycle, failure and isolation contracts. Add representative application and web-platform fixtures, then implement only the layout/input behavior those workflows require. Preserve explicit bounds, provider identity and unsupported results throughout. Hostile-content execution remains unadvertised until OS isolation has its own proof.
