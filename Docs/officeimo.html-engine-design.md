...
For runtime work, complete navigation and document replacement before widening application coverage. Then freeze one named application/API profile, add representative application and web-platform fixtures, and implement only the layout/input behavior those workflows require. Preserve explicit bounds, provider identity and unsupported results throughout. Hostile-content execution remains unadvertised until OS isolation has its own proof.
