...
- Starting with NPOI 2.8.0, the README and release notes describe an additional
  binary EULA / Open Source Maintenance Fee requirement for revenue-generating
  users. That matters before adding a committed benchmark package reference.
...
Before committing a hard package reference, choose the benchmark dependency
policy:

1. Use latest NPOI 2.8.x for current comparison, accepting its binary EULA /
   maintenance-fee implications for benchmark restores.
2. Use NPOI 2.7.6 as the last pre-OSMF line for historical Apache-licensed
   comparison, clearly labeling it as not-latest.
3. Keep NPOI comparison in an opt-in benchmark adapter project so normal repo
   restore/build does not pull NPOI unless explicitly requested.

The cleanest route is option 3: add `OfficeIMO.Excel.Benchmarks.NPOI` or an
opt-in benchmark project with its own README documenting the selected NPOI
version and license boundary, then merge its JSON output into the existing
