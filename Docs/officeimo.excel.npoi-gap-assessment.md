...
- Starting with NPOI 2.8.0, the README and release notes describe an additional
  ...
  users. Benchmark-only local comparison is still a valid opt-in use for this
  repository, but the dependency should not become part of normal solution
  restore/build or any OfficeIMO runtime path.
...
`.xls` row/cell read against the same HSSF-generated bytes. Later lanes can add
DataTable/DataSet-style import/export, formula text/cached value reads, and richer
`.xls` compatibility buckets, then merge the opt-in JSON output into the existing
