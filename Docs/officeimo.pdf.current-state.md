...
| Render pages | Managed Drawing projection for supported paths, clipping, forms, images, shadings, patterns, appearances, alpha, blend modes, masks, fonts, and color spaces, including ICC A2B0 LUT8/LUT16, v4 mAB transforms bounded to 33 CLUT grid points per input dimension and 65,536 entries per embedded curve, and linear, cubic sampled, and nested stitching color functions | Type 3/CFF, ICC mBA and higher-channel/intent variants, general calculator functions, advanced patterns, and incomplete layer cases remain in per-page diagnostics |
