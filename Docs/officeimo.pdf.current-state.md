...
| Extract text and layout | `PdfDocument.Load(...).Read(...)` owns one canonical semantic result. Fast and Structured profiles share glyph, line, region, recursive XY-cut, logical projection, and table stages; Structured adds document-wide tagged-PDF, outline, repeated-edge, heading-tier, hierarchy, and continuation evidence. Built-in and custom stages share cancellation and bounded page/document work contexts. Stable JSON, Markdown, ALTO, hOCR, and PAGE XML exports project that result. Paragraph and table continuation groups recover strongly evidenced adjacent-page boundaries without rewriting page-local objects; table extraction also supplies culture-aware typed-value profiles. Caller-provided OCR can add bounded paragraphs, headings, lists, and conservative aligned-column tables with confidence, provenance, and normalized visual geometry. | Over-complex semantic input fails with `PdfReadLimitKind.UnderstandingWork`. The deterministic labelled scorecard protects CER, reading order, semantic kinds, heading levels, table adjacency, and continuation pairs, but independent-producer generalization remains open. Continuations are inferences with explicit confidence and evidence, not reconstructed source-application paragraphs or tables; OCR quality belongs to the selected provider |
