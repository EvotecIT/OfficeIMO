...
| Render PDF pages | Broad managed subset with explicit diagnostics | Static pages project to shared Drawing primitives with paths/clipping, forms, images, axial/radial shadings, supported annotation/form appearances, alpha, exact embedded TrueType outlines, and managed CMYK/Lab plus simplified calibrated-gray/RGB conversion through `OfficeIMO.Drawing`. PNG/SVG batches provide ranges, DPI/scale/background, thumbnails, cancellation, budgets, and per-page reports. A generated manifest reports every simplified/unsupported operator or resource, and optional image codecs plug into shared Drawing rasterization without becoming core dependencies. | Extend fidelity from corpus failures while keeping CFF/Type 3 gaps, resource-specific calibrated parameters, unsupported ICC spaces, tiling patterns, blend modes, ExtGState soft masks, or incomplete layer projection explicit in page reports. |
| Serialize generated PDFs | Bounded serialized-object buffering | `PdfOptions.ObjectBufferMemoryLimitBytes` bounds completed indirect-object bytes retained in memory and spills excess objects to a temporary indexed store for plain and encrypted stream saves. Spill files are removed on disposal. | The layout scene and page model are still retained, and `ToBytes()` necessarily buffers the final artifact. True page-at-a-time layout/output streaming needs a deeper writer architecture and representative memory gates. |
...
- [x] Bound completed serialized-object retention for stream saves with a
  configurable memory threshold and indexed temporary-file spillover, including
  encrypted output.
- [ ] Add true page-at-a-time layout and incremental/streaming serialization
  before describing async APIs as fully streaming. `ToBytes()` and the retained
  layout scene remain buffered operations.
