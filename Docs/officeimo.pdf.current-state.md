...
| Extract text and layout | Fast heuristic plus a pluggable understanding pipeline with stable JSON, Markdown, ALTO, hOCR, and PAGE XML. `PdfLogicalReadingOrderAnalysis` supplies shared crop-, rotation-, spanning-band-, and column-aware ordering with confidence and evidence; table extraction supplies continuation groups and culture-aware typed-value profiles. Caller-provided OCR can add bounded paragraphs, headings, lists, and conservative aligned-column tables with confidence, provenance, and normalized visual geometry. | OCR quality belongs to the selected provider; arbitrary fixed-layout PDFs are not promised to become exact editable source documents |
