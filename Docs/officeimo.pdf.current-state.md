...
| Render pages | Managed Drawing projection for supported paths, clipping, forms, images, shadings, patterns, appearances, alpha, blend modes, masks, fonts, and color spaces. Bounded Type 3 vector, raster-image, colored tiling/shading-pattern glyph programs, and outer colored, tinted, DeviceGray/DeviceRGB axial-shading, or concentric radial-shading pattern paint for vector and image-mask PaintType 2 glyph content honor font/text transforms, declared widths, clipping, resources, image masks, paint semantics, standard blend modes, supported alpha/luminosity soft masks, isolated non-knockout transparency groups with explicit DeviceRGB blending color spaces including inherited pattern paint through vector and image-mask group content, and named OCG or inline OCMD visibility inside glyph programs and nested Forms. | Shading BBoxes, nonlinear shading color spaces, nonconcentric radial families, elliptical radial strokes, implicit/non-isolated/knockout/non-RGB Type 3 transparency groups, ordinary color-image PaintType 2 glyph content, advanced or recursively nested patterns, advanced optional-content usage-application or intent behavior, CFF substitution, and ICC approximation remain in per-page diagnostics |
