...
| PDF to PowerPoint | The library default remains `VisualPages` for compatibility and explicitly reports that each slide is a non-editable page image. `CreateEditableContent()` reconstructs text blocks, detected tables, safe basic vectors, and supported source images as native objects. `CreateEditableTables()` keeps tables only, while `CreateHybrid()` retains each successfully rendered page as a visual layer and overlays editable tables at source-relative geometry. Reports surface renderer capability diagnostics and distinguish reconstructed, visual-only, simplified, and omitted content. Original charts, groups, themes, animations, notes, and authoring intent are not claimed as recoverable. |
