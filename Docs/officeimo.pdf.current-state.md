...
| Digital signatures | Partial | External-signature placeholders, digest preparation, signature application, byte-range validation, structural diagnostics, and DSS/VRI readiness markers exist. | Cryptographic CMS validation, certificate-chain policy, timestamps, OCSP/CRL evidence, trust callbacks, LTV update workflows, and proof that permitted post-signature changes remain valid. |
