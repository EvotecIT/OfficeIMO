...
| Remove, duplicate, move, reorder, and rotate pages | Ready for rewrite-safe inputs | Fluent and static APIs cover the standard page-editing operations. | Broader object-stream, tagged, layered, form-heavy, attachment-heavy, and incremental-file proof. |
...
| Digital signatures | Partial | External-signature placeholders, digest preparation, signature application, byte-range validation, structural diagnostics, and DSS/VRI readiness markers exist. The dependency-free parser exposes an optional cryptography seam, and `OfficeIMO.Pdf.Cryptography.Pkcs` can validate detached CMS/CAdES math and digests, X.509 chains, caller trust callbacks, revocation policy, and RFC 3161 timestamps on modern .NET. Reports keep structure, math, digest, trust, revocation, time, permissions, and later revisions separate. | Add signing profiles and visible appearances, deeper timestamp/revocation fixture coverage, DSS/VRI creation, append-only LTV enrichment, and proof that permitted post-signature changes remain valid. |
...
- [x] Put CMS signature verification behind a small cryptography seam so the
  ...
  or optional package owns certificate-chain, timestamp, OCSP, and CRL policy.
- [ ] Add signing profiles for approval signatures, certification signatures,
...
- [ ] Add DSS/VRI creation and append-only LTV enrichment only after
...
- [ ] Add convenience workflows for interleaving, repeating, reversing, and
...
- [ ] Deepen shared table, column, keep-with-next, widow/orphan, absolute-layout,
  drawing, and pagination behavior before adding adapter-specific layout code.
...
- [ ] Add true incremental/streaming serialization before describing async APIs
  as streaming. Writing a completed byte array to an async stream remains a
  buffered operation.
