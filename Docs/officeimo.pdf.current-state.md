...
| Read and inspect | Ready for common born-digital PDFs | Text, geometry, images, attachments, outlines, links, annotations, forms, actions, metadata, XMP, tagged content, layers, output intents, security, revisions, and signature structure are exposed. `PdfReadOptions.Limits` bounds input bytes, indirect objects, raw stream allocation, and the core object-parsing pass with typed limit exceptions; path and stream entry points reject oversized input before buffering. | Extend the same caller-controlled budgets through decoded streams, content/layout parsers, page-tree traversal, and every convenience entry point; add strict/lenient repair diagnostics. |
