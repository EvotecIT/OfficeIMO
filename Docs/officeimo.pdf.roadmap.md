...
- Create simple text fields and checkboxes. Initial generated text fields and check boxes exist through `PdfDoc.TextField(...)` and `PdfDoc.CheckBox(...)`, including flow placement, visible normal appearance streams, catalog `/AcroForm` registration, inspector/logical readback, and fill/fill-and-flatten compatibility; radio buttons, choice fields, richer appearance styling, and row/column compose placement remain roadmap work.
...
- Create PDF text fields and check boxes. Initial generated text field and check box creation exists through `PdfDoc.TextField(...)` and `PdfDoc.CheckBox(...)`.
