...
- Create simple text fields and checkboxes. Initial generated text fields exist through `PdfDoc.TextField(...)`, including flow placement, visible normal appearance streams, catalog `/AcroForm` registration, inspector/logical readback, and fill/fill-and-flatten compatibility; checkboxes, radio buttons, choice fields, richer appearance styling, and row/column compose placement remain roadmap work.
...
- Create PDF text fields. Initial generated text field creation exists through `PdfDoc.TextField(...)`.
