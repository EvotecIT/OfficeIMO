...
- Create simple text fields, checkboxes, and scalar choice fields. Initial generated text fields, check boxes, and choice fields exist through `PdfDoc.TextField(...)`, `PdfDoc.CheckBox(...)`, and `PdfDoc.ChoiceField(...)`, including flow placement, visible normal appearance streams, catalog `/AcroForm` registration, inspector/logical readback, and fill/fill-and-flatten compatibility; radio buttons, generated multi-select choice fields, richer appearance styling, and row/column compose placement remain roadmap work.
...
- Create PDF text fields, check boxes, and scalar choice fields. Initial generated text field, check box, and choice field creation exists through `PdfDoc.TextField(...)`, `PdfDoc.CheckBox(...)`, and `PdfDoc.ChoiceField(...)`.
