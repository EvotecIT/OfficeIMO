...
- Font embedding. Initial full-file TrueType embedding now exists for generated standard-font slots through `PdfOptions.EmbedStandardFont(...)` and fluent `PdfDoc.EmbedStandardFont(...)`, emitting simple `/Subtype /TrueType` fonts with `/FontDescriptor`, compressed `/FontFile2` streams that preserve original `/Length1`, `/Widths`, and `/ToUnicode` resources. `PdfOptions.CompressEmbeddedFonts` defaults to true and can be disabled for diagnostics. The next font milestones are subsetting, OpenType/CFF support, Unicode cmap/text writing beyond WinAnsi, shaping/fallback, and veraPDF-backed PDF/A fixture validation.
- XMP metadata. Generated PDFs can now opt into catalog XMP metadata through `PdfOptions.IncludeXmpMetadata`, synchronized from Info title, author, subject, keywords, and producer metadata. This is PDF/A/PDF/UA groundwork only; formal archival/accessibility profiles still require output intents, embedded-font coverage, tagging, language, structure, and external validator lanes.
- Output intents. Generated PDFs can now emit catalog `/OutputIntents` through `PdfOutputIntent`, `PdfOptions.SetOutputIntent(...)`, and fluent `PdfDoc.OutputIntent(...)`, backed by caller-supplied RGB, GRAY, or CMYK ICC profile bytes. This is PDF/A groundwork only; the next milestones are built-in approved profile handling, profile-specific ICC/output-intent validation, and veraPDF-backed fixtures.
- Document language. Generated PDFs can now emit catalog `/Lang` through `PdfOptions.Language`, `PdfOptions.SetLanguage(...)`, and fluent `PdfDoc.Language(...)`, with inspector/preflight readback. This is PDF/UA and PDF/A-a groundwork only; formal accessibility profiles still require tagged structure, role mapping, reading order, alternate text, and external validation.
- Page labels. Generated PDFs can now opt into a simple catalog `/PageLabels` number tree through `PdfOptions.IncludePageLabels`, `PdfOptions.SetPageLabels(...)`, and fluent `PdfDoc.PageLabels(...)`, using the configured visible page-number start/style and optional prefix. The generated labels are readable through `PdfInspector` and remain compatible with simple page-extraction reindexing.
- Viewer preferences. Generated PDFs can now emit simple catalog `/ViewerPreferences` dictionaries through `PdfViewerPreferencesOptions`, `PdfOptions.ViewerPreferences`, `PdfOptions.ConfigureViewerPreferences(...)`, and fluent `PdfDoc.ViewerPreferences(...)`, covering common boolean viewer-polish preferences such as document-title display, window fitting/centering, and optional toolbar/menu/window UI hiding with inspector/preflight readback.
- Embedded and associated files. Generated PDFs can now attach caller-supplied files through `PdfEmbeddedFile`, `PdfOptions.AddEmbeddedFile(...)`, and fluent `PdfDoc.AttachFile(...)`, emitting simple `/EmbeddedFile`, `/Filespec`, `/Names /EmbeddedFiles`, and catalog `/AF` entries with explicit `/AFRelationship` values. This is a reusable PDF primitive and PDF/A-3/Factur-X/ZUGFeRD groundwork only; profile-specific XML conventions, XMP identification, and veraPDF/Mustang validation remain planned before any compliance claim.
- Compliance profiles. `PdfOptions.ComplianceProfile`, `PdfOptions.RequireCompliance(...)`, and fluent `PdfDoc.Compliance(...)` now expose forward-compatible intent names for PDF/A-2b/2u/2a, PDF/A-3b/3u/3a, PDF/UA-1, Factur-X, and ZUGFeRD, but non-`None` profiles intentionally throw clear diagnostics until the required profile primitives and veraPDF/Mustang validation lanes exist. This lets wrappers design stable options now without allowing generated files to claim conformance prematurely.
...
- Sections: page size, orientation, margins, page background color, and generated page decorations. Initial options-level, document-default, page-scoped, and section-scoped flow size, portrait/landscape orientation helpers, scalar margins, reusable Word-compatible margin presets, full-page background fills, fitted JPEG/PNG page background images, gradient/opacity-aware vector page background shapes, anchored decorative bands, behind-content text plus JPEG/PNG image watermarks, and page frames exist through `PdfOptions.PageSize`, `PdfOptions.Margins`, `PdfOptions.BackgroundColor`, `PdfOptions.PageBackgroundImage`, `PdfOptions.PageBackgroundShapes`, `PdfOptions.TextWatermark`, `PdfOptions.ImageWatermark`, `PdfOptions.PageBorder`, `PdfPageBackgroundImage`, `PdfPageBackgroundShape`, `PdfTextWatermark`, `PdfImageWatermark`, `PdfPageBorder`, `PdfDoc.Size(...)`, `PdfDoc.Orientation(...)`, `PdfDoc.Portrait()`, `PdfDoc.Landscape()`, `PdfDoc.Margin(...)`, `PdfDoc.Margin(PageMargins)`, `PdfDoc.Background(...)`, `PdfDoc.BackgroundImage(...)`, `PdfDoc.BackgroundShape(...)`, `PdfDoc.BackgroundRectangle(...)`, `PdfDoc.BackgroundRoundedRectangle(...)`, `PdfDoc.BackgroundEllipse(...)`, `PdfDoc.BackgroundTopBand(...)`, `PdfDoc.BackgroundBottomBand(...)`, `PdfDoc.BackgroundLeftBand(...)`, `PdfDoc.BackgroundRightBand(...)`, `PdfDoc.Watermark(...)`, `PdfDoc.ImageWatermark(...)`, `PdfDoc.PageBorder(...)`, top-level `PdfDoc.Page(...)` / `PdfDoc.Section(...)`, `PdfDoc.Compose(...Page...)` / `Compose(...Section...)`, matching `PdfPageCompose` methods, and `PageMargins`; richer section inheritance and mid-page section breaks remain roadmap work.
...
13. Add professional report example that becomes the visual quality reference. Initial professional report baseline exists in `PdfDocVisualBaselineTests`, runnable example coverage exists in `OfficeIMO.Examples/Pdf/Pdf.ProfessionalReport.cs`, and Poppler-rendered PNG approval exists in `PdfDocRasterVisualBaselineTests` alongside a two-page line-item statement fixture, a Word-like table style gallery with compact Accent1-6 swatches, a landscape showcase dashboard, a native Word-to-first-party-PDF report fixture, a native Word daily-layout fixture, a native Word table-cell picture-control fixture, a native Excel daily-workbook fixture, compact smoke, core-layout, style-cheatsheet, styled-runs, tabs-leaders, drawing-gallery, row-columns, links-rules, lists-tables, default-styles, three-page flow-dsl, and two-page headers-footers approvals.
