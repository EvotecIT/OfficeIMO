...
`OfficeIMO.Word.Pdf` now routes through `OfficeIMO.Pdf`; the remaining roadmap work is fidelity, coverage, and visual quality rather than preserving the old external engine path.
...
- Add package guardrails that fail if `OfficeIMO.Pdf` or the native `OfficeIMO.Word.Pdf` adapter gains runtime package dependencies.
