...
- Rich text runs. Initial rich paragraph wrapping, alignment, justification, underline/strike/link rectangles, superscript/subscript baseline shifts, scaled run measurement, Word-compatible default half-inch tab stops with paragraph-style overrides, explicit paragraph tab runs with dotted leaders and right-aligned tab values, and line breaking use proportional Helvetica/Times-family standard-font glyph widths instead of average character counts.
