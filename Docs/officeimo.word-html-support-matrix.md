...
| Ruby text | Supported | `ruby` elements import as native Word ruby annotation structures and export as `ruby`/`rb`/`rt`. Multiple explicit `rb`/`rt` pairs remain distinct and ordered, and inline formatting is retained as separate base and annotation runs. Unequal or ambiguous explicit segment counts are combined only with `HtmlRubyPairingApproximation`; ruby without an annotation remains visible text. |
...
| Figures | Supported | `figure` and `figcaption` import as figure content plus Word caption paragraphs. Image figures and figures that materialize as exactly one paragraph or table retain reciprocal grouping and caption order. Content that materializes as zero or multiple Word blocks remains visible but emits `HtmlFigureStructureFlattened` instead of attaching an incorrect grouping marker. |
...
| Tables | Supported | Table/cell width, borders, background color, horizontal and table-cell vertical alignment, padding, margins, `border-collapse`, `border-spacing`, captions, and colgroup widths are handled for common document tables. Cell padding maps to native cell margins; values above 32,767 twips are bounded and diagnosed. |
...
| Runs | Supported | Emits `strong`, `em`, `u`, `s`, `del`, `ins`, `sup`, `sub`, `q`, `cite`, `dfn`, `time`, `abbr`, `mark`, `code`, font spans, run classes, colors, highlights, cached simple-field results, and run-level `lang` attributes when the run language differs from the document language. Imported `time datetime` values are preferred over parsing visible labels when available, and imported `mark` runs prefer semantic tags over generic highlight CSS. |
| Ruby text | Supported | Native Word ruby runs emit ordered `ruby`/`rb`/`rt` markup. HTML imports with explicit one-to-one segment pairs produce separate native Word ruby runs, including independently formatted base and annotation runs, so the pairs survive export and package reload. |
...
| Tables | Supported | Emits tables with spans, widths, borders, background, horizontal and vertical alignment, cell spacing as `border-spacing`, cell styles, optional `colgroup` column widths, adjacent `Caption`-style table captions as `<caption>`, leading header rows as `<thead>` / `<th scope="col">` where Word marks them as repeating header rows, and a final `<tfoot>` row when Word last-row conditional formatting is enabled. Explicit Word `nil` cell borders become CSS `hidden` and suppress a conflicting adjacent edge; `none` yields to the opposing border. |
| Figures and captions | Supported | Exports common image-plus-caption patterns and imported single-block figures, including a paragraph or table with a leading or trailing caption, as `figure` and `figcaption`. HTML import reports a multi-block figure as `HtmlFigureStructureFlattened` and omits a reciprocal grouping marker; a later Word-to-HTML export keeps those blocks readable but cannot reconstruct or re-diagnose the original figure grouping without native evidence. |
