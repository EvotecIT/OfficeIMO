...
| Other non-image aggregate resource budgets | Not supported | Non-CSS embedded media and other resource types do not share an aggregate byte-budget pipeline. |
...
HTML-focused tests under `OfficeIMO.Html.Tests`, `OfficeIMO.Word.Tests`, and `OfficeIMO.Reader.Tests` cover import, export, tables, lists, images, links, inline styles, page settings, code blocks, task lists, async and cancellation behavior, saved DOCX validity, and option contracts. Open fidelity and evidence work is tracked in [ROADMAP.md](ROADMAP.md).
