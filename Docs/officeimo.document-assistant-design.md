...
| Studio | Read-only PDF questions, connection recovery, local evidence preparation and reuse, OCR review handoff, cited answer copy/export, source navigation and per-tab cancellation; no dependency from the engine to the GUI |
...
6. The engine validates the untrusted response locally: fixed shape, duplicate/unknown properties, operation-specific collections, bounds, evidence identifiers, exact text quotes, field names, and scalar normalization. Extraction schemas require one stable key per requested field; request metadata maps each key to its caller-supplied name, and public results retain those names and their requested order. Truncated generations are rejected. IX rejects JSON nesting beyond 128 containers before its recursive parser; the document response validator applies its narrower depth contract.
...
| Exports | Versioned evidence report, proposed Reader JSON, CSV and Excel review tables; cited plain-text answers with source and snapshot hashes | Core multi-file exports refuse overwrite and are not atomic as a set. Studio answer export uses the selected destination and shared publication guards |
...
The current API processes immutable snapshots through a caller-owned executor. Studio binds those snapshots to the active PDF's identity, revision and page scope. It has no edit-proposal application, document collections, persistent history or index. AI requests do not authorize filesystem actions, shell commands, web research, publication or autonomous edits; explicit host commands own OCR and answer export. The [document assistant roadmap](ROADMAP.md#document-assistant) owns planned integrations and their acceptance requirements.
