...
2. `OfficeAiDocument` snapshots observations, original-byte SHA-256, source geometry and page provenance. `FromReadResult` is a trusted-producer boundary: the caller must establish that observations and images belong to those bytes and that access is permitted.
...
5. The executor receives explicit evidence and a fixed response schema. The IX adapter creates an ephemeral Treatment request without ambient tools or a fallback model. Compatible local profiles require loopback and bypass proxies; redirects are disabled.
6. The engine validates the untrusted response locally: fixed shape, duplicate/unknown properties, operation-specific arrays, bounds, evidence identifiers, exact text quotes, field names, and scalar normalization. Truncated generations are rejected.
...
| Long documents | Bounded batches, per-batch claims, explicit omissions | No retrieval index, mid-block splitting, or global summary synthesis |
...
| Compatible hosted/local | Same operations, inline image mapping, enforced-schema or prompted-JSON profiles | Loopback protocol tests establish transport contracts, not another vendor's or local model's accuracy |
...
The example's opt-in evaluation corpus generates English and Polish native PDFs, tables, image scans, scanned PDFs, rotated scans, mixed native/scan documents, abstention, conflicting fields, injected instructions, summary, and explanation cases. Sources are synthetic and generated by the example; reports retain source hashes, profiles, latency, token counters where available, expected values and case outcomes. Evaluation mode explicitly saves synthetic provider responses for inspection.
...
Held-out document quality, specialist recognition comparison, diagrams/multi-column reading order, geometry accuracy, peak-memory benchmarks, other independently authenticated hosted providers and actual local-model inference need their own measured evidence before those capabilities are qualified.
