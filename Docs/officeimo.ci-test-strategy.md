...
The solution has domain projects such as `OfficeIMO.Word.Tests`, `OfficeIMO.Excel.Tests`, `OfficeIMO.PowerPoint.Tests`, `OfficeIMO.Visio.Tests`, `OfficeIMO.Rtf.Tests`, `OfficeIMO.Html.Tests`, `OfficeIMO.Reader.Tests`, `OfficeIMO.Pdf.Tests`, `OfficeIMO.Markdown.Tests`, `OfficeIMO.Drawing.Tests`, `OfficeIMO.Markup.Tests`, `OfficeIMO.GoogleWorkspace.Tests`, `OfficeIMO.CSV.Tests`, `OfficeIMO.VerifyTests`, and `OfficeIMO.MarkdownRenderer.Wpf.Tests`. Repo-wide contracts that intentionally span product areas live in `OfficeIMO.Shared.Tests`, and shared fixtures/helpers live under `OfficeIMO.TestAssets`.

`OfficeIMO.Tests` is retired as a test owner. It made sense while features were being built quickly, but it should not own product-domain suites or shared fixtures anymore. The CI workflow includes a guard that fails if root test sources or `OfficeIMO.Tests/OfficeIMO.Tests.csproj` come back.

`OfficeIMO.Word.Tests`, `OfficeIMO.Excel.Tests`, `OfficeIMO.PowerPoint.Tests`, `OfficeIMO.Visio.Tests`, `OfficeIMO.Rtf.Tests`, `OfficeIMO.Html.Tests`, `OfficeIMO.Reader.Tests`, `OfficeIMO.Pdf.Tests`, `OfficeIMO.Markdown.Tests`, `OfficeIMO.Drawing.Tests`, `OfficeIMO.Markup.Tests`, `OfficeIMO.GoogleWorkspace.Tests`, and `OfficeIMO.Shared.Tests` are real splits from the aggregate. `OfficeIMO.Word.Tests` owns the core Word API, Word PDF conversion, Word/Markdown round-trip conversion tests, Google Docs payload, and fixture contracts. `OfficeIMO.Excel.Tests` owns the Excel test sources, Excel image export, Excel PDF, Google Sheets payload, and Excel compatibility contracts. `OfficeIMO.PowerPoint.Tests` owns the PowerPoint presentation API, PowerPoint image export, and PowerPoint PDF conversion tests. `OfficeIMO.Visio.Tests` owns the Visio document, stencil, diagram, SVG/PNG export, validation, and visual baseline contracts. `OfficeIMO.Rtf.Tests` owns native RTF, Word RTF conversion, RTF Markdown, RTF PDF, lossless editor, tokenizer, syntax, and golden corpus contracts. `OfficeIMO.Html.Tests` owns shared HTML policy, HTML ingestion, HTML PDF, and Office-to-HTML bridge contracts. `OfficeIMO.Reader.Tests` owns the unified reader API, modular reader adapters, reader packaging guardrails, and golden reader fixtures. `OfficeIMO.Pdf.Tests` owns native PDF, PDF compliance/readback, PDF visual-quality, raster baseline, and PDF conversion scenario contracts. `OfficeIMO.Markdown.Tests` owns pure Markdown parsing, standards profiles, rendering, native/source-map behavior, HTML-to-Markdown, transcript rendering, golden fixtures, and Markdown conversion contracts that belong with the Markdown engine. `OfficeIMO.Drawing.Tests` owns shared Drawing primitives, raster/SVG rendering, image composition, sparkline rendering, shape presets, and architecture guardrails for Drawing consumers. `OfficeIMO.Markup.Tests` owns Office Markup parsing, emitters, and Word/Excel/PowerPoint export contracts for the Markup authoring surface. `OfficeIMO.GoogleWorkspace.Tests` owns Google Workspace credential, diagnostics, and Google Docs/Google Sheets transport-boundary contracts. `OfficeIMO.Shared.Tests` owns repo-wide dependency, workflow, conversion-option, font, path, and cross-application encryption contracts. These projects have their own references and friend-assembly access, so their test warnings and internal contracts no longer ride through one aggregate test assembly.
...
- `OfficeIMO.Markdown.Tests` for pure Markdown parsing, rendering, CommonMark/GFM conformance, Markdown HTML conversion, source-map/native behavior, transcript rendering, golden fixtures, and Markdown conversion contracts. This project exists now and should be the target for new Markdown tests.
...
- `OfficeIMO.GoogleWorkspace.Tests` for Google Workspace credentials, diagnostics, and Google Docs/Google Sheets transport-boundary contracts. This project exists now and should be the target for new Google Workspace tests.
- `OfficeIMO.Shared.Tests` for repo-wide contracts that intentionally span products, such as dependency guardrails, workflow security checks, shared conversion option contracts, font resolution, path behavior, and cross-application encryption.
- `OfficeIMO.TestAssets` for shared test fixtures and helper sources only. It is not a test project.
...
Word, Excel, RTF, HTML, Reader, PDF, Markdown, Drawing, Markup, Google Workspace, and shared partitions run `OfficeIMO.Word.Tests`, `OfficeIMO.Excel.Tests`, `OfficeIMO.Rtf.Tests`, `OfficeIMO.Html.Tests`, `OfficeIMO.Reader.Tests`, `OfficeIMO.Pdf.Tests`, `OfficeIMO.Markdown.Tests`, `OfficeIMO.Drawing.Tests`, `OfficeIMO.Markup.Tests`, `OfficeIMO.GoogleWorkspace.Tests`, and `OfficeIMO.Shared.Tests` directly. The `other-projects` partition runs Drawing, Google Workspace, Markup, PowerPoint, Visio, Reader, Shared, CSV, Verify, and MarkdownRenderer WPF tests directly. The `markdown-large` partition also runs the Word/Markdown round-trip tests from `OfficeIMO.Word.Tests`, because those tests exercise Word document conversion behavior even though their class names are Markdown-oriented.
...
Some moved test projects carry scoped transitional suppressions for existing nullable, platform/framework analyzer, and xUnit style analyzer warnings so the real project split can continue without reintroducing annotation spam. New Word, Excel, PowerPoint, Visio, RTF, HTML, Reader, PDF, Markdown, Drawing, Markup, Google Workspace, and Shared tests should avoid adding new nullable debt, and follow-up cleanup should remove suppressions as the moved tests are made nullable-clean.
...
Restore and build the shared contract test project:

```powershell
dotnet restore OfficeIMO.Shared.Tests/OfficeIMO.Shared.Tests.csproj
dotnet build OfficeIMO.Shared.Tests/OfficeIMO.Shared.Tests.csproj --configuration Release --framework net8.0 --no-restore
```
...
Restore and build the Google Workspace test project:

```powershell
dotnet restore OfficeIMO.GoogleWorkspace.Tests/OfficeIMO.GoogleWorkspace.Tests.csproj
dotnet build OfficeIMO.GoogleWorkspace.Tests/OfficeIMO.GoogleWorkspace.Tests.csproj --configuration Release --framework net8.0 --no-restore
```
