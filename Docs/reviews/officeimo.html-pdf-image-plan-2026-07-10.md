...
- [ ] Complete advanced SVG, filters, masks, blend modes, and raster fallbacks. A bounded primitives, cubic-normalized path, affine transform-attribute, local shape/group/clipped-symbol reference, local object-bounding-box/user-space linear/radial paint-server with affine linear and axis-aligned radial transforms, and positioned/affine searchable tspan scene is now shared across `<img>` and CSS background sources in the current output backends, alongside layered inset/outer spread shadows; rotated/sheared radial and repeating paint servers, gradient/per-glyph/path text, masks, filters, and CSS-style transforms remain.
