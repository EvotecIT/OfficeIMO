...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors, full per-corner circular/elliptical rounded color/border/gradient/URL boxes, uniform solid/dashed/dotted/double borders, matching offset outlines, one outer alpha/blur box shadow, layered URL backgrounds with per-layer `auto`, `contain`, clipped `cover`, explicit size/position and bounded `repeat`/`space`/`round` patterns, opaque multi-stop linear and centered radial gradients, root-canvas propagation, and per-axis block/flex/grid/positioned overflow clipping are implemented. Side-specific border paint, inset/spread/multiple shadows, root/body viewport propagation, interactive scrollbar UI, `overflow-clip-margin`, margin collapsing, and remaining gradient forms remain.
- [ ] Implement replaced elements and image/SVG intrinsic sizing. Raster image pixel/DPI dimensions, authored width/height, content-box/border-box sizing, bounded positive aspect ratios, min/max constraints, flex/float intrinsic planning, all five object-fit modes, common/edge-offset object positions, and per-corner circular/elliptical content clipping now share native placement/source-crop output. Normal-inline replaced boxes, richer SVG intrinsic metadata, and replaced elements beyond images remain.
