...
- [ ] Keep searchable text runs and semantic nodes beside visual operations. Searchable HTML text, positioned/affine SVG tspan runs, links, and retained heading/paragraph roles are implemented. Rendered PDF now enables tagged catalog structure and maps heading levels plus paragraph fallback through a typed canvas-text role; grouped semantic nodes for lists, tables, figures, captions, and multi-fragment ownership remain.
