...
- [x] Table columns use bounded intrinsic text measurement for `table-layout:auto`, including min-content tokens, preferred unwrapped content, cell width constraints, colspans, and `<col>`/`<colgroup>` declarations. `table-layout:fixed` honors declared column and first-row cell widths, then distributes remaining width deterministically; cell placement, spans, wrapping, page breaks, and all output backends consume the same resolved track geometry.
- [x] Separate-border tables honor inherited one/two-axis non-negative absolute `border-spacing` around and between tracks, including rowspans and page-safe row offsets. `border-collapse:collapse` suppresses spacing in the shared grid geometry; complete collapsed-border conflict resolution remains open.
...
- [x] Shared SVG image metadata resolves absolute CSS lengths, pica and quarter-millimeter units, positive `viewBox` geometry, and intrinsic aspect ratios. A single authored width or height now derives its missing partner from `viewBox` without discarding the authored dimension; relative dimensions fall back to the intrinsic `viewBox` size. The corrected natural size feeds block, inline, flex, float, and background image planning. Dependency-free SVG scene interpretation for PNG/PDF remains a separate open renderer capability.
...
- [ ] Complete table layout and row/cell fragmentation. Styled top/bottom captions, empty-table caption retention, bounded auto/fixed intrinsic column sizing with column declarations and spanning constraints, separate-border spacing, collapsed-grid spacing suppression, occupancy-grid column placement, column spans, row-group-bounded row spans, span height, and span-safe row breaks are implemented; collapsed-border conflict resolution, non-text intrinsic cell contributions, and cell-content fragmentation remain.
...
- [ ] Complete advanced SVG, filters, masks, blend modes, and raster fallbacks. Layered inset/outer spread shadows are now shared across the current output backends.
