...
- [x] Shared SVG image metadata resolves absolute CSS lengths, pica and quarter-millimeter units, positive `viewBox` geometry, and intrinsic aspect ratios. A single authored width or height derives its missing partner from `viewBox` without discarding the authored dimension; relative dimensions fall back to the intrinsic `viewBox` size. A bounded dependency-free `OfficeIMO.Drawing` scene reader activates solid-painted rect, rounded-rect, circle, ellipse, line, polygon, polyline, and path primitives in `<img>` sources. Path data covers absolute/relative move, line, horizontal, vertical, cubic, smooth cubic, quadratic, smooth quadratic, elliptical arc, and close commands under a command limit. Endpoint arcs normalize radii and convert rotated large/small clockwise/counter-clockwise geometry to shared cubic paths with an exact final endpoint. The shared drawing visual scales and clips those scenes through PNG, native SVG, and direct PDF output; DTD/entity input is rejected and omitted features diagnose. SVG transforms, gradients, text, references, masks, filters, and CSS SVG backgrounds remain open.
...
- [ ] Complete advanced SVG, filters, masks, blend modes, and raster fallbacks. A bounded solid-primitives and cubic-normalized path SVG scene is now shared across the current output backends, alongside layered inset/outer spread shadows; transforms, paint servers, text, references, masks, filters, and background-image scene reuse remain.
