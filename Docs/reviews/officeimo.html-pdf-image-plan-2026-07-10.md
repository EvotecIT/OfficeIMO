...
- [x] Adjacent non-empty normal-flow block margins collapse using the CSS positive/negative margin rule. The collapsed flow offsets drive paint placement, positioned static origins, page-break offsets, PNG/SVG coordinates, and paged PDF fit decisions. Parent/first-child, parent/last-child, and collapse-through empty-block margin sets remain.
...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors, adjacent non-empty sibling margin collapse including negative margins, full per-corner circular/elliptical rounded color/border/gradient/URL boxes, independent side widths/colors and solid/dashed/dotted/double styles, matching offset outlines, one outer alpha/blur box shadow, layered URL backgrounds with per-layer `auto`, `contain`, clipped `cover`, explicit size/position and bounded `repeat`/`space`/`round` patterns, opaque multi-stop linear and centered radial gradients, root-canvas propagation, and per-axis block/flex/grid/positioned overflow clipping are implemented. Parent/child and collapse-through empty margins, inset/spread/multiple shadows, root/body viewport propagation, interactive scrollbar UI, `overflow-clip-margin`, and remaining gradient forms remain.
- [ ] Implement replaced elements and image/SVG intrinsic sizing. Raster image pixel/DPI dimensions, authored width/height, content-box/border-box sizing, bounded positive aspect ratios, min/max constraints, flex/float intrinsic planning, normal-inline wrapping and baseline participation, all five object-fit modes, common/edge-offset object positions, and per-corner circular/elliptical content clipping now share native placement/source-crop output. Richer SVG intrinsic metadata and replaced elements beyond images remain.
