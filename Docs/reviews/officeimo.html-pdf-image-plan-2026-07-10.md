...
- [x] Block-level `column` and `column-reverse` flex layout for one vertical column, including definite-height percentage bases, auto-height percentage fallback, vertical grow/shrink/basis and min/max constraints, main-axis gaps/distribution, horizontal alignment/stretch, order-modified reverse placement, complete-item page breaks, and shared PNG/SVG/searchable-PDF output. Multi-column wrapping remains explicit fallback behavior.
...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, relative positioning, horizontal and vertical flex geometry and pagination, resources, timeout, cancellation, and diagnostics.
...
- [ ] Implement flexbox, grid, and multicolumn formatting contexts. Block-level row flex covers ordering, reversal, wrapping and reverse wrapping, per-line grow/shrink/basis, min/max constraints, gaps, main/cross alignment, stretch, and nesting. Single-column vertical flex also covers normal/reverse placement, definite-height percentage bases, vertical sizing constraints, cross alignment, and item-boundary pagination; column wrapping, anonymous text items, auto margins, generated items, inline flex, richer intrinsic sizing, and advanced fragmentation remain.
...
- [ ] Make backgrounds, borders, positioned elements, flex/grid content, and links fragment correctly. Relatively positioned visuals retain separate flow and paint coordinates so page ownership follows normal flow. A supported single flex row that fits one page is kept together and moved to the next page, wrapped rows expose safe page boundaries only between complete flex lines, and vertical flex exposes boundaries between complete items; oversized line/item fragmentation, other positioned modes, and grid fragmentation remain.
