...
- [ ] Implement floats and relative/absolute/fixed/sticky positioning. Relative positioning is implemented for block and nested inline content with normal-flow-preserving fragmentation. Absolute elements resolve against the initial or nearest supported block/flex/grid containing block without consuming flow space, fixed elements repeat per page/viewport, and sticky elements produce stable in-flow document snapshots. Floats, inline positioned containing blocks, automatic static-position anchoring, and stacking contexts remain.
...
- [ ] Complete deterministic z-order, clipping, resource reuse, and fallback diagnostics. Surface/root/content paint order, source-order relative-position paint, source-order absolute/fixed overlays, back-to-front layered URL and linear/radial-gradient backgrounds, clipped cover, repeat patterns, encoded-image snapshot reuse, PDF image-XObject deduplication, native multi-stop axial/radial PDF gradient stitching, and stable layer/repeat/value/tile/stop-limit/positioning diagnostics are implemented; numeric `z-index`, negative layers, nested stacking contexts, remaining gradient forms, and general overflow clipping remain.
