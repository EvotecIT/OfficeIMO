...
- [x] Out-of-flow `position: absolute` for the initial containing block, nested supported positioned block ancestors, and direct flex/grid children. Explicit and opposing insets resolve against the padding box, percentage insets remain deterministic, nested descendants are extracted without consuming flow space, and the same positioned visuals feed PNG, SVG, and searchable PDF output. Negative layers paint behind in-flow content but above the containing background; auto/zero and positive integer layers paint in numeric then source order; nested descendants cannot escape their parent positioned stacking context. Unsupported inline containing blocks fall back to the initial containing block with a stable diagnostic; automatic static-position anchoring and relative/sticky stacking remain.
...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, relative/absolute/fixed/sticky positioning, nearest supported containing blocks, fixed page repetition, horizontal/vertical/inline flex, block/inline grid, row/column and dense grid flow, named areas/lines, wrapping, spans, reverse wrapping, anonymous/generated items, `display: contents`, auto margins, safe pagination, resources, timeout, cancellation, limits, and diagnostics.
...
- [ ] Implement floats and relative/absolute/fixed/sticky positioning. Relative positioning is implemented for block and nested inline content with normal-flow-preserving fragmentation. Absolute elements resolve against the initial or nearest supported block/flex/grid containing block without consuming flow space, fixed elements repeat per page/viewport, numeric absolute/fixed stacking contexts remain nested, and sticky elements produce stable in-flow document snapshots. Floats, inline positioned containing blocks, automatic static-position anchoring, and relative/sticky stacking contexts remain.
