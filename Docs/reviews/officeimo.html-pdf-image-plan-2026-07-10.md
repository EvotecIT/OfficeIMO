...
The current renderer deliberately starts with normal flow, block-level horizontal and vertical flex layout with wrapping, atomic inline flex boxes, block and atomic inline grid with explicit/implicit and responsive auto-fit/auto-fill tracks, row/column auto-flow, and named areas/lines, paint-only relative positioning, out-of-flow absolute positioning against the initial or nearest supported block/flex/grid containing block, block/flex/grid static-position anchoring for automatic insets, grid-area containing rectangles, fixed overlays repeated at viewport coordinates, negative/auto/positive numeric stacking bands for absolute and fixed contexts, stable static-document snapshots for sticky content, styled source text and `::before`/`::after` text with attributes and scoped counters, grapheme-safe token fragmentation, non-BMP font cmap lookup, policy-bound TrueType `@font-face` activation, managed PDF font fallback and Latin-ligature controls, exact explicit-whitespace extraction, policy-bound external stylesheet/import loading, preserved CSS font-family fallback lists, tables with row/column spans, images, layered CSS URL backgrounds with clipped `cover` and all standard repeat modes, opaque multi-stop linear gradients, centered opaque multi-stop radial ellipses, and root-canvas propagation, generic `@page` geometry, named page assignment, first/left/right and named pseudo-page margin content, all standard page-margin box positions, repeated table header/footer groups, span-safe line/row pagination, and bounded asynchronous resource resolution. It is not yet a complete browser layout engine. Full inline flex/grid baseline synthesis, nested generated flex/grid formatting contexts, subgrid, masonry, generated images and quote-state tokens, circle, off-center, explicit-radius, conic, repeating, alpha-varying, hard-stop, and length-positioned gradients, other CSS URL paint assets, WOFF/WOFF2 and CFF decoding, inline positioned containing blocks/static anchors, relative/sticky stacking contexts, per-master page geometry, pseudo-page body reflow, bidi, complex shaping, and other unfinished areas emit stable diagnostics or remain open below.
...
- [x] CSS `position: relative` for block, image, table, rule, and nested inline content. Leading insets win over trailing insets, horizontal percentages resolve against the containing width, vertical percentages resolve for explicitly sized containing blocks, and paint offsets do not change line wrapping, flow height, sibling placement, or page-fragment ownership.
- [x] Out-of-flow `position: absolute` for the initial containing block, nested supported positioned block ancestors, and direct flex/grid children. Explicit and opposing insets resolve against the padding box, percentage insets remain deterministic, and automatic insets use the hypothetical block-flow position accumulated through nested blocks, flex items, and grid items. Direct flex children honor main/cross alignment; direct grid children use declared numeric/named grid areas as their containing rectangles and honor self-alignment. Nested descendants are extracted without consuming flow space, and the same positioned visuals feed PNG, SVG, and searchable PDF output. Negative layers paint behind in-flow content but above the containing background; auto/zero and positive integer layers paint in numeric then source order; nested descendants cannot escape their parent positioned stacking context. Unsupported inline containing blocks and inline static anchors retain stable diagnostics; relative/sticky stacking remains.
...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, relative/absolute/fixed/sticky positioning, nearest supported containing blocks, block/flex/grid static anchors, positioned grid areas, numeric stacking contexts, fixed page repetition, horizontal/vertical/inline flex, block/inline grid, row/column and dense grid flow, named areas/lines, wrapping, spans, reverse wrapping, anonymous/generated items, `display: contents`, auto margins, safe pagination, resources, timeout, cancellation, limits, and diagnostics.
...
- [ ] Resolve display roles, generated content, counters, replaced elements, pseudo-elements, and stacking contexts. Styled `::before`/`::after` text, attributes, scoped counters, cascade/specificity, inline/block-container placement, and nested numeric stacking bands for absolute/fixed contexts are implemented; generated images, quote-state tokens, richer pseudo boxes, replaced elements, and relative/sticky/general stacking contexts remain.
...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors, layered URL backgrounds with per-layer `auto`, `contain`, clipped `cover`, explicit size/position and bounded `repeat`/`space`/`round` patterns, opaque multi-stop linear and centered radial gradients, and root-canvas propagation are implemented; remaining gradient forms and general overflow clipping remain.
- [ ] Implement replaced elements and image/SVG intrinsic sizing.
- [ ] Implement floats and relative/absolute/fixed/sticky positioning. Relative positioning is implemented for block and nested inline content with normal-flow-preserving fragmentation. Absolute elements resolve against the initial or nearest supported block/flex/grid containing block without consuming flow space; automatic insets retain block-flow positions through nested block/flex/grid items; direct flex alignment and grid-area containing rectangles are active; fixed elements repeat per page/viewport; numeric absolute/fixed stacking contexts remain nested; and sticky elements produce stable in-flow document snapshots. Floats, inline positioned containing blocks/static anchors, and relative/sticky stacking contexts remain.
...
- [ ] Make backgrounds, borders, positioned elements, flex/grid content, and links fragment correctly. Relatively positioned visuals retain separate flow and paint coordinates so page ownership follows normal flow. Absolute visuals remain attached to their containing block, root absolute visuals appear once, fixed visuals repeat on every page without affecting fragmentation, and negative/auto/positive absolute/fixed stacking bands retain backend-neutral paint order. A supported single flex row that fits one page is kept together and moved to the next page, wrapped rows expose safe page boundaries only between complete flex lines, a single vertical flex column exposes boundaries between complete items, and grid exposes only row boundaries that no spanning item crosses; oversized line/item, multi-column flex, overlapping grid, inline positioned containing blocks, relative/sticky stacking, and advanced grid fragmentation remain.
...
- [ ] Reuse or extend `OfficeIMO.Drawing` for paths, text outlines, images, gradients, transforms, clipping, shadows, and opacity. Shapes, text, images, active font faces, source crop, clipping, bounded image-pattern visuals with independent repeat steps, and multi-stop linear/radial gradient paint are shared now; advanced clipping groups and paint effects remain.
...
- [ ] Complete deterministic z-order, clipping, resource reuse, and fallback diagnostics. Surface/root/content paint order, source-order relative-position paint, numeric negative/auto/positive absolute/fixed stacking bands, nested positioned contexts, back-to-front layered URL and linear/radial-gradient backgrounds, clipped cover, repeat patterns, encoded-image snapshot reuse, PDF image-XObject deduplication, native multi-stop axial/radial PDF gradient stitching, and stable layer/repeat/value/tile/stop-limit/positioning diagnostics are implemented; relative/sticky/inline stacking contexts, remaining gradient forms, and general overflow clipping remain.
...
- [ ] Complete mapping to PDF structures. Searchable Unicode text with managed fallback controls, exact explicit spaces, active TrueType web-font embedding, basic shapes, multi-stop axial/radial gradients through native PDF stitching functions, images, and external links are implemented; more than three simultaneous distinct web-font families currently diagnose and fall back because the PDF writer exposes three semantic generated-font slots, while positioned shaped glyphs and richer semantics remain.
