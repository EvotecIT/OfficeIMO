...
- [x] URL policy, per-resource and total byte budgets, timeout, cancellation, content-type checks, surface limits, page limits, layout-depth limits, and operation-wide background-tile limits.
...
- [x] Layered CSS background-image loading from inline, data-URI, linked, and recursively imported CSS, including per-layer size, position, clipped `cover`, `repeat`/`repeat-x`/`repeat-y`, root-canvas propagation, normal boxes, and table cells. Layers paint back-to-front and are bounded per element; a shared pattern visual keeps SVG compact while the thin PDF adapter expands clipped placements over deduplicated image resources. Gradients and `space`/`round` repeat still diagnose.
...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors plus layered URL backgrounds with per-layer `auto`, `contain`, clipped `cover`, explicit size/position, bounded repeat patterns, and root-canvas propagation are implemented; gradients, `space`/`round`, and general overflow clipping remain.
...
- [ ] Reuse or extend `OfficeIMO.Drawing` for paths, text outlines, images, gradients, transforms, clipping, shadows, and opacity. Shapes, text, images, active font faces, source crop, clipping, and bounded image-pattern visuals are shared now; advanced clipping groups and paint effects remain.
...
- [ ] Complete deterministic z-order, clipping, resource reuse, and fallback diagnostics. Surface/root/content paint order, back-to-front layered URL backgrounds, clipped cover, repeat patterns, encoded-image snapshot reuse, PDF image-XObject deduplication, and stable layer/repeat/value/tile-limit diagnostics are implemented; gradient layers and general stacking contexts remain.
