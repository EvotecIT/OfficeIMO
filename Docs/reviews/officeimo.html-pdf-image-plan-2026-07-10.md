...
- [ ] Complete deterministic z-order, clipping, resource reuse, and fallback diagnostics. Surface/root/content paint order, numeric negative/auto/positive absolute/fixed/relative/sticky stacking bands across block/flex/grid/inline contexts, nested positioned contexts, back-to-front layered URL and linear/radial-gradient backgrounds, clipped cover, repeat patterns, encoded-image snapshot reuse, PDF image-XObject deduplication, native multi-stop axial/radial PDF gradient stitching, and stable layer/repeat/value/tile/stop-limit/positioning diagnostics are implemented; transform/opacity stacking contexts, remaining gradient forms, and general overflow clipping remain.
