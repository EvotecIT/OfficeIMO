...
- [x] Shared SVG image metadata resolves absolute CSS lengths, pica and quarter-millimeter units, positive `viewBox` geometry, and intrinsic aspect ratios. A single authored width or height derives its missing partner from `viewBox` without discarding the authored dimension; relative dimensions fall back to the intrinsic `viewBox` size. A bounded dependency-free `OfficeIMO.Drawing` scene reader activates rect, rounded-rect, circle, ellipse, line, polygon, polyline, path, and simple text primitives in `<img>` sources. Paint supports solid colors plus local `url(#id)` linear/radial fill and stroke servers in object-bounding-box coordinates, bounded stop colors/opacities, normalized endpoints, and bounded cycle-safe same-type `href` inheritance. Local `<use>` references expand ordinary shapes and groups with inherited paint, x/y placement, transforms, duplicate-ID rejection, cycle/depth protection, and the shared element-operation budget. Path data covers absolute/relative move, line, horizontal, vertical, cubic, smooth cubic, quadratic, smooth quadratic, elliptical arc, and close commands under a command limit. Endpoint arcs normalize radii and convert rotated large/small clockwise/counter-clockwise geometry to shared cubic paths with an exact final endpoint. Ordered nested SVG transform attributes support matrix, translate, scale, rotate around the origin or a center, and x/y skew in normalized `viewBox` coordinates. Simple text supports one x/y/dx/dy position, whitespace normalization or `xml:space="preserve"`, start/middle/end anchoring, family, size, weight, style, solid fill, and opacity. PNG and SVG consume Drawing text directly; the thin PDF adapter projects those runs to native searchable text in scene order. The shared drawing visual scales and clips the whole scene through all output paths; DTD/entity input, external or ambiguous references, reference cycles, oversized stop lists, user-space/transformed/repeating paint servers, and omitted features diagnose. Gradient text, nested spans, transformed text, `symbol` viewport references, masks, filters, CSS-style transforms, and CSS SVG backgrounds remain open.
...
- [ ] Complete advanced SVG, filters, masks, blend modes, and raster fallbacks. A bounded primitives, cubic-normalized path, affine transform-attribute, local shape/group reference, local object-bounding-box linear/radial paint-server, and simple searchable-text SVG scene is now shared across the current output backends, alongside layered inset/outer spread shadows; user-space/transformed/repeating paint servers, gradient text, nested spans, transformed text, `symbol` viewport references, masks, filters, CSS-style transforms, and background-image scene reuse remain.
