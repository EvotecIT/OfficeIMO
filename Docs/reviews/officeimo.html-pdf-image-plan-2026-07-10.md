...
- [ ] Keep searchable text runs and semantic nodes beside visual operations. Searchable HTML text, positioned/affine SVG tspan runs, and links are implemented; semantic nodes remain.
...
- [ ] Complete advanced SVG, filters, masks, blend modes, and raster fallbacks. A bounded primitives, cubic-normalized path, affine transform-attribute, local shape/group reference, local object-bounding-box linear/radial paint-server, and positioned/affine searchable tspan scene is now shared across the current output backends, alongside layered inset/outer spread shadows; user-space/transformed/repeating paint servers, gradient/per-glyph/path text, `symbol` viewport references, masks, filters, CSS-style transforms, and background-image scene reuse remain.
