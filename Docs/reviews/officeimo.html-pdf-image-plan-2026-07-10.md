...
- [x] Layered CSS background-image loading from inline, data-URI, linked, and recursively imported CSS, including per-layer size, position, clipped `cover`, `repeat`, `space`, `round`, `repeat-x`, `repeat-y`, root-canvas propagation, normal boxes, and table cells. Layers paint back-to-front, `none` retains CSS list semantics without blocking body-canvas propagation, and paint is bounded per element and operation. A shared pattern visual keeps SVG compact while the thin PDF adapter expands clipped placements over deduplicated image resources. Bounded opaque multi-stop `linear-gradient()` layers share native Drawing gradients and PDF stitching functions; other gradient functions still diagnose.
...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors, layered URL backgrounds with per-layer `auto`, `contain`, clipped `cover`, explicit size/position and bounded `repeat`/`space`/`round` patterns, opaque multi-stop linear gradients, and root-canvas propagation are implemented; remaining gradient forms and general overflow clipping remain.
...
- [ ] Reuse or extend `OfficeIMO.Drawing` for paths, text outlines, images, gradients, transforms, clipping, shadows, and opacity. Shapes, text, images, active font faces, source crop, clipping, bounded image-pattern visuals with independent repeat steps, and multi-stop linear-gradient paint are shared now; advanced clipping groups and paint effects remain.
