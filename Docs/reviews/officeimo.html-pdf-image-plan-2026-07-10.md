...
- [ ] Preserve source and semantic references needed for diagnostics, links, accessibility, and extraction order. Visuals retain source descriptions, links, paint order, layout ownership, and heading/paragraph roles; heading fragments from the same source element now retain one operation-scoped semantic identity and aggregate into ordered shared heading nodes, while the shared render document retains bounded source title, language, and typed root direction metadata for all adapters. General grouped DOM-semantic ownership and reading-order nodes remain.
...
- [ ] Implement managed shaping, bidi resolution, ligatures, kerning, combining marks, and script-aware fallback. The computed-style cascade now treats valid HTML `dir` attributes as low-specificity presentational hints, the shared document retains the resolved root direction, rendered PDF maps RTL roots to right-to-left viewer page progression, and one shared Drawing scalar detector drives PDF analysis plus stable per-source HTML bidi diagnostics. The direct PDF adapter exposes the existing managed Latin-ligature mode and optional shaping-provider seam, but shared inline layout must still perform bidi resolution and consume positioned glyph results.
...
- [ ] Keep searchable text runs and semantic nodes beside visual operations. Searchable HTML text, positioned/affine SVG tspan runs, links, and retained heading/paragraph roles are implemented. Multi-fragment HTML headings share stable semantic identity and aggregate into a backend-neutral navigation node. Rendered PDF enables tagged catalog structure, maps heading levels plus paragraph fallback through a typed canvas-text role, projects those shared heading nodes through the native PDF outline model, and groups multi-operation SVG image scenes under one typed canvas Figure with retained alternative text; grouped semantic nodes for lists, tables, captions, and general multi-fragment ownership remain.
...
- [ ] Complete mapping to PDF structures. Searchable Unicode HTML and positioned/affine SVG tspan text with managed fallback controls, exact explicit spaces, active TrueType web-font embedding, basic shapes, multi-stop axial/radial gradients through native PDF stitching functions, raster and grouped vector figures with alternative text, external links, affine effect transforms, isolated transparency Form XObjects, tagged document markers, typed H1-H6/P canvas structure, and nested native PDF outlines from shared heading nodes are implemented; more than three simultaneous distinct web-font families currently diagnose and fall back because the PDF writer exposes three semantic generated-font slots, while positioned shaped glyphs and richer list/table/caption semantics remain.
...
- [ ] Validate page geometry, extraction, links, outlines, metadata, encryption, and tagged structure. Rendered PDF now carries retained HTML title and document language into PDF metadata/catalog, requests display of the document title, maps resolved RTL root direction to viewer page progression, emits typed H1-H6/P marked content, retains nested heading outlines with absolute page destinations, and proves raster plus multi-operation SVG image alt text through Figure structure; richer structure and encryption-profile coverage remain.
