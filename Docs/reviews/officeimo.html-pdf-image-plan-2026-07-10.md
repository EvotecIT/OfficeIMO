...
- [ ] Keep searchable text runs and semantic nodes beside visual operations. Searchable HTML text, positioned/affine SVG tspan runs, links, and retained heading/paragraph roles are implemented. Multi-fragment HTML headings share stable semantic identity and aggregate into a backend-neutral navigation node. PDF canvas now owns typed nested section/division/list/list-item/label/body/table/row/header-cell/data-cell/caption containers with table scope and span attributes, while the adapter already projects heading outlines and grouped multi-operation SVG figures; shared HTML list/table/caption ownership still needs to feed those containers.
