...
- [ ] Complete advanced SVG, filters, masks, blend modes, and raster fallbacks. A bounded solid-primitives SVG scene is now shared across the current output backends, alongside layered inset/outer spread shadows; paths, transforms, paint servers, text, references, masks, filters, and background-image scene reuse remain.
