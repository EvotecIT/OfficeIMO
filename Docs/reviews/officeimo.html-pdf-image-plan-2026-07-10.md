...
- [ ] Keep searchable text runs and semantic nodes beside visual operations. Searchable HTML text, positioned/affine SVG tspan runs, links, and retained heading/paragraph roles are implemented. Multi-fragment HTML headings share stable semantic identity and aggregate into a backend-neutral navigation node. Paint-neutral shared semantic groups now preserve list/item/label/body and table/caption/row/header-cell/data-cell ownership, scope, and spans through translation, pagination, PNG/SVG traversal, font discovery, and typed PDF canvas containers. More general reading-order grouping remains.
