...
- [ ] Complete table layout and row/cell fragmentation. Styled top/bottom captions, empty-table caption retention, bounded auto/fixed intrinsic column sizing with column declarations and spanning constraints, occupancy-grid column placement, column spans, row-group-bounded row spans, span height, and span-safe row breaks are implemented; separate/collapsed border models, non-text intrinsic cell contributions, and cell-content fragmentation remain.
