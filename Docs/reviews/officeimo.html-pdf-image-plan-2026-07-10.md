...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, floats and clearance, overflow clipping and static scroll snapshots, relative/absolute/fixed/sticky positioning, block/flex/grid/wrapped-inline containing rectangles and static anchors, positioned grid areas, nested numeric and transform/opacity stacking contexts, isolated descendant and gradient opacity, affine scale/rotation, uniform rounded color/border/gradient boxes, solid/dashed/dotted/double uniform borders, offset outlines, outer alpha/blur shadows, fixed page repetition, horizontal/vertical/inline flex, block/inline grid, row/column and dense grid flow, named areas/lines, wrapping, spans, multicolumn balancing/fill/spanners/rules, reverse wrapping, anonymous/generated items, `display: contents`, auto margins, effect-group pagination, resources, timeout, cancellation, limits, and diagnostics.
...
- [ ] Resolve display roles, generated content, counters, replaced elements, pseudo-elements, and stacking contexts. Styled `::before`/`::after` text, attributes, scoped counters, cascade/specificity, inline/block-container placement, nested numeric stacking bands for absolute/fixed/relative/sticky block, root, flex, grid, and inline contexts, and block/atomic-inline transform/opacity stacking contexts are implemented; generated images, quote-state tokens, richer pseudo boxes, replaced elements, and normal-inline effect grouping remain.
...
- [ ] Reuse or extend `OfficeIMO.Drawing` for paths, text outlines, images, gradients, transforms, clipping, shadows, and opacity. Shapes, uniform rounded rectangles, native dashed/dotted strokes, layered double strokes, offset outlines, one outer alpha/blur shadow, text, images, active font faces, source crop, clipping, bounded image-pattern visuals with independent repeat steps, multi-stop linear/radial gradient paint, nested affine effect groups, and isolated group opacity are shared now; per-side borders, per-corner rounded clips, inset/spread/multiple shadows, filters, blend modes, and other advanced paint effects remain.
