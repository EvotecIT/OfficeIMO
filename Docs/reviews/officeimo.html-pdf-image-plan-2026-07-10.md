...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, relative positioning, horizontal and vertical flex geometry, wrapping, reverse wrapping, and safe pagination, resources, timeout, cancellation, and diagnostics.
...
- [ ] Implement flexbox, grid, and multicolumn formatting contexts. Block-level row and column flex cover ordering, main/cross reversal, wrapping and reverse wrapping, per-line grow/shrink/basis, min/max constraints, gaps, main/cross alignment, stretch, definite-height vertical percentage bases, intrinsic cross-size planning, nesting, and safe single-axis boundaries; anonymous text items, auto margins, generated items, inline flex, richer intrinsic sizing, and advanced multi-column fragmentation remain.
