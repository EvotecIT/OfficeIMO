...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, relative positioning, single- and multi-line row flex geometry and pagination, resources, timeout, cancellation, and diagnostics.
...
- [ ] Implement flexbox, grid, and multicolumn formatting contexts. Block-level row flex now covers ordering, reversal, wrapping and reverse wrapping, per-line grow/shrink/basis, min/max constraints, gaps, main/cross alignment, stretch, and nesting; columns, anonymous text items, auto margins, generated items, inline flex, richer intrinsic sizing, and advanced fragmentation remain.
...
- [ ] Make backgrounds, borders, positioned elements, flex/grid content, and links fragment correctly. Relatively positioned visuals retain separate flow and paint coordinates so page ownership follows normal flow. A supported single flex row that fits one page is kept together and moved to the next page when needed, while wrapped rows expose safe page boundaries only between complete flex lines; oversized line/item fragmentation, other positioned modes, and grid fragmentation remain.
