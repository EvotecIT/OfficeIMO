...
- [x] Block-level single-line `row` and `row-reverse` flex layout with ordered element items, `flex` grow/shrink/basis allocation, min/max freezing and redistribution, gaps, main-axis distribution, cross-axis alignment/stretch, nested flex containers, relative-positioned items, stable source-order paint, and explicit fallback diagnostics for unsupported flex cases and values.
...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, relative positioning, single-line row flex geometry and pagination, resources, timeout, cancellation, and diagnostics.
...
- [ ] Implement flexbox, grid, and multicolumn formatting contexts. Block-level single-line row flex now covers ordering, reversal, grow/shrink/basis, min/max constraints, gaps, main/cross alignment, stretch, and nesting; wrapping, columns, anonymous text items, auto margins, generated items, inline flex, richer intrinsic sizing, and advanced fragmentation remain.
...
- [ ] Make backgrounds, borders, positioned elements, flex/grid content, and links fragment correctly. Relatively positioned visuals retain separate flow and paint coordinates so page ownership follows normal flow. A supported single flex row that fits one page is kept together and moved to the next page when needed; oversized and wrapped flex fragmentation, other positioned modes, and grid fragmentation remain.
