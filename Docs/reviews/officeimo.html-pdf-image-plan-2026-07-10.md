...
- [x] Separate-border tables honor inherited one/two-axis non-negative absolute `border-spacing` around and between tracks, including rowspans and page-safe row offsets. `border-collapse:collapse` suppresses spacing and resolves each shared cell-edge segment once using hidden, width, and supported style precedence; solid/dashed/dotted/double winners become shared Drawing lines for PNG, SVG, and PDF. Table/row-group/row/column origin precedence remains open.
...
- [ ] Complete table layout and row/cell fragmentation. Styled top/bottom captions, empty-table caption retention, bounded auto/fixed intrinsic column sizing with column declarations and spanning constraints, separate-border spacing, collapsed-grid spacing suppression, cell-to-cell collapsed-border conflict resolution, occupancy-grid column placement, column spans, row-group-bounded row spans, span height, and span-safe row breaks are implemented; full table/row-group/row/column collapsed-border origin precedence, non-text intrinsic cell contributions, and cell-content fragmentation remain.
