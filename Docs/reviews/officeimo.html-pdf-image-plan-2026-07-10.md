...
- [x] Single-layer CSS background-image loading from inline, data-URI, linked, and recursively imported CSS, including size, position, clipped `cover`, `repeat`/`repeat-x`/`repeat-y`, root-canvas propagation, normal boxes, and table cells. A bounded shared pattern visual keeps SVG compact while the thin PDF adapter expands clipped placements over one deduplicated image resource; extra layers and `space`/`round` repeat still diagnose.
...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors plus single background images with `auto`, `contain`, clipped `cover`, explicit size/position, bounded repeat patterns, and root-canvas propagation are implemented; layers, gradients, `space`/`round`, and general overflow clipping remain.
...
- [ ] Complete deterministic z-order, clipping, resource reuse, and fallback diagnostics. Surface/root/content paint order, clipped cover, repeat patterns, encoded-image snapshot reuse, PDF image-XObject deduplication, and stable layer/repeat/value/tile-limit diagnostics are implemented; layered paint and general stacking contexts remain.
