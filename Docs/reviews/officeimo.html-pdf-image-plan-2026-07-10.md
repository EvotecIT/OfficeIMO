...
- [x] Uniform circular CSS `border-radius` for color, border, and gradient box paint through native rounded Drawing shapes and PDF paths. Asymmetric/elliptical corners and rounded URL-image clipping use explicit diagnostics until shared per-corner clip geometry lands.
...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, floats and clearance, overflow clipping and static scroll snapshots, relative/absolute/fixed/sticky positioning, block/flex/grid/wrapped-inline containing rectangles and static anchors, positioned grid areas, nested numeric and transform/opacity stacking contexts, isolated descendant and gradient opacity, affine scale/rotation, uniform rounded color/border/gradient boxes, outer alpha/blur shadows, fixed page repetition, horizontal/vertical/inline flex, block/inline grid, row/column and dense grid flow, named areas/lines, wrapping, spans, multicolumn balancing/fill/spanners/rules, reverse wrapping, anonymous/generated items, `display: contents`, auto margins, effect-group pagination, resources, timeout, cancellation, limits, and diagnostics.
...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors, uniform circular rounded color/border/gradient boxes, one outer alpha/blur box shadow, layered URL backgrounds with per-layer `auto`, `contain`, clipped `cover`, explicit size/position and bounded `repeat`/`space`/`round` patterns, opaque multi-stop linear and centered radial gradients, root-canvas propagation, and per-axis block/flex/grid/positioned overflow clipping are implemented. Asymmetric/elliptical corners, rounded URL-image clipping, inset/spread/multiple shadows, root/body viewport propagation, interactive scrollbar UI, `overflow-clip-margin`, margin collapsing, and remaining gradient forms remain.
...
- [ ] Reuse or extend `OfficeIMO.Drawing` for paths, text outlines, images, gradients, transforms, clipping, shadows, and opacity. Shapes, uniform rounded rectangles, one outer alpha/blur shadow, text, images, active font faces, source crop, clipping, bounded image-pattern visuals with independent repeat steps, multi-stop linear/radial gradient paint, nested affine effect groups, and isolated group opacity are shared now; per-corner rounded clips, inset/spread/multiple shadows, filters, blend modes, and other advanced paint effects remain.
