...
- [x] Layered CSS background-image loading from inline, data-URI, linked, and recursively imported CSS, including per-layer size, position, clipped `cover`, `repeat`/`repeat-x`/`repeat-y`, root-canvas propagation, normal boxes, and table cells. Layers paint back-to-front and are bounded per element; a shared pattern visual keeps SVG compact while the thin PDF adapter expands clipped placements over deduplicated image resources. Bounded opaque multi-stop `linear-gradient()` layers share native Drawing gradients and PDF stitching functions. Other gradient functions and `space`/`round` repeat still diagnose.
...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors, layered URL backgrounds with per-layer `auto`, `contain`, clipped `cover`, explicit size/position and bounded repeat patterns, opaque multi-stop linear gradients, and root-canvas propagation are implemented; remaining gradient forms, `space`/`round`, and general overflow clipping remain.
...
- [ ] Reuse or extend `OfficeIMO.Drawing` for paths, text outlines, images, gradients, transforms, clipping, shadows, and opacity. Shapes, text, images, active font faces, source crop, clipping, bounded image-pattern visuals, and multi-stop linear-gradient paint are shared now; advanced clipping groups and paint effects remain.
...
- [ ] Complete deterministic z-order, clipping, resource reuse, and fallback diagnostics. Surface/root/content paint order, back-to-front layered URL and linear-gradient backgrounds, clipped cover, repeat patterns, encoded-image snapshot reuse, PDF image-XObject deduplication, native multi-stop PDF gradient stitching, and stable layer/repeat/value/tile/stop-limit diagnostics are implemented; remaining gradient forms and general stacking contexts remain.
...
- [ ] Complete mapping to PDF structures. Searchable Unicode text with managed fallback controls, exact explicit spaces, active TrueType web-font embedding, basic shapes, multi-stop linear gradients through native PDF stitching functions, images, and external links are implemented; more than three simultaneous distinct web-font families currently diagnose and fall back because the PDF writer exposes three semantic generated-font slots, while positioned shaped glyphs and richer semantics remain.
