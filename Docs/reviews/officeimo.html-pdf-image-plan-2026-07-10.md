...
- [ ] Complete advanced SVG, filters, masks, blend modes, and raster fallbacks. A bounded solid-primitives, cubic-normalized path, and affine transform-attribute SVG scene is now shared across the current output backends, alongside layered inset/outer spread shadows; paint servers, text, references, masks, filters, CSS-style transforms, and background-image scene reuse remain.
