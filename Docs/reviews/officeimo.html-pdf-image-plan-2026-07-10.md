...
- [x] Uniform circular CSS `border-radius` for color, border, and gradient box paint through native rounded Drawing shapes and PDF paths. Uniform `solid`, `dashed`, `dotted`, and `double` borders plus matching outlines and signed outline offsets use the same shared vector scene across PNG, SVG, and PDF. Side-specific border paint, asymmetric/elliptical corners, and rounded URL-image clipping use explicit diagnostics until shared per-side/per-corner geometry lands.
...
- [x] Replaced block/flex/grid/float image sizing from intrinsic pixel dimensions and DPI, authored width/height, `box-sizing`, positive `aspect-ratio`, and min/max constraints. `object-fit: fill | contain | cover | none | scale-down` plus keyword, length, percentage, and edge-offset `object-position` values resolve to one backend-neutral placement and source crop shared by PNG, SVG, and PDF. Normal-inline image boxes and rounded replaced-image clipping remain explicit gaps.
...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, floats and clearance, overflow clipping and static scroll snapshots, relative/absolute/fixed/sticky positioning, block/flex/grid/wrapped-inline containing rectangles and static anchors, positioned grid areas, nested numeric and transform/opacity stacking contexts, isolated descendant and gradient opacity, affine scale/rotation, uniform rounded color/border/gradient boxes, solid/dashed/dotted/double uniform borders, offset outlines, outer alpha/blur shadows, intrinsic/aspect/min-max image sizing, all five object-fit modes, object-position placement/cropping, fixed page repetition, horizontal/vertical/inline flex, block/inline grid, row/column and dense grid flow, named areas/lines, wrapping, spans, multicolumn balancing/fill/spanners/rules, reverse wrapping, anonymous/generated items, `display: contents`, auto margins, effect-group pagination, resources, timeout, cancellation, limits, and diagnostics.
...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors, uniform circular rounded color/border/gradient boxes, uniform solid/dashed/dotted/double borders, matching offset outlines, one outer alpha/blur box shadow, layered URL backgrounds with per-layer `auto`, `contain`, clipped `cover`, explicit size/position and bounded `repeat`/`space`/`round` patterns, opaque multi-stop linear and centered radial gradients, root-canvas propagation, and per-axis block/flex/grid/positioned overflow clipping are implemented. Side-specific border paint, asymmetric/elliptical corners, rounded URL-image clipping, inset/spread/multiple shadows, root/body viewport propagation, interactive scrollbar UI, `overflow-clip-margin`, margin collapsing, and remaining gradient forms remain.
- [ ] Implement replaced elements and image/SVG intrinsic sizing. Raster image pixel/DPI dimensions, authored width/height, content-box/border-box sizing, positive aspect ratios, min/max constraints, flex/float intrinsic planning, all five object-fit modes, and common/edge-offset object positions now share native placement/source-crop output. Normal-inline replaced boxes, richer SVG intrinsic metadata, replaced elements beyond images, and rounded image clipping remain.
