...
The current renderer deliberately starts with normal flow, block-level horizontal and vertical flex layout with wrapping, atomic inline flex boxes, block and atomic inline grid with explicit/implicit and responsive auto-fit/auto-fill tracks, row/column auto-flow, and named areas/lines, bounded multicolumn layout with balanced or explicit-height fill, full-width direct block spanners, vector column rules, and safe shared page-break boundaries, left/right and direction-aware logical-side floats with line-band wrapping and clearance, per-axis `visible`/`hidden`/`clip`/`auto`/`scroll` overflow groups, paint-only relative positioning, out-of-flow absolute positioning against initial, block, flex, grid, and measured inline containing rectangles, block/flex/grid/inline static-position anchoring for automatic insets, grid-area containing rectangles, fixed overlays repeated at viewport coordinates, negative/auto/positive numeric stacking bands across absolute, fixed, relative, and sticky block/flex/grid/inline contexts, block and atomic inline 2D transform/opacity stacking contexts, stable static-document snapshots for sticky content, styled source text and `::before`/`::after` text with attributes and scoped counters, grapheme-safe token fragmentation, non-BMP font cmap lookup, policy-bound TrueType `@font-face` activation, managed PDF font fallback and Latin-ligature controls, exact explicit-whitespace extraction, policy-bound external stylesheet/import loading, preserved CSS font-family fallback lists, tables with row/column spans, intrinsic/aspect-sized block/flex/grid/float images with all five object-fit modes and positioned source crops, layered CSS URL backgrounds with clipped `cover` and all standard repeat modes, opaque multi-stop linear gradients, centered opaque multi-stop radial ellipses, and root-canvas propagation, generic `@page` geometry, named page assignment, first/left/right and named pseudo-page margin content, all standard page-margin box positions, repeated table header/footer groups, span-safe line/row pagination, and bounded asynchronous resource resolution. It is not yet a complete browser layout engine. Root/body overflow propagation, interactive scrollbars, `overflow-clip-margin`, cross-block float intrusion, `shape-outside`, full inline flex/grid baseline synthesis, normal-inline replaced images and transform/opacity grouping, 3D and perspective transforms, blend modes, inline background/border fragment painting, nested generated flex/grid formatting contexts, subgrid, masonry, nested descendant column spanners and advanced column-rule styles, generated images and quote-state tokens, circle, off-center, explicit-radius, conic, repeating, alpha-varying, hard-stop, and length-positioned gradients, other CSS URL paint assets, WOFF/WOFF2 and CFF decoding, per-master page geometry, pseudo-page body reflow, bidi, complex shaping, and other unfinished areas emit stable diagnostics or remain open below.
...
- [x] Layered CSS background-image loading from inline, data-URI, linked, and recursively imported CSS, including per-layer size, position, clipped `cover`, `repeat`, `space`, `round`, `repeat-x`, `repeat-y`, root-canvas propagation, normal boxes, and table cells. Layers paint back-to-front, `none` retains CSS list semantics without blocking body-canvas propagation, and paint is bounded per element and operation. A shared pattern visual keeps SVG compact while the thin PDF adapter expands clipped placements over deduplicated image resources. Bounded opaque multi-stop `linear-gradient()` and centered elliptical `radial-gradient()` layers share native Drawing gradients and PDF stitching functions; other gradient forms still diagnose.
- [x] Backend-neutral clipped visual groups for per-axis `overflow`, `overflow-x`, and `overflow-y` values `visible`, `hidden`, `clip`, `auto`, and `scroll`. Generic, positioned, flex, grid, nested, and paginated content keeps vector shapes, native images, searchable text, clipped PDF links, and identical PNG/SVG/PDF geometry; scrollable boxes use a diagnosed initial static snapshot without fake interactive controls.
...
- [x] Uniform circular CSS `border-radius` for color, border, gradient, URL-image, repeated-pattern, and replaced-image paint through native rounded Drawing shapes and shared path-clipped groups. Uniform `solid`, `dashed`, `dotted`, and `double` borders plus matching outlines and signed outline offsets use the same shared vector scene across PNG, SVG, and PDF. Side-specific border paint and asymmetric/elliptical corners use explicit diagnostics until shared per-side/per-corner geometry lands.
- [x] One outer CSS `box-shadow` with offsets, optional bounded approximate blur, zero spread, current/supported color, alpha, and uniform rounded geometry. Shared Drawing raster/SVG and PDF shapes now honor the same blur layers and combined color/shadow opacity; inset, non-zero spread, and multiple shadows diagnose explicitly.
- [x] Replaced block/flex/grid/float image sizing from intrinsic pixel dimensions and DPI, authored width/height, `box-sizing`, bounded positive `aspect-ratio`, and min/max constraints. `object-fit: fill | contain | cover | none | scale-down` plus keyword, length, percentage, and edge-offset `object-position` values resolve to one backend-neutral placement and source crop shared by PNG, SVG, and PDF. Uniform rounded content clipping is shared; normal-inline image boxes remain an explicit gap.
...
- [x] Contract tests for PNG, SVG, searchable PDF text, links, page geometry, media rules, custom properties, generated content and counters, floats and clearance, overflow clipping and static scroll snapshots, relative/absolute/fixed/sticky positioning, block/flex/grid/wrapped-inline containing rectangles and static anchors, positioned grid areas, nested numeric and transform/opacity stacking contexts, isolated descendant and gradient opacity, affine scale/rotation, uniform rounded color/border/gradient/URL/replaced-image boxes, solid/dashed/dotted/double uniform borders, offset outlines, outer alpha/blur shadows, intrinsic/aspect/min-max image sizing, all five object-fit modes, object-position placement/cropping, fixed page repetition, horizontal/vertical/inline flex, block/inline grid, row/column and dense grid flow, named areas/lines, wrapping, spans, multicolumn balancing/fill/spanners/rules, reverse wrapping, anonymous/generated items, `display: contents`, auto margins, effect-group pagination, resources, timeout, cancellation, limits, and diagnostics.
...
- [ ] Implement block flow, margin collapsing, padding, borders, backgrounds, sizing, and overflow. Background colors, uniform circular rounded color/border/gradient/URL boxes, uniform solid/dashed/dotted/double borders, matching offset outlines, one outer alpha/blur box shadow, layered URL backgrounds with per-layer `auto`, `contain`, clipped `cover`, explicit size/position and bounded `repeat`/`space`/`round` patterns, opaque multi-stop linear and centered radial gradients, root-canvas propagation, and per-axis block/flex/grid/positioned overflow clipping are implemented. Side-specific border paint, asymmetric/elliptical corners, inset/spread/multiple shadows, root/body viewport propagation, interactive scrollbar UI, `overflow-clip-margin`, margin collapsing, and remaining gradient forms remain.
- [ ] Implement replaced elements and image/SVG intrinsic sizing. Raster image pixel/DPI dimensions, authored width/height, content-box/border-box sizing, bounded positive aspect ratios, min/max constraints, flex/float intrinsic planning, all five object-fit modes, common/edge-offset object positions, and uniform rounded content clipping now share native placement/source-crop output. Normal-inline replaced boxes, richer SVG intrinsic metadata, and replaced elements beyond images remain.
...
- [ ] Reuse or extend `OfficeIMO.Drawing` for paths, text outlines, images, gradients, transforms, clipping, shadows, and opacity. Shapes, uniform rounded rectangles, native dashed/dotted strokes, layered double strokes, offset outlines, one outer alpha/blur shadow, text, intrinsic/aspect-sized images, object-fit/object-position source crops, active font faces, rectangular overflow clips, rounded/freeform path-clipped visual groups, bounded image-pattern visuals with independent repeat steps, multi-stop linear/radial gradient paint, nested affine effect groups, and isolated group opacity are shared now; per-side borders, per-corner rounded clips, inset/spread/multiple shadows, filters, blend modes, and other advanced paint effects remain.
...
- [ ] Complete mapping to PDF structures. Searchable Unicode text with managed fallback controls, exact explicit spaces, active TrueType web-font embedding, basic shapes, multi-stop axial/radial gradients through native PDF stitching functions, images, external links, affine effect transforms, and isolated transparency Form XObjects are implemented; more than three simultaneous distinct web-font families currently diagnose and fall back because the PDF writer exposes three semantic generated-font slots, while positioned shaped glyphs and richer semantics remain.
...
- [ ] Complete advanced SVG, filters, masks, blend modes, inset/spread/multiple shadows, and raster fallbacks.
