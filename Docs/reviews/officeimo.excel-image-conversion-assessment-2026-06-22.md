...
Latest font diagnostics checkpoint: Excel cell, rich text run, chart text, and page-sliced header/footer image export use Drawing's `IMAGE_FONT_SUBSTITUTED` contract. Caller-supplied TrueType faces are resolved before platform fallback, and diagnostics retain the Excel cell, chart, or header/footer source reference. Focused tests cover missing and caller-scoped fonts across plain cells, rich text, chart titles, and formatted header/footer text.
...
- Text/layout: wrapping, vertical alignment, styled sizes, shrink-to-fit, rotation, stacked text, clipping, and run-preserving rich text use shared Drawing measurement and emission paths. Excel retains cell policy and emits `ExcelCellTextClipped`, `ExcelCellTextRotationApproximation`, and `ExcelCellRichTextLayoutApproximation`; missing requested faces use Drawing's `IMAGE_FONT_SUBSTITUTED`. Remaining work is Excel-exact spill, rotated/stacked baselines, broader font matrices, and deeper layered-object clipping parity.
...
- Charts: chart snapshots carry the supported layout, area/series/point/marker styling, gridlines, axes, titles, labels, number formats, and typography into the shared Drawing renderer. Caller-supplied fonts flow into the chart drawing and missing concrete chart faces use `IMAGE_FONT_SUBSTITUTED`. Stable Excel diagnostics cover unsupported trendlines, richer labels/effects, axis/style approximations, unsupported chart kinds, and other source-specific loss. Remaining work is richer point/marker/effect styling, log and non-default axis geometry, broader number formats, per-element rich text, and closer Excel chart geometry.
