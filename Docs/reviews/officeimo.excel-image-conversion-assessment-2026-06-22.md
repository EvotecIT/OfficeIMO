...
Latest rich-text SVG checkpoint: Drawing now has `OfficeTextBlockRenderer.AppendSvgRichTextBlock` as the SVG companion to `DrawRasterRichTextBlock`. Excel cell rich-text SVG and header/footer formatted-run SVG now route through the shared Drawing helper for line top, baseline, cursor, style, decoration, and optional rotation emission. The approved rich-text baseline still passes without regeneration, so this is a renderer-brain consolidation rather than a visual refresh.
...
Latest shape-text vertical-orientation diagnostic checkpoint: Excel worksheet drawing-object extraction now maps DrawingML `bodyPr vert` into a neutral `ExcelDrawingTextOrientation` value instead of leaking OpenXML enum spellings through the image snapshot. PNG and SVG export still render non-horizontal shape text through the shared horizontal `OfficeDrawingText` path, but now emit `ExcelDrawingShapeTextVerticalOrientationUnsupported` with a sheet/cell source reference so vertical shape text is a tracked premium gap rather than a silent approximation.
