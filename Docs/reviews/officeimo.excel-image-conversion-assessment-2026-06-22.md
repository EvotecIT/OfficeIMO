...
Latest stacked-text checkpoint: `OfficeIMO.Drawing` now owns `OfficeTextLayoutEngine.LayoutStackedTextBlock` for upright stacked text layout. Excel `TextRotation=255` no longer renders horizontally or emits `ExcelCellStackedTextRotationUnsupported`; it uses the shared Drawing layout in PNG/SVG and reports `ExcelCellTextRotationApproximation` while we keep Excel-exact stacked rich text as future work. A dedicated approved stacked-text PNG/SVG baseline now gates this output.
