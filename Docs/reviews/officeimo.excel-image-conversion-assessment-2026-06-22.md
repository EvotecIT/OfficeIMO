...
Latest PDF image-placement consolidation checkpoint: `OfficeIMO.Drawing.OfficeImageRenderPlan` now owns reusable image target-box, fit, and source-crop placement math, including separate top-left and bottom-left coordinate-system entrypoints. The first-party PDF writer now consumes that Drawing plan for image draw rectangles in flow, table, header/footer, and canvas contexts instead of maintaining a local crop-plus-fit algorithm. PDF remains responsible for PDF clip paths, XObject streams, tagging, annotations, and compression, but generic visual placement math has moved into the shared engine where Excel worksheet images and later adapters can reuse it.
