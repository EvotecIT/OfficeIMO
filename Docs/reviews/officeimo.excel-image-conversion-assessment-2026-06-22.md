...
Latest header/footer image checkpoint: page-sliced worksheet PNG/SVG export now renders supported `&G` header/footer PNG images through shared Drawing composition instead of rejecting all picture placeholders as unsupported chrome. Multiple header/footer image positions are preserved in the VML header/footer drawing part, SVG output embeds Drawing-approved image MIME types, PNG output raster-composes decodable PNG header/footer images, and unsupported raster formats are reported through `ExcelHeaderFooterImageUnsupported`. A dedicated approved PNG/SVG header-footer-image visual baseline now gates the public worksheet export path; placement remains a diagnosed `ExcelHeaderFooterImageApproximation` until Excel-exact image geometry and broader raster decoding are implemented.
