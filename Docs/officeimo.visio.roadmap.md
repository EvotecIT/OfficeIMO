...
- Graph, architecture, and network stencil nodes selected from a catalog preserve catalog provenance in stencil profiles, and built-in catalog stencil nodes inherit diagram theme styling unless they come from package-backed external artwork.
...
- Architecture and network builder components now use first-party stencil catalogs for provenance, so cloud architecture and network segmentation gallery output are baseline-reviewed as stencil-backed diagram content rather than anonymous geometry.
