...
- Graph, architecture, network, flowchart, and block-diagram stencil nodes selected from a catalog now keep their catalog name in inspection/profile output and built-in catalog stencil nodes inherit theme styling, so the premium executive dependency graph, cloud architecture gallery, network segmentation gallery, print audit trail gallery, and technical topology gallery can use first-party stencils without losing the polished rendered look.
...
- Stencil-backed visuals are not broad enough. The premium executive dependency graph, premium cloud architecture sample, premium network segmentation sample, premium print audit trail sample, and premium technical topology sample now use first-party stencil metadata, but several premium samples still rely on basic geometry rather than recognizable first-party/native/external stencil symbols.
