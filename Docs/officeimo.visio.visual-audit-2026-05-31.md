...
- `CreateStencilProfile()` now summarizes generated-master, package-backed, and basic-geometry shape usage plus Shape Data keys, semantic kind usage, stencil catalogs, categories, tags, source package paths, and connection-point richness from inspection snapshots; generated and package-backed stencil provenance survives save/load.
- Graph stencil nodes selected from a catalog now keep their catalog name in inspection/profile output and built-in catalog stencil nodes inherit theme styling, so the premium executive dependency graph can use first-party architecture stencils without losing the polished rendered look.
