...
- Imported master artwork children are filtered out of stencil profile counts, so package-backed profiles report logical placed stencil shapes instead of internal master artwork, and typed stencil-family sections show package/catalog provenance, shape counts, master-kind counts, and connection-point coverage at family level.
...
6. Expand stencil extraction beyond current catalog/category/source-pack/family provenance into icon/preview metadata, richer package-family details, aliases, dimensions, and deeper native-pack metadata.
