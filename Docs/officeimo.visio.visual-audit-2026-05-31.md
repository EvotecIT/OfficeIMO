...
- Graph and architecture stencil nodes selected from a catalog now keep their catalog name in inspection/profile output and built-in catalog stencil nodes inherit theme styling, so the premium executive dependency graph and cloud architecture gallery can use first-party architecture stencils without losing the polished rendered look.
...
- Stencil-backed visuals are not broad enough. The premium executive dependency graph and premium cloud architecture sample now use first-party architecture stencil metadata, but most premium samples still rely on basic geometry rather than recognizable first-party/native/external stencil symbols.
