...
- Connector label cleanup now runs a second stabilization pass over all current labels, can slide labels along connector paths before offsetting them, ignores generated adornment shapes, and can avoid unrelated connector paths, reducing order-sensitive label and line collisions in dense pages.
...
- Connector labels are improved but still not finished. Cleanup now revisits all current labels, can slide labels along their connector paths, ignores generated adornments, can avoid unrelated connector paths, can prefer common endpoint zones, and normal sequence messages avoid intermediate lifelines, activation bands, and fragment guard labels, but denser graph pages and whole-page label strategy still need more work.
...
2. Continue connector label cleanup beyond current connector-path-aware, path-position-aware, and lifeline-aware placement with denser graph, whole-page, and deeper sequence edge-case strategies.
