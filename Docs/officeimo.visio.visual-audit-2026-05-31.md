...
- Sequence self-message placement was fixed for the reviewed long-label case, semantic notes now exist as first-party sequence-stencil shapes, and activation bars plus combined fragment frames now have first-party stencil/profile coverage. Note collision avoidance in compact premium layouts and richer guarded/partitioned interaction fragments are still missing.
...
- Stencil-backed visuals are not broad enough. All eight current premium baselines now carry first-party stencil catalog provenance for their domain shapes, including Sequence Diagram provenance for incident participants, activations, and combined fragments, but deeper sequence constructs still need guarded/partitioned fragments and collision-safe notes before the sequence gallery can be called fully premium.
...
5. Add richer sequence-diagram features: collision-safe notes, guarded/partitioned fragments, and visible error/remediation bands.
