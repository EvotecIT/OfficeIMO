...
- standards smoke baseline: 242 CommonMark `0.31.2` fixtures, 36 cmark-gfm extension fixtures, and a focused upstream ignored-autolink crash regression
...
- parsed raw HTML processing-instruction and CDATA blocks now expose source-backed opening marker, body, and closing marker syntax tokens plus native source fields, caret lookup, source edits, and snapshot projection while broader declarations and malformed raw HTML stay on the whole-block fallback
- parsed simple raw HTML tag frames now expose source-backed opening tag, body, and closing tag syntax tokens plus native source fields, caret lookup, source edits, and snapshot projection while broader declarations and malformed raw HTML stay on the whole-block fallback
