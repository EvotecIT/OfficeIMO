...
| Markdig extension inventory | 33 Markdig extension-family rows in `Docs/officeimo.markdown.markdig-extension-inventory.md`: 8 covered, 11 partial, 3 intentional, 11 gap |
...
- [ ] Markdig extension parity is not closed: 8 extension families are `Covered`, 11 are `Partial`, 3 are `Intentional`, and 11 are `Gap`.
...
- [ ] **4. Engine plus proof: close `UseDefinitionLists`.** Existing behavior is real but still `Partial`. Finish marker-group, lazy-continuation, nested-block, loose-definition, source-map, writer, and reparse-stability edges before promotion.
...
- [ ] **P1 - Finish `UseDefinitionLists`.** Type: engine plus proof. Remaining work is source-map and writer edge breadth for grouped markers, lazy continuation, nested blocks, loose definitions, and reparse stability.
- [ ] **P1 - Finish `UseNonAsciiNoEscape`.** Type: renderer policy. Visible text paths, image/link attribute text, sanitizer escaping, and custom renderer helper APIs are now routed through `HtmlOptions.EscapeNonAsciiText`; remaining work is direct attribute/id/head/asset/security-owned output paths that still call the platform encoder directly.
...
- [ ] **Markdig extension-family coverage is far from closed.** The current inventory is 8 `Covered`, 11 `Partial`, 3 `Intentional`, and 11 `Gap`. Every non-covered row needs one decision: implement in core, implement as optional extension, route to renderer/host policy, or mark intentional out of scope.
...
- [ ] Stop broad fixture-chasing and work the remaining rows in this order.
  ...
  - [ ] `UseNonAsciiNoEscape`: continue moving renderer output paths to `HtmlOptions.EscapeNonAsciiText`; inline text, link display text, code text, captions, simple quote text, abbreviation output, TOC labels/titles, heading helper text, page titles, raw-HTML escape output, image-blocked placeholder text, portable fallback helper text, image/link title and alt attributes, picture-source descriptor attributes, sanitizer escape output, and custom HTML render-extension helper APIs are covered, while remaining direct attribute/id/head/asset/security-owned output paths still need explicit policy routing or documentation.
