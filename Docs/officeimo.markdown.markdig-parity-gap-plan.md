...
- [x] **Moved `UseGenericAttributes` through definition-list interactions.** Definition-list term attributes now project to `<dt>` and stay source-backed on the semantic term, while definition-value paragraph attributes are consumed without `<dd>` projection and preserve Markdig's consumed separator whitespace.
