...
| Markdig extension inventory | 33 Markdig extension-family rows in `Docs/officeimo.markdown.markdig-extension-inventory.md`: 9 covered, 10 partial, 3 intentional, 11 gap |
...
- [ ] Markdig extension parity is not closed: 9 extension families are `Covered`, 10 are `Partial`, 3 are `Intentional`, and 11 are `Gap`.
...
- [ ] **C. Close high-value partial extension families.** Finish `UseDefinitionLists`, `UseAbbreviations`, `UseAlertBlocks`, `UseGenericAttributes`, `UsePreciseSourceLocation`, parser/render extensions, `UseCjkFriendlyEmphasis`, `UseDiagrams`, `UseFigures`, `UseMathematics`, and `UseMediaLinks` with an explicit owner and promotion bar.
- [ ] **D. Classify every gap row before coding.** Decide whether `UseCustomContainers`, `UseGridTables`, `UseSmartyPants`, `UseCitations`, `UseEmojiAndSmiley`, `UseFooters`, `UseGlobalization`, `UseJiraLinks`, `UseListExtras`, `UsePragmaLines`, and `UseReferralLinks` belong in core, optional extension packages, renderer/host policy, deferred scope, or intentional out-of-scope docs.
...
- [ ] **4. Engine plus proof: close `UseDefinitionLists`.** Existing behavior is real but still `Partial`. Parsed and generated/rebuilt definition marker tokens, native source-backed marker edits, tight nested-list writer preservation, and multiline native definition-body source edits are now covered; finish remaining marker-group, lazy-continuation, loose-definition, source-map, writer, and reparse-stability edges before promotion.
- [ ] **5. Mostly proof plus lossless decision: close `UseAbbreviations`.** Existing behavior is real but still `Partial`. Parse-owned Markdown writing preserves abbreviation definitions including empty-title and list-item-contained definitions, and unresolved bracket text, nested link-label, blockquote, list, dash/opening-punctuation boundary, pipe-table-cell expansion, and Unicode visible-text output have Markdig comparison plus AST/native proof where applicable. The formerly named bracket-text, list-item-definition, and Unicode text-rendering residuals are closed; final source-location/lossless writer breadth remains before promotion.
- [ ] **6. Scope decision first: classify every `Gap` row before coding it.** Assign `UseCustomContainers`, `UseGridTables`, `UseSmartyPants`, `UseCitations`, `UseEmojiAndSmiley`, `UseFooters`, `UseGlobalization`, `UseJiraLinks`, `UseListExtras`, `UsePragmaLines`, and `UseReferralLinks` to core parser, optional extension, renderer/host policy, deferred, or intentional out-of-scope.
...
- [ ] **P1 - Finish `UseAbbreviations` promotion decision.** Type: mostly evidence/lossless decision. Current parser/render/write behavior is substantial; the missing decision is whether list-contained definition source-location and lossless writer breadth are enough to promote or need one more source-map slice.
- [ ] **P1 - Finish `UseDefinitionLists`.** Type: engine plus proof. Parsed and generated/rebuilt definition marker tokens, source-backed native fields, tight nested-list writer preservation, and multiline native definition-body source edits are covered; remaining work is source-map and writer edge breadth for grouped markers, lazy continuation, loose definitions, and reparse stability.
...
- [ ] **P2 - Decide parser-extension gap rows.** Type: scope decision before implementation. Classify `UseCustomContainers`, `UseGridTables`, `UseListExtras`, `UseMathematics`, `UseEmojiAndSmiley`, and `UseSmartyPants` as core, optional extension, deferred, or intentional.
- [ ] **P2 - Decide renderer/policy gap rows.** Type: scope decision before implementation. Classify `UseMediaLinks`, `UseReferralLinks`, `UseDiagrams`, `UseFigures`, citations, Jira links, pragma lines, footers, and globalization before adding syntax.
...
- [ ] **Definition-list syntax breadth is partial.** OfficeIMO now parses the pinned Markdig colon-marker form, including multiple terms, multiple definitions, grouped AST/source/native/html proof, parsed and generated/rebuilt definition marker tokens, native source-backed marker edits, Markdig lazy paragraph and nested block continuation, loose-definition HTML, edge-continuation comparison, empty-marker first-continuation source mapping, grouped Markdown writing that preserves the marker form for reparsing, loose-definition writer preservation, blank-separated marker-group writer preservation, tight nested-list writer preservation, and multiline native definition-body source edits that preserve valid continuation indentation. Remaining source-map and writer edge breadth still need focused comparison before `UseDefinitionLists` can move to `Covered`.
...
- [ ] **Markdig extension-family coverage is far from closed.** The current inventory is 9 `Covered`, 10 `Partial`, 3 `Intentional`, and 11 `Gap`. Every non-covered row needs one decision: implement in core, implement as optional extension, route to renderer/host policy, or mark intentional out of scope.
- [ ] **High-priority partial rows need closure.** Work through `UseDefinitionLists`, `UseAbbreviations`, `UseAlertBlocks`, `UseGenericAttributes`, `UsePreciseSourceLocation`, and parser/render extensions with parser, AST/source, renderer, writer, and fixture evidence.
...
- [ ] Stop broad fixture-chasing and work the remaining rows in this order.
  ...
  - [ ] `UseDefinitionLists`: parsed and generated/rebuilt definition marker syntax, source-backed native fields, tight nested-list writer preservation, and multiline native definition-body source edits are covered; finish remaining source-map and writer edge breadth for marker groups, lazy continuation, loose definitions, and reparse stability; promote only after nested source/native and writer evidence is complete.
