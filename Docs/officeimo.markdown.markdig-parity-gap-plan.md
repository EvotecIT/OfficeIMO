...
- [ ] **5. Close `UseAbbreviations`.** Existing behavior is real but still `Partial`. Parse-owned Markdown writing preserves abbreviation definitions including empty-title definitions, and nested link-label, blockquote, list, dash/opening-punctuation boundary, and pipe-table-cell expansion has Markdig comparison plus AST/native proof. A 449-case disposable `UseAbbreviations` sweep is down to 11 residual mismatches: Unicode HTML entity rendering, bracket-text expansion, and list-item abbreviation definitions. Finish or intentionally classify those residuals before promotion.
...
- [ ] **Abbreviation parity is partial, not closed.** `UseAbbreviations` now has opt-in parser, semantic AST, syntax/native metadata, HTML rendering, source-edit, selected Markdig comparison evidence across nested inline/container/table-cell contexts, Markdig-compatible trailing-dash and opening-punctuation boundaries, empty-title definition handling, AST/native propagation proof, and parse-owned definition-preserving Markdown writing. The current disposable 449-case sweep leaves 11 named residuals: Unicode HTML entity rendering, bracket-text expansion, and list-item abbreviation definitions.
...
- [ ] Stop broad fixture-chasing and work the remaining rows in this order.
  ...
  - [ ] `UseAbbreviations`: parse-owned Markdown writing preserves abbreviation definition syntax including empty-title definitions, and nested link-label, blockquote, list, boundary, and pipe-table-cell expansion now has Markdig and AST/native proof; close or intentionally classify the 11 residual sweep mismatches before promotion.
