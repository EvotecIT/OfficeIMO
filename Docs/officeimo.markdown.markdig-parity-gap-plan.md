...
- [ ] **Definition-list syntax breadth is partial.** OfficeIMO now parses the pinned Markdig colon-marker form, including multiple terms, multiple definitions, grouped AST/source/native/html proof, Markdig lazy paragraph and nested block continuation, loose-definition HTML, edge-continuation comparison, and grouped Markdown writing that preserves the marker form for reparsing. Remaining writer and source-map edge breadth still need focused comparison before `UseDefinitionLists` can move to `Covered`.
