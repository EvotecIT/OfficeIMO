...
- [ ] **1. Promote or bound `UseDefinitionLists`.**
  Current probes closed empty-marker blank-separated body source/writer preservation, the blank-separated pre-marker term boundary, table-shaped continuation profile behavior, the setext-following lazy-continuation boundary, and the thematic-break-following lazy-continuation boundary with writer reparse preservation. Next probes should target remaining lazy continuation variants plus nested/multiline body source spans.
