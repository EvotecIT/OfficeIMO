...
- [ ] Stop broad fixture-chasing and work the remaining rows in this order.
  ...
  - [ ] `UseNonAsciiNoEscape`: continue moving visible renderer text paths to `HtmlOptions.EscapeNonAsciiText`; inline text, link display text, code text, captions, simple quote text, and abbreviation output are covered, while attribute, TOC/helper, raw-HTML escape, and renderer-extension paths still need explicit policy routing.
