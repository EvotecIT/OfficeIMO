...
- [ ] **Parser gate: close `UseAutoLinks`.** This is the current active engine row. Recent work added Markdig-compatible single trailing punctuation/underscore trimming, `www` host-underscore rejection, http/www/ftp user-info authority rejection, Markdig left-boundary comparison, bare `mailto:` path/query parsing, and optional href `~` percent-encoding with source/native/writer proof. Remaining work is broader Markdig/GFM edge breadth plus any non-autolink renderer-policy deltas. Promote only when Markdig-compatible and GFM-compatible profiles are both explicit; otherwise leave a named intentional delta.
