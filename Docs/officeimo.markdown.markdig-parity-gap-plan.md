...
- [ ] **1. Close `UseAutoLinks` as the next parser slice.** Finish the remaining bare URL, `www`, email, boundary, punctuation, source/native, writer, and GFM breadth. Current work is engine work: Markdig-style punctuation-before-outside-`)`, lowercase `www.` prefix behavior, lowercase bare scheme prefix behavior, profile-selectable bare scheme prefixes, Unicode HTTP/WWW/FTP rendered-href normalization, and AutoLinks+PipeTables comparison evidence are in place, but the row stays `Partial` until the wider corpus is covered.
