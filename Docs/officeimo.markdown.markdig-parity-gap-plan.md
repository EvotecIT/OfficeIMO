...
- [ ] **4. Engine plus proof: close `UseDefinitionLists`.** Existing behavior is real but still `Partial`. Definition marker tokens and native marker source edits are now covered; finish remaining marker-group, lazy-continuation, nested-block, loose-definition, source-map, writer, and reparse-stability edges before promotion.
...
- [ ] **P1 - Finish `UseDefinitionLists`.** Type: engine plus proof. Definition marker tokens/source fields are covered; remaining work is source-map and writer edge breadth for grouped markers, lazy continuation, nested blocks, loose definitions, and reparse stability.
...
- [ ] **Definition-list syntax breadth is partial.** OfficeIMO now parses the pinned Markdig colon-marker form, including multiple terms, multiple definitions, grouped AST/source/native/html proof, source-backed definition marker tokens and native marker source edits, Markdig lazy paragraph and nested block continuation, loose-definition HTML, edge-continuation comparison, empty-marker first-continuation source mapping, grouped Markdown writing that preserves the marker form for reparsing, loose-definition writer preservation, and blank-separated marker-group writer preservation. Remaining source-map and writer edge breadth still need focused comparison before `UseDefinitionLists` can move to `Covered`.
...
- [ ] Stop broad fixture-chasing and work the remaining rows in this order.
  ...
  - [ ] `UseDefinitionLists`: definition marker syntax/source fields are covered; finish remaining source-map and writer edge breadth for marker groups, lazy continuation, nested blocks, and reparse stability; promote only after nested source/native and writer evidence is complete.
