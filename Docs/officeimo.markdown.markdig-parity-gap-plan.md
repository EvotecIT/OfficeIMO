...
- [ ] **1. Close `UseAutoLinks` as the next parser slice.** Finish the remaining bare URL, `www`, email, scheme, previous-character, boundary, punctuation, table-cell, source/native, and writer breadth. Current work is engine work: Markdig-style punctuation-before-outside-`)`, lowercase `www.` prefix behavior, lowercase bare scheme prefix behavior, and Unicode HTTP/WWW rendered-href normalization are options, but the row stays `Partial` until the wider corpus is covered.
...
- [x] **4. Finish the Unicode autolink href-normalization slice.** Unicode HTTP/WWW destinations now render Markdig-compatible hrefs through IDNA hostnames and UTF-8 percent-encoded paths/queries while source/native/writer literals preserve the author spelling.
- [ ] **5. Continue `UseAutoLinks` until it can promote or the remaining deltas are explicit.** The next autolink pass should broaden email, `www`, scheme, boundary, table-cell, and writer/source cases rather than only adding one-off fixtures.
- [ ] **6. Move to raw HTML/tag-filter/security or definition lists only after the autolink row has a clear promotion decision.**
...
- [ ] **Autolinks are still partial.** CommonMark angle autolinks are green, Markdig-style previous-character/domain-without-period/query-fragment/balanced-parenthesis punctuation, punctuation-before-closing-parenthesis preservation, lowercase `www.` prefix, and lowercase bare scheme prefix options exist, bare `ftp://`, `tel:`, and `mailto:` scheme autolinks now have Markdig/source/writer evidence, Unicode HTTP/WWW href rendering now uses IDNA hostnames and UTF-8 percent-encoded paths while preserving source/native/writer literals, and source-backed Markdown writing preserves parsed bare and angle autolink spelling, but `UseAutoLinks` remains partial because extended bare URL, `www`, plain-email-vs-Markdig policy, scheme, boundary, punctuation, and table-cell breadth still need broader Markdig/GFM evidence before promotion.
