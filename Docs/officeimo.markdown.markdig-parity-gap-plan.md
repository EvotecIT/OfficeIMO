...
- [ ] `UseGenericAttributes`: generic attribute storage now exists on semantic and syntax nodes, with fenced-code metadata projected through ordinary and semantic fenced-block paths and rendered on default fenced HTML; opt-in trailing attribute blocks now parse/render/write for ATX headings, Setext headings, and paragraphs. Finish broader block-family coverage, inline span attribute parsing, token-level source-edit coverage, and writer/source breadth before promotion.
