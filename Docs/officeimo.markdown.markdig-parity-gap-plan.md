...
- [ ] **Parser gate: close `UseAutoLinks`.** This is the current active engine row. Recent work added Markdig-compatible single trailing punctuation/underscore trimming, `www` host-underscore rejection, http/www/ftp user-info authority rejection, optional closing-bracket URL consumption, Markdig left-boundary comparison, bare `mailto:` path/query parsing, and optional href `~` percent-encoding with source/native/writer proof. Remaining work is broader Markdig/GFM edge breadth plus any non-autolink renderer-policy deltas. Promote only when Markdig-compatible and GFM-compatible profiles are both explicit; otherwise leave a named intentional delta.
...
- [ ] **Parser gate: close `UseAbbreviations`.** Existing behavior is real but still `Partial`. Parse-owned Markdown writing preserves abbreviation definitions, and nested link-label, blockquote, list, and pipe-table-cell expansion now has Markdig comparison plus AST/native proof; finish remaining Markdig edge breadth and any remaining parser/source/writer deltas before promotion.
...
- [ ] Choose exactly one next row: finish `UseAutoLinks`, decide whether `UseAbbreviations` has enough remaining value to push to promotion, close `UseDefinitionLists`, or split raw HTML/tag-filter/security. Do not start a new extension family until that choice is explicit.
...
- [ ] **1. Close `UseAutoLinks` as the next parser slice.** Finish the remaining bare URL, `www`, email, boundary, punctuation, source/native, writer, renderer-policy, and GFM breadth. Current work is engine work: Markdig-style punctuation-before-outside-`)`, single trailing punctuation/underscore trimming, lowercase `www.` prefix behavior, `www` host-underscore rejection, http/www/ftp user-info authority rejection, optional closing-bracket URL consumption, lowercase bare scheme prefix behavior, profile-selectable bare scheme prefixes, bare `mailto:` path/query parsing, Unicode HTTP/WWW/FTP rendered-href normalization, optional Markdig-compatible href `~` percent-encoding, and AutoLinks+PipeTables comparison evidence are in place, but the row stays `Partial` until the wider corpus and remaining policy deltas are covered.
...
- [x] **3. Finish the current autolink slice.** The engine/profile split for Markdig punctuation-before-outside-`)` behavior versus cmark-gfm trimming, plus lowercase-only `www.` and bare scheme prefix matching, is validated and committed.
- [x] **4. Finish the Unicode autolink href-normalization slice.** Unicode HTTP/WWW/FTP destinations now render Markdig-compatible hrefs through IDNA hostnames and UTF-8 percent-encoded paths/queries while source/native/writer literals preserve the author spelling.
- [x] **5. Add profile-selectable bare scheme prefixes.** The Markdig comparison lane can now link `mailto:`, `ftp://`, and `tel:` while leaving bare `xmpp:` literal, without removing OfficeIMO/GFM support for `xmpp:` where that profile wants it.
- [ ] **6. Continue `UseAutoLinks` until it can promote or the remaining deltas are explicit.** The next autolink pass should broaden email, `www`, boundary, writer/source, and GFM cases rather than only adding one-off fixtures.
- [ ] **7. Move to raw HTML/tag-filter/security or definition lists only after the autolink row has a clear promotion decision.**
