...
- [ ] Finish `UseGenericAttributes`.
  ...
  - [ ] Prove container interactions such as blockquotes, lists, tables, footnotes, and definition lists by contract, not incidental HTML output.
    ...
    - [x] Blockquote-contained standalone generic attributes before unordered, ordered, and task lists now attach to the nested list like Markdig, with syntax/native source-field and source-edit proof on the unordered-list path.
...
- [ ] Complete delimiter-token coverage for extension nodes.
  ...
  - [x] Structured details blocks now carry source-backed details opening/closing and summary opening/text/closing syntax/native fields with snapshots, position lookup, and source-edit proof.
