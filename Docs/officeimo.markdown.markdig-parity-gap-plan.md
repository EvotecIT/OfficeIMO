...
- [ ] **Definition-list syntax breadth is partial.** OfficeIMO now parses the pinned Markdig colon-marker form, including multiple terms, multiple definitions, grouped AST/source/native/html proof, Markdig lazy paragraph continuation, and grouped Markdown writing that preserves the marker form for reparsing. Broader nested-block and edge-continuation breadth still need focused comparison before `UseDefinitionLists` can move to `Covered`.
