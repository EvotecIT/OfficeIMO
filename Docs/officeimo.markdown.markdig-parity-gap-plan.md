...
- [ ] **1. Close `UseAutoLinks` as the next parser slice.** Finish the remaining bare URL, `www`, email, scheme, previous-character, boundary, punctuation, Unicode, table-cell, source/native, and writer breadth. Current work is engine work: Markdig-style punctuation-before-outside-`)`, lowercase `www.` prefix behavior, and lowercase bare scheme prefix behavior are options, but the row stays `Partial` until the wider corpus is covered.
