...
- [ ] **5. Close `UseAbbreviations`.** Existing behavior is real but still `Partial`. Parse-owned Markdown writing preserves abbreviation definitions including empty-title and list-item-contained definitions, and unresolved bracket text, nested link-label, blockquote, list, dash/opening-punctuation boundary, and pipe-table-cell expansion has Markdig comparison plus AST/native proof. The formerly named bracket-text and list-item-definition parser residuals are closed; Unicode HTML entity rendering policy and final source-location/lossless writer breadth remain before promotion.
...
- [ ] **Abbreviation parity is partial, not closed.** `UseAbbreviations` now has opt-in parser, semantic AST, syntax/native metadata, HTML rendering, source-edit, selected Markdig comparison evidence across nested inline/container/table-cell contexts, Markdig-compatible trailing-dash, unresolved-bracket-text, list-item-definition, and opening-punctuation behavior, empty-title definition handling, AST/native propagation proof, and parse-owned definition-preserving Markdown writing. The current named residuals are Unicode HTML entity rendering policy differences and final source-location/lossless writer breadth.
...
- [ ] Stop broad fixture-chasing and work the remaining rows in this order.
  ...
  - [ ] `UseAbbreviations`: parse-owned Markdown writing preserves abbreviation definition syntax including empty-title and list-item-contained definitions, and unresolved bracket text plus nested link-label, blockquote, list, boundary, and pipe-table-cell expansion now has Markdig and AST/native proof; classify Unicode HTML escaping as renderer policy and decide final source-location/lossless writer breadth before promotion.
