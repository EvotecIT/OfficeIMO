...
| Markdig extension inventory | 33 Markdig extension-family rows in `Docs/officeimo.markdown.markdig-extension-inventory.md`: 7 covered, 12 partial, 3 intentional, 11 gap |
...
- [ ] Markdig extension parity is not closed: 7 extension families are `Covered`, 12 are `Partial`, 3 are `Intentional`, and 11 are `Gap`.
...
- [ ] **1. Finish `UseAutoLinks` to a promotion decision.** This is the current active parser slice. Recent engine work added Markdig-compatible trailing punctuation behavior, trailing semicolon retention, trailing quote retention with paired single-quote literal fallback, `www` and URL authority host-underscore rejection, http/www/ftp user-info authority rejection, optional closing-bracket URL consumption, Markdig left-boundary comparison, apostrophe-started bare scheme literal fallback, bare `mailto:` path/query parsing, bare `mailto:` semicolon and address-only colon/dash handling, Unicode href normalization, optional href `~` percent-encoding, and percent-encoded quote href output. An 831-case disposable AutoLinks+PipeTables sweep now has zero mismatches against Markdig for the covered option matrix. Remaining work is deciding whether that matrix is enough to promote `UseAutoLinks`, or naming the residual broader-corpus and renderer-policy limits explicitly.
- [ ] **2. Broaden GFM inventory beyond the smoke corpus.** The current GFM inventory is green but small: 52 tracked fixtures total. Add upstream-compatible breadth for autolinks, strikethrough delimiter edges, tag filtering, tables, task lists, footnotes, and extension interactions before claiming GFM breadth.
- [ ] **3. Split raw HTML, GFM tag filtering, and security policy.** Treat CommonMark raw HTML parsing, cmark-gfm tag filtering, OfficeIMO allow/strip/escape/sanitize modes, URL policy, source metadata, and Markdown writing as separate contracts so security behavior is not counted as parser parity.
- [ ] **4. Close `UseDefinitionLists`.** Existing behavior is real but still `Partial`. Finish marker-group, lazy-continuation, nested-block, loose-definition, source-map, writer, and reparse-stability edges before promotion.
- [ ] **5. Close `UseAbbreviations`.** Existing behavior is real but still `Partial`. Parse-owned Markdown writing preserves abbreviation definitions including empty-title and list-item-contained definitions, and unresolved bracket text, nested link-label, blockquote, list, dash/opening-punctuation boundary, pipe-table-cell expansion, and Unicode visible-text output have Markdig comparison plus AST/native proof where applicable. The formerly named bracket-text, list-item-definition, and Unicode text-rendering residuals are closed; final source-location/lossless writer breadth remains before promotion.
- [ ] **6. Decide the real scope for every `Gap` row before coding it.** Assign `UseCustomContainers`, `UseGridTables`, `UseSmartyPants`, `UseCitations`, `UseEmojiAndSmiley`, `UseFooters`, `UseGlobalization`, `UseJiraLinks`, `UseListExtras`, `UsePragmaLines`, and `UseReferralLinks` to core parser, optional extension, renderer/host policy, deferred, or intentional out-of-scope.
- [ ] **7. Finish or intentionally limit high-value partial Markdig rows.** Work through `UseAlertBlocks`, `UseGenericAttributes`, `UsePreciseSourceLocation`, parser/render extensions, `UseCjkFriendlyEmphasis`, `UseDiagrams`, `UseFigures`, `UseMathematics`, and `UseMediaLinks` with an explicit owner and promotion bar.
- [ ] **8. Finish canonical AST cleanup.** Remove duplicated or adapter-heavy ownership around list items, tables, callouts, definition lists, front matter, and extension nodes so semantic AST, syntax tree, native snapshots, renderer contexts, writer contexts, and source edits share one model.
- [ ] **9. Finish lossless/source architecture.** Complete whitespace, blank-line, tab, delimiter-token, raw-slice, original-to-normalized mapping, generated-node diagnostic, caret/source-edit, and source-preserving roundtrip fallback behavior before claiming editor-grade roundtrip.
- [ ] **10. Finish renderer/writer extension parity.** Custom parser, transform, renderer, and writer APIs must carry source slices and token metadata so extension nodes render and write without downstream string rescanning.
- [ ] **11. Run performance proof last.** After parser, AST/source, renderer/writer, and security contracts stabilize, capture release-mode Markdig comparisons for parse, parse-with-syntax, HTML render, Markdown write, transforms, source edits, allocations, and representative README/docs/chat corpora.
...
- [ ] **Markdig extension-family coverage is far from closed.** The current inventory is 7 `Covered`, 12 `Partial`, 3 `Intentional`, and 11 `Gap`. Every non-covered row needs one decision: implement in core, implement as optional extension, route to renderer/host policy, or mark intentional out of scope.
...
- [ ] **Abbreviation parity is partial, not closed.** `UseAbbreviations` now has opt-in parser, semantic AST, syntax/native metadata, HTML rendering, source-edit, selected Markdig comparison evidence across nested inline/container/table-cell contexts, Markdig-compatible trailing-dash, unresolved-bracket-text, list-item-definition, opening-punctuation behavior, Unicode visible-text rendering, empty-title definition handling, AST/native propagation proof, and parse-owned definition-preserving Markdown writing. The current named residual is final source-location/lossless writer breadth.
...
- [ ] Stop broad fixture-chasing and work the remaining rows in this order.
  - [ ] `UseAutoLinks`: decide promotion after the zero-mismatch 831-case AutoLinks+PipeTables sweep, focused Markdig comparison cases, and source/native/writer proof. Promote only if the parser/render/write/source contracts are judged covered; otherwise leave a named residual broader-corpus or renderer-policy gap.
  - [ ] `UseAbbreviations`: parse-owned Markdown writing preserves abbreviation definition syntax including empty-title and list-item-contained definitions, and unresolved bracket text plus nested link-label, blockquote, list, boundary, pipe-table-cell expansion, and Unicode visible-text output now has Markdig and AST/native proof where applicable; decide final source-location/lossless writer breadth before promotion.
  - [ ] `UseNonAsciiNoEscape`: continue moving visible renderer text paths to `HtmlOptions.EscapeNonAsciiText`; inline text, link display text, code text, captions, simple quote text, abbreviation output, TOC labels/titles, heading helper text, page titles, raw-HTML escape output, image-blocked placeholder text, and portable fallback helper text are covered, while attribute, sanitizer/security-owned escape, and custom renderer-extension output still need explicit policy routing.
