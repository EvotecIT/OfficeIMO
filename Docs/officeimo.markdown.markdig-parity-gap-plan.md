...
- [ ] **Finish `UseGenericAttributes`.** Engine plus AST/source work. Generic attribute storage now exists on semantic and syntax nodes, fenced-code metadata is projected into it, and default fenced-code HTML rendering emits those attributes. Still missing: Markdig-style arbitrary block attributes, inline attributes, Markdown writer preservation, and source-edit coverage.
