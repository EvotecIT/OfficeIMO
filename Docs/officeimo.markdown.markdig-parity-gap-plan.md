...
- [ ] **Definition-list syntax breadth is partial.** OfficeIMO now parses the pinned Markdig colon-marker form, including multiple terms and multiple definitions with grouped AST/source/native/html proof, and grouped Markdown writing preserves the marker form for reparsing. Lazy continuation and nested block breadth still need focused comparison before `UseDefinitionLists` can move to `Covered`.
