...
- [ ] **1. Finish `UseAutoLinks` to a promotion decision.** This is the current active parser slice. Recent engine work added Markdig-compatible trailing punctuation behavior, trailing quote retention with paired single-quote literal fallback, `www` host-underscore rejection, http/www/ftp user-info authority rejection, optional closing-bracket URL consumption, Markdig left-boundary comparison, bare `mailto:` path/query parsing, bare `mailto:` semicolon handling, Unicode href normalization, optional href `~` percent-encoding, and percent-encoded quote href output. Remaining work is a final Markdig/GFM edge sweep plus any renderer-policy deltas. Promote only when the Markdig-compatible and GFM-compatible profiles are both explicit; otherwise leave named intentional deltas.
...
- [ ] Stop broad fixture-chasing and work the remaining rows in this order.
  - [ ] `UseAutoLinks`: finish the remaining renderer-policy, broader edge, writer, source/native, and GFM evidence; promote only if the parser/render/write/source contracts are all covered, otherwise leave a named residual gap.
