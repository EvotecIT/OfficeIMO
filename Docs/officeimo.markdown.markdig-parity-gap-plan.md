...
- [ ] **Finish `UseGenericAttributes`.** Engine plus AST/source work. Generic attribute storage now exists on semantic and syntax nodes, fenced-code metadata is projected into it, default fenced-code HTML rendering emits those attributes, and opt-in trailing attribute blocks now parse/render/write for ATX headings and paragraphs. Still missing: broader arbitrary block-family attributes, inline span attributes, token-level source-edit coverage, and full writer/source breadth.
...
- [x] **Moved `UseGenericAttributes` through opt-in heading/paragraph trailing blocks.** `MarkdownReaderOptions.GenericAttributes` now parses Markdig-style trailing attribute blocks on ATX headings and paragraphs into the shared semantic/syntax attribute storage, renders them to HTML, and writes normalized trailing attribute blocks that reparse stably.
...
- [ ] `UseGenericAttributes`: generic attribute storage now exists on semantic and syntax nodes, with fenced-code metadata projected through ordinary and semantic fenced-block paths and rendered on default fenced HTML; opt-in trailing attribute blocks now parse/render/write for ATX headings and paragraphs. Finish broader block-family coverage, inline span attribute parsing, token-level source-edit coverage, and writer/source breadth before promotion.
