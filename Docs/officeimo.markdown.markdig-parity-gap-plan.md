...
- [ ] **1. Finish `UseAutoLinks` to a promotion decision.** This is the current active parser slice. Recent engine work added Markdig-compatible trailing punctuation behavior, trailing semicolon retention, trailing quote retention with paired single-quote literal fallback, `www` host-underscore rejection, http/www/ftp user-info authority rejection, optional closing-bracket URL consumption, Markdig left-boundary comparison, bare `mailto:` path/query parsing, bare `mailto:` semicolon handling, Unicode href normalization, optional href `~` percent-encoding, and percent-encoded quote href output. Remaining work is a final Markdig/GFM edge sweep plus any renderer-policy deltas. Promote only when the Markdig-compatible and GFM-compatible profiles are both explicit; otherwise leave named intentional deltas.
