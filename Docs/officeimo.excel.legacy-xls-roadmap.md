...
`CrtLayout12`, `ChartFrtInfo`, and `CrtMlFrt` records now decode preserve-only chart-group
...
chart future-record version/range envelopes, and XmlTkChain byte-count envelopes, with compact import-report
...
layout states, future-record range states, XmlTkChain completion states, and pie explosion percentages.
