...
| CSS cascade | `css-cascade` | Css | Full | !important, @supports, author stylesheets, caller stylesheets, custom properties, inheritance, inline styles | Applies the bounded author cascade, selector specificity, inherited values, var() substitution, supported @supports conditions, and caller stylesheets appended after document styles. | None |
| CSS selectors | `css-selectors` | Css | Partial | ::after, ::before, attribute, class, combinators, id, structural pseudo-classes, type | Matches the documented selector subset and generated before/after content; selectors outside the bounded subset do not match. | None |
...
| CSS values | `css-length-units` | Css | Full | %, cm, em, in, mm, pc, pt, px, q, rem | Resolves absolute, font-relative, and percentage lengths against the active layout reference. | None |
| Color and paint | `css-backgrounds` | Css | Partial | background-color, background-image, background-position, background-repeat, background-size, linear-gradient(), radial-gradient() | Paints bounded image layers plus linear and radial gradients. Unsupported layers or repeat modes use diagnosed fallbacks. | `HtmlRenderBackgroundImageRepeatUnsupported`, `HtmlRenderBackgroundImageValueUnsupported` |
...
| Color and paint | `css-color` | Css | Partial | hex colors, hsl(), hsla(), named colors, rgb(), rgba(), transparent | Resolves the listed legacy and modern color forms through the shared Drawing parser. Broader CSS Color forms remain outside the declared subset. | `HtmlRenderBackgroundImageValueUnsupported`, `HtmlRenderBorderPaintValueUnsupported` |
...
| Generated content | `generated-content` | Css | Partial | ::after, ::before, attr(), content, counter(), counter-increment, counter-reset, counter-set, counters() | Generates quoted text, attributes, and supported counters. Unsupported expressions are omitted with diagnostics. | `HtmlRenderGeneratedContentUnsupported`, `HtmlRenderGeneratedCounterUnsupported` |
| Images | `images` | Resource | Partial | img, JPEG, object-fit, object-position, picture, PNG, srcset, SVG, TIFF, WebP | Resolves bounded responsive image candidates and paints supported raster and vector sources. | None |
| Layout | `layout-block-inline` | Css | Full | block flow, box sizing, inline flow, inline-block, line boxes, margins, padding | Builds searchable block and inline layout with box-model sizing and line wrapping. | None |
| Layout | `layout-columns` | Css | Partial | column-count, column-fill, column-gap, column-rule, column-span, column-width, columns | Builds bounded multi-column layout with balancing, rules, and spanning blocks. Advanced cross-page fragmentation remains limited. | `HtmlRenderMultiColumnValueUnsupported` |
| Layout | `layout-flex` | Css | Partial | alignment, display:flex, display:inline-flex, flex, flex-direction, flex-wrap, gap | Lays out supported row and column flex containers, wrapping, gaps, ordering, and alignment. Unsupported values use deterministic fallbacks. | `HtmlRenderFlexLayoutPending`, `HtmlRenderFlexValueUnsupported` |
| Layout | `layout-grid` | Css | Partial | display:grid, display:inline-grid, gap, grid-auto-*, grid-template-*, minmax(), placement, repeat() | Lays out bounded explicit and implicit grids with numeric placement. Subgrid, named-line resolution, and intrinsic track sizing are outside the current contract. | `HtmlRenderGridLayoutPending`, `HtmlRenderGridValueUnsupported` |
...
| Layout | `layout-tables` | Html | Partial | border-collapse, caption, colspan, rowspan, table, table-layout, tbody, tfoot, thead | Builds bounded table grids with spans, collapsed or separate borders, captions, and repeated paged headers and footers. | `HtmlRenderTableValueUnsupported` |
...
| Output accessibility | `pdf-accessibility` | Output | Partial | alternate text, document language, headings, links, lists, reading order, tables, tagged PDF | Maps supported semantic groups into tagged PDF structures. Broader validator-backed conformance remains an explicit release gate. | None |
...
| Paged media | `paged-fragmentation` | PagedMedia | Partial | break-after, break-before, break-inside, orphans, table footer repetition, table header repetition, widows | Honors supported break constraints, text widows/orphans, and repeated table sections. Oversized atomic visuals may require a diagnosed forced fragment. | `HtmlRenderForcedFragment`, `HtmlRenderVisualFragmentUnsupported` |
| Paged media | `paged-page-rules` | PagedMedia | Partial | :first, :left, :right, @page, counter(page), counter(pages), margin, margin boxes, named pages, size | Applies generic page geometry and supported per-page margin content. Pseudo-page geometry that requires body reflow uses the generic geometry. | `HtmlRenderPageMarginContentUnsupported`, `HtmlRenderPagePseudoGeometryPending`, `HtmlRenderPageSelectorPending`, `HtmlRenderPageSizeUnsupported` |
