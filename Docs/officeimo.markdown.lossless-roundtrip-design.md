...
Span-backed native edits can already replace a fenced code block and an inline token while preserving surrounding normalized source, and the roundtrip writer now has first-step original-source preservation for those edits when `PreserveTrivia` is enabled. The parser still does not capture enough trivia, full original/normalized offset mapping, delimiter metadata, or changed-node identity to promise general lossless markdown rewriting.
