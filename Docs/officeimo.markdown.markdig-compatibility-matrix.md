...
| `UseCustomContainers` | `Partial` | Optional/core block extension | First root/nested colon-fence parser slice supports Markdig-style div/class HTML for scoped cases | Source-backed syntax/native opening fence, info, body, closing fence, source slices, snapshots, caret lookup, source edits, and child block ownership exist for scoped cases | Parsed-block Markdown writing and default HTML output exist for scoped cases; renderer override and broader writer seams remain partial | Has focused Markdig comparison plus syntax/native/source-edit proof; needs malformed-fence, list/blockquote, writer, and ownership breadth | Broaden malformed-fence, list/blockquote/container, and writer/override coverage, then decide whether the built-in parser should stay core opt-in or move behind an optional extension package. |
