...
| `UseDefinitionLists` | `Partial` | Core opt-in parser | Partial | Needs remaining source-map breadth | Needs writer/reparse edge breadth | Needs focused edge proof before promotion | Broaden remaining Markdig definition-list parser, source-map, writer, and reparse edge cases before promotion. |
...
| `UsePreciseSourceLocation` | `Partial` | Cross-cutting source architecture | Partial parser spans | Needs lossless trivia and mapping | Needs roundtrip diagnostics | Needs broader source-edit proof | Continue Phase 3 source-map and trivia work before claiming parity. |
