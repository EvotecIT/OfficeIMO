...
| --- | --- | --- | --- | --- | --- |
| opendocument-text-via-word | odt, ott | direct-loss-aware-adapter | `OfficeIMO.OpenDocument.Odt.Pdf/OfficeIMO.OpenDocument.Odt.Pdf.csproj` | `OfficeIMO.OpenDocument.Converters.Tests/OpenDocumentPdfConversionContracts.cs#OdtFacadePreservesProjectionLossAndProducesReadablePdf` | The direct adapter merges ODT-to-Word feature mappings with Word-to-PDF diagnostics in PdfDocumentConversionResult. |
| opendocument-spreadsheet-via-excel | ods, ots | direct-loss-aware-adapter | `OfficeIMO.OpenDocument.Ods.Pdf/OfficeIMO.OpenDocument.Ods.Pdf.csproj` | `OfficeIMO.OpenDocument.Converters.Tests/OpenDocumentPdfConversionContracts.cs#OdsFacadeUsesExcelPdfEngineAndExposesInformationEvidence` | The direct adapter merges ODS-to-Excel feature mappings with Excel-to-PDF diagnostics in PdfDocumentConversionResult. |
| opendocument-presentation-via-powerpoint | odp, otp | direct-loss-aware-adapter | `OfficeIMO.OpenDocument.Odp.Pdf/OfficeIMO.OpenDocument.Odp.Pdf.csproj` | `OfficeIMO.OpenDocument.Converters.Tests/OpenDocumentPdfConversionContracts.cs#OdpFacadeUsesPowerPointPdfEngineAndKeepsAnimationLoss` | The direct adapter merges ODP-to-PowerPoint feature mappings with PowerPoint-to-PDF diagnostics in PdfDocumentConversionResult. |
...
| visio-diagram | vsdx, vsdm, vstx, vstm | direct-loss-aware-adapter | `OfficeIMO.Visio.Pdf/OfficeIMO.Visio.Pdf.csproj` | `OfficeIMO.Visio.Tests/Visio.Pdf.ApiConsistency.cs#LoadedDocumentProjectsDirectlyWithoutReopeningOrChangingItsDestination` | The document-shaped adapter reuses the shared Visio reader and PDF projection owners, emits searchable diagram semantics, and retains explicit preview-versus-semantic fallback evidence. It does not claim native vector-page equivalence when the Reader projection cannot preserve it. |
