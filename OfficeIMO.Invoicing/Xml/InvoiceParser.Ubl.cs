...
public static partial class InvoiceParser {
    private static Invoice ReadUbl(XElement root, InvoiceXmlReadContext c) {
        ...
        foreach (XElement note in c.Children(root, Cbc + "Note")) {
            ...
            if (InvoiceNote.HasEncodedSubject(text))
                c.AddTo(invoice.Notes, new InvoiceNote(text.Substring(5), text.Substring(1, 3)));
            else c.AddTo(invoice.Notes, new InvoiceNote(text));
        }
        ...
        foreach (XElement billing in c.Children(root, Cac + "BillingReference")) {
            ...
            c.AddTo(invoice.PrecedingInvoices, new InvoiceReference(c.Required(reference, Cbc + "ID"), c.Date(c.Child(reference, Cbc + "IssueDate"))));
        }
        ...
        if (payee != null) {
            ...
            foreach (XElement identifier in c.Children(payee, Cac + "PartyIdentification")) c.AddTo(invoice.Payee.Identifiers, c.Identifier(c.Child(identifier, Cbc + "ID")) ?? new InvoiceIdentifier(string.Empty));
        }
        ...
        foreach (XElement line in c.Children(root, Cac + (credit ? "CreditNoteLine" : "InvoiceLine"))) {
            ...
            c.AddTo(invoice.Lines, UblLine(c, line, invoice.Currency, credit));
        }
    ...
    private static void UblAdditionalReference(InvoiceXmlReadContext c, XElement reference, Invoice invoice) {
        ...
        c.AddTo(invoice.SupportingDocuments, new InvoiceSupportingDocument { Reference = c.Value(identifier) ?? string.Empty, Description = c.Text(reference, Cbc + "DocumentDescription"),
    ...
    private static void UblTotals(InvoiceXmlReadContext c, XElement root, Invoice invoice) {
        ...
        foreach (XElement tax in c.Children(root, Cac + "TaxTotal")) {
            ...
            foreach (XElement subtotal in c.Children(tax, Cac + "TaxSubtotal")) {
                ...
                c.AddTo(invoice.DeclaredTaxes, new InvoiceDeclaredTax { Category = UblTaxCategory(c, c.Child(subtotal, Cac + "TaxCategory")),
