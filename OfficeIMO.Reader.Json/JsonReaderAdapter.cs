...
/// <summary>
/// JSON ingestion helpers for <see cref="OfficeDocumentReader"/>.
/// </summary>
internal static class JsonReaderAdapter {
    /// <summary>
    /// Reads JSON content from a path with AST-aware chunking.
    /// </summary>
    public static IEnumerable<ReaderChunk> Read(string path, ReaderOptions? readerOptions = null, JsonReadOptions? jsonOptions = null, CancellationToken cancellationToken = default) {
        ...
        foreach (var chunk in Read(fs, source, effectiveReaderOptions, jsonOptions, cancellationToken)) {
            yield return chunk;
        }
    }
    ...
    /// </summary>
    public static IEnumerable<ReaderChunk> Read(Stream stream, string? sourceName = null, ReaderOptions? readerOptions = null, JsonReadOptions? jsonOptions = null, CancellationToken cancellationToken = default) {
        ...
        return Read(stream, BuildSourceMetadataFromLogicalStream(stream, sourcePath, effectiveReaderOptions, cancellationToken), effectiveReaderOptions, options, cancellationToken);
    }

    private static IEnumerable<ReaderChunk> Read(Stream stream, SourceMetadata source, ReaderOptions effectiveReaderOptions, JsonReadOptions? jsonOptions, CancellationToken cancellationToken) {
        var options = Normalize(jsonOptions);
