...
public static partial class DocumentReaderOpenDocumentExtensions {
    private static IEnumerable<ReaderChunk> ReadSpreadsheet(OdsDocument document, string sourceName, ReaderOptions options,
        ...
        if (!string.IsNullOrWhiteSpace(options.ExcelSheetName)) {
            selected = selected.Where(sheet => string.Equals(sheet.Name, options.ExcelSheetName, StringComparison.Ordinal));
        }
        foreach (OdsSheet sheet in selected) {
            ...
            string[] columns = Enumerable.Range(0, columnCount).Select(index => {
                string value = options.ExcelHeadersInFirstRow ? sheet.GetValue(headerRow, range.FirstColumn + index).ToString() : string.Empty;
                return value.Length == 0 ? "Column " + (index + 1).ToString(CultureInfo.InvariantCulture) : value;
            }).ToArray();
            long dataStart = checked(range.FirstRow + (options.ExcelHeadersInFirstRow ? 1 : 0));
