...
namespace OfficeIMO.Word.Html.Converters {
    ...
    internal partial class HtmlToWordConverter {
        ...
        internal HtmlToWordConverter(IImageDownloader? imageDownloader = null) {
            _imageDownloader = imageDownloader ?? new HttpClientImageDownloader();
        }
            ...
            if (node is IElement element) {
                ...
                switch (element.TagName.ToLowerInvariant()) {
                    ...
                    case "figure": {
                            ...
                            if (img != null) {
                                ProcessImageAsync(img, doc, options, currentParagraph, headerFooter, _cancellationToken).GetAwaiter().GetResult();
                            }
                    ...
                    case "img": {
                        ProcessImageAsync((IHtmlImageElement)element, doc, options, currentParagraph, headerFooter, _cancellationToken).GetAwaiter().GetResult();
                            break;
                        }
                    case "style": {
        ...
        private void ParseCss(string css, string? key = null) {
            ...
            foreach (var rule in rules) {
                _cssRules.Add(rule);
                var style = CssStyleMapper.MapParagraphStyle(rule.Style.CssText);
                if (style.HasValue) {
                    foreach (var cls in ExtractClassSelectors(rule.SelectorText)) {
                        _cssClassStyles[cls] = style.Value;
                    }
                }
            }
        }

        private static IEnumerable<string> ExtractClassSelectors(string selectorText) {
            foreach (var part in selectorText.Split(',')) {
                ...
                while (idx >= 0) {
                    int start = idx + 1;
                    int end = start;
                    while (end < s.Length && (char.IsLetterOrDigit(s[end]) || s[end] == '-' || s[end] == '_')) end++;
                    if (end > start) yield return s.Substring(start, end - start);
                    idx = s.IndexOf('.', end);
                }
            }
        }
