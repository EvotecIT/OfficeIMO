...
public sealed class HtmlRenderShape : HtmlRenderVisual {
    internal HtmlRenderShape(OfficeShape shape, double x, double y, int paintOrder, string? linkUri = null, string? source = null)
        : base(HtmlRenderVisualKind.Shape, x, y, shape?.Width ?? 0D, shape?.Height ?? 0D, paintOrder, linkUri, source) {
    ...
    internal override HtmlRenderVisual Translate(double offsetX, double offsetY, int paintOrder) =>
        new HtmlRenderShape(Shape, X + offsetX, Y + offsetY, paintOrder, LinkUri, Source);
}
