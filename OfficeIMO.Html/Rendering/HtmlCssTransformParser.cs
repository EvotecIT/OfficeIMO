...
internal static class HtmlCssTransformParser {
    ...
    internal static bool TryParse(
        ...
        if (!TryParseFunctionList(value, boxWidth, boxHeight, fontSize, rootFontSize, viewportWidth, viewportHeight, out OfficeTransform functions, out detail)) return false;
        if (!TryResolveOrigin(transformOriginValue, boxWidth, boxHeight, fontSize, rootFontSize, viewportWidth, viewportHeight, out double originX, out double originY)) {
    ...
    internal static bool IsSupportedTransformSyntax(string value) =>
        TryParseFunctionList(value.Trim().ToLowerInvariant(), 100D, 100D, 16D, 16D, 100D, 100D, out _, out _);

    internal static bool IsSupportedOriginSyntax(string value) =>
        TryResolveOrigin(value, 100D, 100D, 16D, 16D, 100D, 100D, out _, out _);
    ...
    private static bool TryParseFunctionList(
        ...
        while (index < value.Length) {
            ...
            if (!TryParseFunction(name, arguments, width, height, fontSize, rootFontSize, viewportWidth, viewportHeight, out OfficeTransform function)) {
        ...
        switch (name) {
            ...
            case "translate":
                ...
                if (values.Count < 1 || values.Count > 2
                    || !TryLength(values[0], width, fontSize, rootFontSize, viewportWidth, viewportHeight, out double translateX)
                    || values.Count == 2 && !TryLength(values[1], height, fontSize, rootFontSize, viewportWidth, viewportHeight, out translateY)) return false;
                transform = OfficeTransform.Translate(translateX, values.Count == 2 ? translateY : 0D);
                return true;
            case "translatex":
                if (values.Count != 1 || !TryLength(values[0], width, fontSize, rootFontSize, viewportWidth, viewportHeight, out double x)) return false;
                transform = OfficeTransform.Translate(x, 0D);
                return true;
            case "translatey":
                if (values.Count != 1 || !TryLength(values[0], height, fontSize, rootFontSize, viewportWidth, viewportHeight, out double y)) return false;
    ...
    private static bool TryResolveOrigin(
        ...
        return TryOriginAxis(first, width, fontSize, rootFontSize, viewportWidth, viewportHeight, horizontal: true, out x)
            && TryOriginAxis(second, height, fontSize, rootFontSize, viewportWidth, viewportHeight, horizontal: false, out y);
    }

    private static bool TryOriginAxis(string value, double reference, double fontSize, double rootFontSize, double viewportWidth, double viewportHeight, bool horizontal, out double result) {
        result = reference / 2D;
        ...
        return TryLength(value, reference, fontSize, rootFontSize, viewportWidth, viewportHeight, out result);
    }
