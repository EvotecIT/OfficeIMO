...
public static partial class HtmlRenderCapabilityCatalog {
    ...
    private static readonly HtmlCapabilityEvidencePin[] StaticScreenEvidence = DocumentEvidence.Concat(new[] {
        ...
        Evidence(HtmlCapabilityEvidenceIds.H4ScreenAdvancedHeldOut, "OfficeIMO H4 advanced held-out screen rendering selection", H4AdvancedHeldOutAcceptanceRevision, HtmlCapabilityEvidenceRole.Qualification, "screen-full-page-v1 and screen-snapshot-paged-v1 passed the declared per-case source-marker, Chromium geometry, pixel, fixed-page, and profile-consistency criteria on Windows, Linux, and macOS; bounded classifications and exclusions remain selected evidence rather than general browser equivalence", required: 8, passed: 8, failed: 0, excluded: 0, untested: 0, caseIds: H4AdvancedHeldOutCaseIds(), selections: CreateH4AdvancedHeldOutScreenSelections())
    }).ToArray();
    ...
    private static readonly HtmlCapabilityEvidencePin[] PagedEvidence = DocumentEvidence.Concat(new[] {
        ...
        Evidence(HtmlCapabilityEvidenceIds.H4PagedAdvancedHeldOut, "OfficeIMO H4 advanced held-out paged rendering selection", H4AdvancedHeldOutAcceptanceRevision, HtmlCapabilityEvidenceRole.Qualification, "print-paged-v1 passed the declared per-case source-marker, page, text, and pixel criteria on Windows, Linux, and macOS; accepted bleed, text-extraction, and managed-layout differences remain explicitly bounded", required: 8, passed: 8, failed: 0, excluded: 0, untested: 0, caseIds: H4AdvancedHeldOutCaseIds(), selections: CreateH4AdvancedHeldOutPagedSelections())
    }).ToArray();
