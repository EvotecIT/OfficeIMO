...
public static class HtmlEditableLayoutCapabilityContracts {
    private static readonly IReadOnlyList<HtmlEditableLayoutCapabilityContract> Contracts = Array.AsReadOnly(new[] {
        ...
        new HtmlEditableLayoutCapabilityContract(
            ...
            "semantic-rich, raw-comment-bearing, bookmark-target, rounded, authored or inherited text-alignment/typography, nested-placement, multi-child flex/grid, clipped or scrolling, root/descendant border, and region or descendant paint-effect regions stay in flow; explicit section/article containers remain as semantic slide owners while their bounded content projects; collision bounds include only successfully imported native shapes, while collision simplification, every CSS shadow parameter approximation, additional shadow layers, and unsupported image/effect types are diagnosed")
    });
