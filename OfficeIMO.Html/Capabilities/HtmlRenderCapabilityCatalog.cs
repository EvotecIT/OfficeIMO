...
public static class HtmlRenderCapabilityCatalog {
    private static readonly IReadOnlyList<HtmlRenderCapability> Capabilities = new[] {
        ...
        Full("css-borders-effects", "Color and paint", HtmlRenderCapabilityKind.Css,
            Features("border", "solid/dashed/dotted/double/groove/ridge/inset/outset", "collapsed table border conflict resolution", "border-radius", "outline", "box-shadow", "opacity", "transform", "clip-path basic shapes", "clip-path geometry boxes"),
            "Paints the declared border styles, including two-tone three-dimensional edges and collapsed-table conflict winners, radius, outline, shadow, opacity, two-dimensional affine transform, and basic-shape clip-path forms against margin, border, padding, or content reference boxes."),
        ...
        Full("generated-content", "Generated content", HtmlRenderCapabilityKind.Css,
            Features("content", "attr()", "counter()", "counters()", "symbols()", "quotes", "open-quote", "close-quote", "no-open-quote", "no-close-quote", "@counter-style", "counter-reset", "counter-set", "counter-increment", "::before", "::after", "::marker", "counter(list-item)"),
            "Generates quoted text, authored nested quote pairs, attributes, standard counters, functional counter styles, and document-scoped named cyclic, fixed, numeric, alphabetic, symbolic, or additive counter styles with bounded range, pad, negative, prefix, suffix, and fallback handling."),
