...
public static class HtmlRenderCapabilityCatalog {
    private static readonly IReadOnlyList<HtmlRenderCapability> Capabilities = new[] {
        ...
        Full("web-fonts", "Fonts", HtmlRenderCapabilityKind.Resource,
            Features("@font-face", "font-family", "font-style", "font-weight", "unicode-range", "TrueType glyf", "WOFF 1", "single-face WOFF 2 on .NET 8+", "CFF/CFF2", "variable-font instances", "fallback packs"),
            "Loads bounded policy-approved font programs, selects constrained faces by Unicode scalar range, treats variation selectors and join controls as shaping inputs, and routes logical text through measurement, complete layout, rasterization, and accessible PDF outlines. OfficeIMO owns TrueType glyf, WOFF 1, CFF/CFF2, and variable-font instances directly; single-face WOFF 2 decoding is built in on .NET 8 and newer. Every outline program uses the bounded contract with cancellation, per-run character limits, and an operation-wide path-command budget."),
        Fallback("web-fonts-fallback", "Fonts", HtmlRenderCapabilityKind.Resource,
            Features("WOFF 2 on earlier target frameworks", "unsupported font containers or outline programs", "invalid font programs", "unavailable font sources"),
