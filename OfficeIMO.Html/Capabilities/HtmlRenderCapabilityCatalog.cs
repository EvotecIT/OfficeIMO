...
public static class HtmlRenderCapabilityCatalog {
    private static readonly IReadOnlyList<HtmlRenderCapability> Capabilities = new[] {
        ...
        Full("paged-page-rules", "Paged media", HtmlRenderCapabilityKind.PagedMedia,
            Features("@page", "size", "margin", ":first", ":left", ":right", "named pages", "page-local viewport units", "top-level block reflow", "inline continuation reflow", "margin boxes", "counter(page)", "counter(pages)"),
            "Applies generic and named page masters, resolves geometry and viewport units per page, reflows top-level blocks and supported text continuations when masters change, and emits running margin content and page counters."),
        Fallback("paged-page-rules-fallback", "Paged media", HtmlRenderCapabilityKind.PagedMedia,
            ...
            "Retains source-page line breaking for a continuation that cannot be reconstructed from logical source progress, or omits unsupported margin content, with a stable diagnostic while preserving document flow.",
