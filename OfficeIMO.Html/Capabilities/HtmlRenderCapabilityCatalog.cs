...
public static partial class HtmlRenderCapabilityCatalog {
    ...
    private static readonly IReadOnlyList<HtmlRenderCapability> Capabilities = new[] {
        ...
        Qualified("css-length-units", "CSS values", HtmlRenderCapabilityKind.Css,
            ...
            "Parses the listed units into an OfficeIMO-owned typed expression and resolves absolute, font-relative, percentage, static viewport-family, and bounded container-query lengths against explicit layout references. Writing-mode inference and font-metric-relative unit families remain outside this subset."),
        Qualified("css-length-math", "CSS values", HtmlRenderCapabilityKind.Css,
            CssScope(HtmlCapabilityStage.CascadeAndCompute | HtmlCapabilityStage.Layout, HtmlCapabilitySpecificationIds.CssValues),
            Features("calc()", "min()", "max()", "clamp()"),
            "Retains a bounded provider-independent expression tree through specified and computed-style inspection, then resolves percentages and relative units at used-value time with dimensional checks and explicit missing-context outcomes."),
