...
public static class HtmlRenderCapabilityCatalog {
    private static readonly IReadOnlyList<HtmlRenderCapability> Capabilities = new[] {
        ...
        Full("svg", "SVG", HtmlRenderCapabilityKind.Resource,
            Features("paths", "basic shapes", "groups", "symbols", "use", "named/hex/rgb/hsl paint", "linearGradient", "radialGradient", "pattern fill", "context-fill/context-stroke", "marker-start", "marker-mid", "marker-end", "clipPath", "mask", "mix-blend-mode", "feDropShadow", "feGaussianBlur", "feOffset", "text", "gradient/pattern text paint", "text stroke", "logical ActualText", "tspan", "textPath", "text-anchor", "dominant-baseline", "baseline-shift", "writing-mode", "text-orientation", "affine transforms"),
            "Maps the listed bounded SVG subset, reusable symbols, gradients, object-bounding-box or user-space vector pattern fills, reusable start/middle/end marker scenes, local clips and masks, static drop-shadow/blur/offset filters, positioned searchable text with baseline control, referenced path placement, mixed-orientation vertical glyph layout, transforms, and standard blend modes into the shared Drawing scene. Blur is a bounded viewport-clipped vector approximation. Gradient-, pattern-, or stroke-painted glyphs remain vector outlines while one logical ActualText value is retained for SVG accessibility and PDF extraction."),
        ...
        Fallback("svg-fallback", "SVG", HtmlRenderCapabilityKind.Resource,
            Features("dynamic or multi-input SVG filter graphs", "external references", "unsupported pattern strokes", "unsupported paint servers"),
