...
public static class HtmlRenderCapabilityCatalog {
    private static readonly IReadOnlyList<HtmlRenderCapability> Capabilities = new[] {
        ...
        Full("layout-grid", "Layout", HtmlRenderCapabilityKind.Css,
            Features("display:grid", "display:inline-grid", "grid-template-*", "grid-auto-*", "repeat()", "minmax() with fixed minima", "auto item minima", "gap", "numeric and named placement"),
            "Lays out bounded explicit and implicit grids with automatic item minima, responsive repeats, fixed minimum tracks, numeric placement, named areas, and named lines."),
        Fallback("layout-grid-fallback", "Layout", HtmlRenderCapabilityKind.Css,
            Features("min-content", "max-content", "intrinsic minmax() minima", "fractional automatic minima exceeding allocated shares", "subgrid", "unsupported track functions", "unsupported auto-flow values"),
        ...
        Full("paged-page-rules", "Paged media", HtmlRenderCapabilityKind.PagedMedia,
            ...
            "Applies generic and named page masters, supported selectors, page geometry, running margin content, and page counters."),
        Fallback("paged-page-rules-fallback", "Paged media", HtmlRenderCapabilityKind.PagedMedia,
            Features("unsupported page selectors", "pseudo-page geometry requiring incompatible body reflow", "unsupported margin content"),
            "Uses diagnosed generic page geometry or omits unsupported margin content while preserving document flow.",
        ...
        Full("svg", "SVG", HtmlRenderCapabilityKind.Resource,
            ...
            "Maps the listed bounded SVG subset, reusable symbols, gradients, local clips and masks, text, transforms, and standard blend modes into the shared Drawing scene."),
