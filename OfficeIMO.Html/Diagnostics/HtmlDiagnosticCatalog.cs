...
public static class HtmlDiagnosticCatalog {
    private static readonly IReadOnlyDictionary<string, HtmlDiagnosticDefinition> Definitions = new Dictionary<string, HtmlDiagnosticDefinition>(StringComparer.OrdinalIgnoreCase) {
        ...
        [HtmlRenderDiagnosticCodes.ComplexTextShapingUnsupported] = RenderDefinition(HtmlRenderDiagnosticCodes.ComplexTextShapingUnsupported, "Typography", HtmlDiagnosticSeverity.Warning, "A joining script used scalar glyphs because contextual shaping is not active yet.", "Use a shaping provider for PDF output or avoid relying on contextual joining until the shared managed shaping stage is enabled."),
