...
public static class HtmlDiagnosticCatalog {
    private static readonly IReadOnlyDictionary<string, HtmlDiagnosticDefinition> Definitions = new Dictionary<string, HtmlDiagnosticDefinition>(StringComparer.OrdinalIgnoreCase) {
        ...
        [HtmlRenderDiagnosticCodes.StylesheetNestedResourcesPending] = RenderDefinition(HtmlRenderDiagnosticCodes.StylesheetNestedResourcesPending, "CssFidelity", HtmlDiagnosticSeverity.Warning, "An external stylesheet contains imports or URL resources that are not recursively loaded yet.", "Inline those nested resources or provide a flattened stylesheet until recursive loading is enabled."),
