...
public static class HtmlDiagnosticCatalog {
    private static readonly IReadOnlyDictionary<string, HtmlDiagnosticDefinition> Definitions = new Dictionary<string, HtmlDiagnosticDefinition>(StringComparer.OrdinalIgnoreCase) {
        ...
        [HtmlRenderDiagnosticCodes.FlexLayoutPending] = RenderDefinition(HtmlRenderDiagnosticCodes.FlexLayoutPending, "LayoutFidelity", HtmlDiagnosticSeverity.Warning, "A flex formatting case currently falls back to normal flow.", "Use a block-level, single-line row flex container with element children, or avoid wrapping, inline-flex, direct text items, positioned items, and auto margins until those cases are enabled."),
        ...
        [HtmlRenderDiagnosticCodes.PositionInsetUnsupported] = RenderDefinition(HtmlRenderDiagnosticCodes.PositionInsetUnsupported, "LayoutFidelity", HtmlDiagnosticSeverity.Warning, "A relative-position inset could not be resolved and used zero offset.", "Use an absolute CSS length, or a horizontal percentage; vertical percentages require an explicitly sized containing block."),
        [HtmlRenderDiagnosticCodes.PositioningModeUnsupported] = RenderDefinition(HtmlRenderDiagnosticCodes.PositioningModeUnsupported, "LayoutFidelity", HtmlDiagnosticSeverity.Warning, "A CSS positioning mode currently falls back to normal flow.", "Use static or relative positioning until absolute, fixed, and sticky formatting contexts are enabled."),
