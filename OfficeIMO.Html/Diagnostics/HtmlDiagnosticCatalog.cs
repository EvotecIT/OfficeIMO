...
public static class HtmlDiagnosticCatalog {
    private static readonly IReadOnlyDictionary<string, HtmlDiagnosticDefinition> Definitions = new Dictionary<string, HtmlDiagnosticDefinition>(StringComparer.OrdinalIgnoreCase) {
        ...
        [HtmlConversionDiagnosticCodes.CssNestingDepthLimitExceeded] = ConversionDefinition(HtmlConversionDiagnosticCodes.CssNestingDepthLimitExceeded, "Safety", HtmlDiagnosticSeverity.Error, "Nested CSS rule blocks exceeded the shared depth budget.", "Flatten nested media, supports, or rule blocks, or raise MaxCssNestingDepth only for trusted input."),
        ...
        [HtmlRenderDiagnosticCodes.RunningStringLimitExceeded] = RenderDefinition(HtmlRenderDiagnosticCodes.RunningStringLimitExceeded, "Safety", HtmlDiagnosticSeverity.Warning, "A paged running-string value exceeded its configured character budget and was omitted.", "Reduce string-set content or raise MaxRunningStringCharacters only for trusted input."),
        ...
        [HtmlRenderDiagnosticCodes.FontFormatUnsupported] = RenderDefinition(HtmlRenderDiagnosticCodes.FontFormatUnsupported, "Typography", HtmlDiagnosticSeverity.Warning, "A font source is not a supported TrueType glyf-outline font.", "Provide a TTF, TrueType-flavored OpenType, or WOFF 1 face; WOFF 2 transformed tables and CFF outlines require pre-conversion."),
        ...
        [HtmlRenderDiagnosticCodes.CounterRepresentationLimitExceeded] = RenderDefinition(HtmlRenderDiagnosticCodes.CounterRepresentationLimitExceeded, "Safety", HtmlDiagnosticSeverity.Warning, "A generated CSS counter representation exceeded its bounded output budget and used a decimal fallback.", "Use a bounded ordinal, a compact counter system, or raise the authored counter range only when its representation remains small."),
        ...
        [HtmlRenderDiagnosticCodes.FormFieldTransformStaticFallback] = RenderDefinition(HtmlRenderDiagnosticCodes.FormFieldTransformStaticFallback, "Interactivity", HtmlDiagnosticSeverity.Warning, "A form control inside a transformed or translucent paint group used faithful static rendering instead of an interactive PDF widget.", "Remove the ancestor transform or opacity when PDF form interactivity is required, or keep the effect when visual fidelity is more important."),
        [HtmlRenderDiagnosticCodes.FormFieldZeroMaximumLengthStaticFallback] = RenderDefinition(HtmlRenderDiagnosticCodes.FormFieldZeroMaximumLengthStaticFallback, "Interactivity", HtmlDiagnosticSeverity.Warning, "A text control with maxlength=0 used faithful static rendering instead of an unrestricted interactive PDF widget.", "Use a positive maxlength when an interactive PDF field is required, or keep maxlength=0 when the zero-length HTML contract matters."),
        [HtmlRenderDiagnosticCodes.RadioDuplicateValueStaticFallback] = RenderDefinition(HtmlRenderDiagnosticCodes.RadioDuplicateValueStaticFallback, "Interactivity", HtmlDiagnosticSeverity.Warning, "A radio group with duplicate submitted values used faithful static rendering instead of synthetic PDF option values.", "Give each radio option a distinct value when an interactive PDF field is required, or keep duplicate HTML values when visual fidelity is more important."),
        [HtmlRenderDiagnosticCodes.ChoiceDuplicateSelectedValueStaticFallback] = RenderDefinition(HtmlRenderDiagnosticCodes.ChoiceDuplicateSelectedValueStaticFallback, "Interactivity", HtmlDiagnosticSeverity.Warning, "A multi-select with duplicate selected submitted values used faithful static rendering instead of an ambiguous PDF choice selection.", "Give selected options distinct values when an interactive PDF field is required, or keep duplicate HTML values when preserving the visible selection is more important."),
        [HtmlRenderDiagnosticCodes.RadioMixedDisabledStateStaticFallback] = RenderDefinition(HtmlRenderDiagnosticCodes.RadioMixedDisabledStateStaticFallback, "Interactivity", HtmlDiagnosticSeverity.Warning, "A radio group mixing enabled and disabled options used faithful static rendering instead of an interactive PDF group that changes disabled semantics.", "Use a consistently enabled or disabled radio group when an interactive PDF field is required, or keep mixed state when visual fidelity is more important."),
        [HtmlRenderDiagnosticCodes.ChoiceDisabledOptionStaticFallback] = RenderDefinition(HtmlRenderDiagnosticCodes.ChoiceDisabledOptionStaticFallback, "Interactivity", HtmlDiagnosticSeverity.Warning, "A select containing disabled options used faithful static rendering instead of an interactive PDF choice field that would make every option selectable.", "Remove disabled options when an interactive PDF choice field is required, or keep them when preserving HTML choice restrictions is more important."),
        [HtmlRenderDiagnosticCodes.FormFieldRepeatedNameStaticFallback] = RenderDefinition(HtmlRenderDiagnosticCodes.FormFieldRepeatedNameStaticFallback, "Interactivity", HtmlDiagnosticSeverity.Warning, "Multiple HTML controls sharing one submitted name used faithful static rendering instead of distinct PDF fields that would rename repeated form-data entries.", "Use unique names when interactive PDF fields are required, or keep repeated names when preserving HTML submission semantics is more important."),
        [HtmlRenderDiagnosticCodes.ChoiceBlankLabelStaticFallback] = RenderDefinition(HtmlRenderDiagnosticCodes.ChoiceBlankLabelStaticFallback, "Interactivity", HtmlDiagnosticSeverity.Warning, "A select containing a blank option label used faithful static rendering instead of an interactive PDF choice field that would invent visible text.", "Use non-empty option labels when an interactive PDF choice field is required, or keep the blank label when preserving the authored choice UI is more important."),
        ...
        [HtmlRenderDiagnosticCodes.PagePseudoGeometryPending] = RenderDefinition(HtmlRenderDiagnosticCodes.PagePseudoGeometryPending, "PagedMedia", HtmlDiagnosticSeverity.Warning, "A fragment crossing into a page with different geometry could not be reconstructed from logical source progress.", "Avoid changing page geometry across this complex continuation, or split the content at an explicit page boundary."),
        ...
        [HtmlRenderDiagnosticCodes.MathMlContentUnsupported] = RenderDefinition(HtmlRenderDiagnosticCodes.MathMlContentUnsupported, "LayoutFidelity", HtmlDiagnosticSeverity.Warning, "MathML content could not be represented by the bounded shared mathematical expression model.", "Use supported Presentation MathML rows, tokens, fractions, roots, scripts, limits, fences, matrices, enclosures, and annotations."),
        ...
        [HtmlRenderDiagnosticCodes.SvgRasterFallback] = RenderDefinition(HtmlRenderDiagnosticCodes.SvgRasterFallback, "ImageFidelity", HtmlDiagnosticSeverity.Info, "A caller-supplied image codec rasterized SVG features outside the bounded shared vector scene.", "Keep the codec configured when visual fidelity matters more than searchable vector content, or simplify the SVG to the supported vector subset."),
