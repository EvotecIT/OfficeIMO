...
public static class HtmlDiagnosticCatalog {
    private static readonly IReadOnlyDictionary<string, HtmlDiagnosticDefinition> Definitions = new Dictionary<string, HtmlDiagnosticDefinition>(StringComparer.OrdinalIgnoreCase) {
        ...
        [HtmlRenderDiagnosticCodes.BidiLayoutUnsupported] = RenderDefinition(HtmlRenderDiagnosticCodes.BidiLayoutUnsupported, "Typography", HtmlDiagnosticSeverity.Warning, "Right-to-left inline content used logical source order because managed bidi positioning is not active yet.", "Use left-to-right content for pixel-accurate output until the shared managed bidi layout stage is enabled."),
