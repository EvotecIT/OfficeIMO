...
namespace OfficeIMO.Examples.Word {
    internal static partial class Charts {
        ...
        public static void Example_Charts_PaletteAndSizing(string folderPath, bool openWord) {
            ...
            bar.AddBar("EMEA", new List<int> { 10, 13, 16, 18 }, OfficeIMO.Drawing.OfficeColor.Black); // initial color is overridden by palette
            bar.AddBar("APAC", new List<int> { 9, 12, 14, 20 }, OfficeIMO.Drawing.OfficeColor.Black);
            bar.AddBar("AMER", new List<int> { 8, 15, 17, 19 }, OfficeIMO.Drawing.OfficeColor.Black);
            ...
            line.AddLine("Throughput", new List<int> { 100, 140, 180, 220 }, OfficeIMO.Drawing.OfficeColor.Black);
            line.AddLine("Latency", new List<int> { 80, 60, 70, 50 }, OfficeIMO.Drawing.OfficeColor.Black);
            line.ApplyPalette(WordChart.WordChartPalette.MonochromeGray)
                .SetSeriesColor(1, OfficeIMO.Drawing.OfficeColor.ParseHex("#d63939")) // emphasize Latency (series index 1)
                .SetWidthToPageContent(1.0, 320);
            ...
            area.AddArea("CPU", new List<int> { 30, 45, 55, 35 }, OfficeIMO.Drawing.OfficeColor.Black);
            area.AddArea("Memory", new List<int> { 40, 50, 60, 70 }, OfficeIMO.Drawing.OfficeColor.Black);
            area.ApplyPalette(WordChart.WordChartPalette.Soft, semanticOutcomes: false)
                .SetWidthToPageContent(1.0, 320);

            // Save/Open
            try { doc.Save(filePath); } catch { doc.Save(); }
            if (openWord) doc.OpenInApplication(filePath);
        }
    }
}
