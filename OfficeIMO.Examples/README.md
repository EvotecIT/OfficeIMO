...
- Run `dotnet run --project OfficeIMO.Examples --framework net8.0 -- --visio-showcase` to generate a curated Visio set from basic fluent shapes through advanced flowchart, block, swimlane, sequence, network, architecture, editing, data-driven inventory, identity, Kubernetes/service-mesh, application dependency, incident/runbook sequence, network segmentation, and quality-gallery diagrams.
- Add `--visio-native-preview` to export dependency-free OfficeIMO-native `.svg` and `.png` previews beside the generated VSDX files.
