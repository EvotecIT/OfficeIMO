...
- Turn off recovery storage in Settings before editing private documents to prevent new edit snapshots. This waits for active writes and applies to the running Studio instance and future launches; close other running instances separately. Editing, undo, redo, and saving remain available, but new unsaved edits cannot be recovered after an unexpected exit. Existing copies remain available until explicitly cleared or expired. This switch does not disable recent documents, saved reading positions, restart sessions, or the recovery copy required for a direct provider workflow write. The workflow confirmation explains that requirement before writing.
...
- Review comment threads in Comment mode, filter by author or completion state, and jump to the next unresolved thread. Selecting a thread reveals its page annotation. Reply, resolve, or reopen through undoable PDF edits; saved replies and review states remain standard PDF metadata. Reply drafts stay with their thread during annotation edits, page reordering, undo, and redo, and survive cancelled operations. If an edit removes the original comment or cannot report its identity, the draft remains available to copy or restore to a chosen thread. Drafts are kept for the current session. Accepted, rejected, and other existing review states remain visible until explicitly changed.
...
- Inspect protection and signatures; protect or decrypt a copy; apply certificate signatures, validate signatures, add Bates numbering, sanitize, repair, optimize, and perform verified redaction. The document-health workbench accepts provider-backed PDFs for inspection, repair planning, comparison, optimization, repair, and sanitization. Inspection and repair planning need no output folder; operations that produce a file accept filesystem or selected provider destinations and recheck inputs before publication.
- Convert supported document formats, export PDF pages to supported image formats, preview print sheets, and assemble PDFs from ordered PDFs, images, Office files, folders, and ZIP archives. Conversion, page-image export, and assembly accept provider-backed files through bounded private staging, retain their original references, and check provider contents again before publication. Assembly also accepts provider ZIPs. Print preview reads a provider once and uses the same bounded document snapshot for its plan and page images. Provider outputs require explicit confirmation for direct writes and local recovery. Studio retains a verified local copy before that write and verifies the destination afterward. An uncertain save appears as **Check output** in Jobs with a recovery copy; it is never retried automatically. Selected provider folders support recursive assembly input and conversion, health, and page-image outputs. Files are published and verified individually. If a later file fails or the operation is cancelled, earlier verified files remain available and Jobs shows any retained recovery copies. Newly created files retain the actual locations assigned by their provider.
...
- Open **Jobs** to follow conversion, document-health, assembly, page-export, and searchable-PDF attempts across document tabs. These workflows share two execution slots; conversion files run sequentially within their batch. Cancel a waiting or running attempt from its card, or cancel its conversion batch. Completed PDF outputs open in Studio; other files and folders use the system association. The history holds up to 500 attempts, retaining active entries and recovery records while discarding the oldest other finished entries when full. **Clear finished** removes records without deleting output files or recovery copies. Ordinary history is kept for the current session. Workflow recovery records return after restarting Studio, but execution never resumes automatically. Open a verified recovery PDF and use Save As to keep it; other recovery formats use the system association. When an attempt has multiple copies, select the copy to open or discard. Discarding requires confirmation for that selected copy. These copies do not expire automatically and are separate from edit snapshots in Settings. Their store admits at most 100 records and defaults to a 1 GiB limit; a full store prevents further provider workflow writes until copies are discarded.
- Create searchable PDFs through `OfficeIMO.Workflows` and `OfficeIMO.Pdf.Ocr`. The workbench supports page ranges, 28 typed language choices, confidence and rendering controls, checksum-verified language-data provisioning, cancellation, and safe output conflict policy. It accepts provider-backed inputs and requires an explicit destination when no local output can be suggested. Choose output selects a local file or provider destination. Provider writes require confirmation and retain a verified recovery copy if publication is uncertain. The runner rechecks source contents and open-document ownership after recognition before publishing the validated PDF.
...
- `OfficeIMO.Pdf.Ocr` owns searchable text generation. `OfficeIMO.Workflows` owns source snapshots, output validation, and publication; Studio configures the optional Tesseract CLI provider. `OfficeIMO.Reader.Ocr` owns OCR enrichment for normalized Reader documents.
