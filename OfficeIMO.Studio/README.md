...
- Inspect protection and signatures; protect or decrypt a copy; apply certificate signatures, validate signatures, add Bates numbering, sanitize, repair, optimize, and perform verified redaction. The document-health workbench accepts provider-backed PDFs for inspection, repair planning, comparison, optimization, repair, and sanitization. Inspection and repair planning need no output folder; operations that produce a file require a filesystem destination and recheck provider inputs before publication.
- Convert supported document formats, export PDF pages to supported image formats, preview print sheets, and assemble PDFs from ordered PDFs, images, Office files, folders, and ZIP archives. Conversion, page-image export, and assembly accept provider-backed files through bounded private staging, retain their original references, and check provider contents again before publication. Assembly also accepts provider ZIPs. Print preview reads a provider once and uses the same bounded document snapshot for its plan and page images. PDF assembly can write to a selected provider after explicit confirmation. Studio retains a verified local copy before that write and verifies the destination afterward. An uncertain save appears as **Check output** in Jobs with a recovery copy; it is never retried automatically. Conversion and page-image output destinations and folder intake currently require filesystem locations.
