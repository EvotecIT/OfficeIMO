...
- Review comment threads in Comment mode, filter by author or completion state, and jump to the next unresolved thread. Selecting a thread reveals its page annotation. Reply, resolve, or reopen through undoable PDF edits; saved replies and review states remain standard PDF metadata. Reply drafts stay with their thread during the current session and survive cancelled operations. Accepted, rejected, and other existing review states remain visible until explicitly changed.
