...
- Save checks that the source still has the physical identity and contents Studio opened. If another application changes, replaces, or removes it, Studio keeps the pending edits and refuses to overwrite the external version; use **Save As** to choose a different file. The shared commit path also checks the displaced source during atomic replacement. Saving a source opened through a symlink updates its target and preserves the link.
