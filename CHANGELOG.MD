...
* Standardize document persistence, conversion results, diagnostics, themes, and async I/O across the solution. Compatibility aliases and fake in-memory async methods were removed; see the [OfficeIMO migration guide](MIGRATION.md#migrating-from-officeimo-3x-to-40).
* Unify the 4.0 PDF bridge surface around `PdfDocument` and `PdfLogicalDocument`, with format-owned save/import options, immutable result reports, and consistent `To...`, `To...Result`, `SaveAs...`, and `SaveAs...Async` entry points. See the [3.x to 4.0 migration section](MIGRATION.md#migrating-from-officeimo-3x-to-40).
