...
- `PdfCatalogSplitScalingBenchmarks`: OfficeIMO splits every page of 5-, 6-, 50-, and 500-page documents with no page labels, one numbering range, or a distinct range for every page. Setup reopens every output and verifies its single page, unique content marker, and source page label. This OfficeIMO-only lane exposes catalog-feature costs that the standard/dense comparison matrix does not exercise.
