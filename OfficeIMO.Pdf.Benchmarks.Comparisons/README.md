...
- `PdfUnderstandingBenchmarks`: OfficeIMO-only route-health evidence over deterministic multi-page PDFs containing running headers/footers, a spanning heading, indented two-column body regions, a list item, caption, and table. `AdvancedUnderstanding` measures open, glyph decoding, word/line grouping, recursive XY-cut segmentation/order, and semantic classification. `LogicalStructureAndTables` separately measures the current logical model and actual table-cell extraction. They are different contracts and are not presented as competing implementations or a ratio comparison.
