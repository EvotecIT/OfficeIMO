...
- `PdfCatalogSplitScalingBenchmarks`: OfficeIMO splits every page of 5-, 6-, 50-, and 500-page documents with no catalog features, one page-label range, a distinct range for every page, or a bookmark on every page. Setup reopens every output and verifies its single page, unique content marker, and applicable page label or named destination. This OfficeIMO-only lane exposes catalog-feature costs that the standard/dense comparison matrix does not exercise.
...
dotnet run --project OfficeIMO.Pdf.Benchmarks.Comparisons/OfficeIMO.Pdf.Benchmarks.Comparisons.csproj -c Release -f net10.0 -- --filter "*PdfCatalogSplitScalingBenchmarks*" --job Short
```
