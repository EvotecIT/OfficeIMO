...
- `PdfStructuredReadBenchmarks`: OfficeIMO-only route-health evidence for the one canonical `PdfDocument.Load(...).Read(...)` contract. Both `FastLoadAndRead` and `StructuredLoadAndRead` include source snapshotting, parsing, glyph recovery, word/line grouping, recursive XY-cut reading order, semantic classification, logical projection, and table extraction; `Structured` additionally applies document-wide evidence. Setup gates the same labelled output contract before either profile is timed, so the profiles are measured as stage-depth choices rather than separate semantic engines. Its labelled Easy fixture uses two pages, rather than the general one-page Easy scenario, because running-header/footer recovery requires repeated document evidence.
