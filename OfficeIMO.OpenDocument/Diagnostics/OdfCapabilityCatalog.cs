...
public static class OdfCapabilityCatalog {
    private static readonly IReadOnlyList<OdfCapability> Capabilities = new[] {
        ...
        new OdfCapability("conditional-style-maps", "Conditional style maps", OdfCapabilityLevel.Limited,
            "Native style maps on common and automatic styles can be authored, edited, and preserved in order. Applied styles must be common named styles in the same family. OfficeIMO renderers do not evaluate map conditions; data-style and embedded-object maps are preservation-only."),
