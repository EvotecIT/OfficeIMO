...
internal sealed class ReaderMemorySampler : IDisposable {
    ...
    internal ReaderMemorySampler(Process process) {
        _process = process;
        ...
        _thread = new Thread(SampleUntilStopped) {
            IsBackground = true,
            Name = "OfficeIMO.Reader memory sampler"
        };
        _thread.Start();
    }

    internal ReaderMemoryPeak Stop() {
        if (Interlocked.Exchange(ref _stopped, 1) == 0) {
            _stop.Set();
            _thread.Join();
            RecordCurrentMemory();
        }
        return new ReaderMemoryPeak(
