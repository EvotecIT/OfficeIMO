...
internal static class OfficeJpeg2000Header {
    internal static bool TryGetOpaqueComponents(byte[] bytes, out int components) {
        components = 0;
        if (TryReadCodestream(bytes, 0, bytes.Length, out components)) return true;
        ...
        while (offset < bytes.Length) {
            ...
            if (type == 0x6A703268) { // jp2h
                if (header || !TryReadHeader(bytes, start, end, out expected)) return false;
                header = true;
            } else if (type == 0x6A703263) { // jp2c
                if (codestream || !TryReadCodestream(bytes, start, end, out components)) return false;
                codestream = true;
        ...
        return header && codestream && expected == components;
    }

    private static bool TryReadHeader(byte[] bytes, int offset, int end, out int components) {
        components = 0;
    ...
    private static bool TryReadCodestream(byte[] bytes, int start, int end, out int components) {
        components = 0;
        ...
        return components is 1 or 3 && length == 38 + components * 3 && length <= end - start - 4;
    }
