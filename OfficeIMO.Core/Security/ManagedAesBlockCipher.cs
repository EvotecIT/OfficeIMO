...
namespace OfficeIMO.Security {
    // Adapted from Bouncy Castle C# AesLightEngine (release-2.7.0).
    // See THIRD-PARTY-NOTICES.md for source and license details.
    internal sealed class ManagedAesBlockCipher : IDisposable {
        private const int BlockSize = 16;
        private readonly bool _forEncryption;
        private readonly uint[][] _workingKey;
        private int _rounds;

        internal ManagedAesBlockCipher(byte[] key, bool forEncryption) {
            if (key == null) {
            ...
            try {
                _workingKey = GenerateWorkingKey(keyCopy, forEncryption);
            } finally {
                Array.Clear(keyCopy, 0, keyCopy.Length);
            }
        }

        internal void ProcessBlock(byte[] input, int inputOffset, byte[] output, int outputOffset) {
            ValidateBlock(input, inputOffset, nameof(input));
            ValidateBlock(output, outputOffset, nameof(output));

            if (_forEncryption) {
                EncryptBlock(input, inputOffset, output, outputOffset, _workingKey);
            } else {
                DecryptBlock(input, inputOffset, output, outputOffset, _workingKey);
            }
        }

        public void Dispose() {
            for (int index = 0; index < _workingKey.Length; index++) {
                Array.Clear(_workingKey[index], 0, _workingKey[index].Length);
            }
        }

        private static void ValidateBlock(byte[] buffer, int offset, string parameterName) {
            if (buffer == null) {
                throw new ArgumentNullException(parameterName);
            }

            if (offset < 0 || offset > buffer.Length - BlockSize) {
                throw new ArgumentOutOfRangeException(parameterName, "A complete 16-byte AES block is required.");
            }
        }

        private static uint ReadUInt32LittleEndian(byte[] input, int offset) {
            return input[offset]
        ...
        private static void WriteUInt32LittleEndian(uint value, byte[] output, int offset) {
        ...
        */
        private uint[][] GenerateWorkingKey(byte[] key, bool forEncryption)
        {
            ...
            uint[][] W = new uint[_rounds + 1][]; // 4 words in a block
            ...
            switch (KC)
            {
                case 4:
                {
                    uint t0 = ReadUInt32LittleEndian(key, 0); W[0][0] = t0;
                    uint t1 = ReadUInt32LittleEndian(key, 4); W[0][1] = t1;
                    uint t2 = ReadUInt32LittleEndian(key, 8); W[0][2] = t2;
                    uint t3 = ReadUInt32LittleEndian(key, 12); W[0][3] = t3;
                ...
                case 6:
                {
                    ...
                    uint t2 = ReadUInt32LittleEndian(key, 8); W[0][2] = t2;
                    uint t3 = ReadUInt32LittleEndian(key, 12); W[0][3] = t3;
                    uint t4 = ReadUInt32LittleEndian(key, 16); W[1][0] = t4;
                    uint t5 = ReadUInt32LittleEndian(key, 20); W[1][1] = t5;
                ...
                case 8:
                {
                    ...
                    uint t4 = ReadUInt32LittleEndian(key, 16); W[1][0] = t4;
                    uint t5 = ReadUInt32LittleEndian(key, 20); W[1][1] = t5;
                    uint t6 = ReadUInt32LittleEndian(key, 24); W[1][2] = t6;
                    uint t7 = ReadUInt32LittleEndian(key, 28); W[1][3] = t7;
        ...
        {
            uint C0 = ReadUInt32LittleEndian(input, inOff + 0);
            uint C1 = ReadUInt32LittleEndian(input, inOff + 4);
            uint C2 = ReadUInt32LittleEndian(input, inOff + 8);
            uint C3 = ReadUInt32LittleEndian(input, inOff + 12);
            ...
            WriteUInt32LittleEndian(C0, output, outOff + 0);
            WriteUInt32LittleEndian(C1, output, outOff + 4);
            WriteUInt32LittleEndian(C2, output, outOff + 8);
            WriteUInt32LittleEndian(C3, output, outOff + 12);
        }
