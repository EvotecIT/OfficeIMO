...
public sealed partial class ProjectView {
    ...
    private double MeasureTextHeight(string text, double size, double width, bool bold = false) {
        var measurer = OfficeTextMeasurer.Create(new OfficeFontInfo("Arial", size));
        var layout = OfficeTextLayoutEngine.LayoutTextBlock(text, size, width, Layout.PageHeight, 1.3, size,
            (value, fontSize) => measurer.MeasureWidth(value, measurer.CreateStyle(new OfficeFontInfo("Arial", fontSize, bold ? OfficeFontStyle.Bold : OfficeFontStyle.Regular))), true);
    ...
    private double MeasureHeaderHeight(double width) {
        ...
        var widths = ColumnWidths(width); var measurer = OfficeTextMeasurer.Create(new OfficeFontInfo("Arial", 10, OfficeFontStyle.Bold));
        double height = 26;
        for (int c = 0; c < Columns.Count; c++) {
            var text = OfficeTextLayoutEngine.LayoutTextBlock(ColumnTitle(Columns[c]), 10, Math.Max(1, widths[c] - 12), Layout.PageHeight - 2 * Layout.Margin - 160,
                1.3, 10, (value, size) => measurer.MeasureWidth(value, measurer.CreateStyle(new OfficeFontInfo("Arial", size, OfficeFontStyle.Bold))), true);
    ...
    private double[] MeasureRows(double labelWidth, double availableHeight, CancellationToken token) {
        var measurer = OfficeTextMeasurer.Create(new OfficeFontInfo("Arial", 11));
        ...
        for (int r = 0; r < Rows.Count; r++) {
            ...
            for (int c = 0; c < (timeline ? 1 : Columns.Count); c++) {
                ...
                var text = OfficeTextLayoutEngine.LayoutTextBlock(value, 11, Math.Max(1, (timeline ? labelWidth : widths[c]) - 12), availableHeight - (timeline ? 40 : 20),
                    1.3, 11, (textValue, size) => measurer.MeasureWidth(textValue, measurer.CreateStyle(new OfficeFontInfo("Arial", size, row.IsSummary ? OfficeFontStyle.Bold : OfficeFontStyle.Regular))), true);
