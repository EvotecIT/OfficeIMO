...
| Cost | Work/material/cost resources, fixed and variable material use, dated rate-table selection, overtime rates, per-use costs, and start/end/prorated accrual | Recalculated actual costs are opt-in. An unknown calculated amount cannot replace an entered assignment, task, summary, or resource amount; incomplete coverage blocks application. Cost-resource application-import limitation still applies |
| Baselines and earned value | Capture slots 0–10, baseline work/cost curves, status-clipped PV/EV/AC, variance, CPI/SPI, and CPI-based forecast | Elapsed curves use wall-clock time; working curves use their effective calendar. Missing actual costs, inconsistent totals, and partial summary curves with mixed time bases produce diagnostics and nullable results |
