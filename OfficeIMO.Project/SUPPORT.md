...
XML and native task duration, actual duration, and remaining duration share one unit, elapsed flag, and estimated flag. Save assessment rejects conflicting formats. XML derives the format from the available progress durations when total duration is absent. Applying assignment calculations expresses all three durations in the calculated task format.

Assignment analysis reports total, actual, and remaining resource-cost caches independently of assignment aggregates. A known mismatch produces a diagnostic without changing either observation. Earned-value analysis accepts entered actual costs at or after the project status date when recorded actual dates do not extend beyond the requested boundary; earlier historical queries still require dated cost evidence.
...
Material actual and remaining consumption must agree with the quantity declared by assignment units and rate scale. Cost-resource calculations reconcile entered total, actual, and remaining amounts, including actual-cost curves; contradictory amounts and work quantities on cost resources block calculation. A finish-only completed milestone stays at its actual finish. A nonzero task actual duration requires an actual start or dated actual work. Duration components and assignment actual dates must agree with their work intervals. Fixed-duration calculation derives missing actual duration from the union of work-assignment actual intervals before planning the remainder.

Actual-work and overtime curves must reconcile with their stored totals. Every nonzero actual-overtime interval requires actual-work coverage, including across calendar breaks. A stopped work or material assignment requires explicit nonzero actual work before calculation; zero or absent actual work cannot establish its completed interval. Recorded task actual duration beyond the duration represented by assignment actuals anchors remaining work after that progress boundary. Concurrent and discontinuous assignment intervals retain their timing through calculation, application, and XML round trips. An earlier resume date cannot shorten the completed period of an unassigned task.
...
Calendar arithmetic uses local wall time, inherited exceptions, dated week overrides, split/overnight shifts, bounded searches, and cancellation. Work-week overrides require both date bounds for calculation and native output; XML preserves omitted bounds with a validation warning. Derived calendars require an existing parent. `CalculateAssignments` enables independent task/resource-calendar intersections, resource availability periods, and assignment intervals. The date-only profile still requires compatible effective calendars. Recurring calendar exceptions remain unqualified.

Timephased XML records require a UID. Optional source fields remain optional when saving XML, but scheduling requires an explicit record type. Earned-value analysis reports unknown totals when an untyped record could affect its inputs. Work and cost calculations validate the dates and lexical values of the curve types they consume.
...
Assignment-enabled application refreshes resource cost, actual cost, and remaining cost together, including zero totals after the final assignment is removed. Retained actual-cost curves must fit the assignment span and recorded actual dates; explicit actual-cost recalculation can replace outdated curves. Resource remaining cost is mapped in XML, MPP8/9/12/14, and MPX4. Explicitly clearing an unresolved project, task, resource, or base-calendar reference advances the document revision and removes that retained reference on save.
...
| Cost | Work/material/cost resources, fixed and variable material use, dated rate-table selection, overtime rates, per-use costs, and start/end/prorated accrual | Recalculated actual costs are opt-in, including summary totals. An unknown calculated amount cannot replace an entered assignment, task, summary, or resource amount; incomplete coverage blocks application. Started cost assignments require actual amounts, actual-cost curves, or total and remaining amounts. Cost-resource application-import limitation still applies |
| Baselines and earned value | Capture slots 0–10, baseline work/cost curves, status-clipped PV/EV/AC, variance, CPI/SPI, and CPI-based forecast | Capture rejects totals that its prepared curves cannot represent, including independent task actual-cost adjustments, before replacing any baseline. Elapsed curves use wall-clock time; working curves use their effective calendar. Missing actual costs, inconsistent totals, and partial summary curves with mixed time bases produce diagnostics and nullable results |
...
| Table projection | Explicit mapped task/resource/assignment/calendar tables with stable identities and invariant units | Default export rejects omitted semantics; explicit lossy projection returns diagnostics. Import preserves nameless entities and validates mappings, references, duplicates, units, and limits |
...
| Lifecycle | Exact unchanged bytes; canonical edited output; task/resource/assignment creation, deletion, hierarchy edits, notes, calendars, and links | Seven authored and seven independently produced lifecycle cases have independent reader comparisons. UID-zero project summaries are emitted first; a different source position requires explicit loss permission |
| Mapped fields | Core task/resource dates, work/cost/progress, constraints, priority, links, default baseline, and supported scalar custom fields | Task Text1–10, resource Text1–5, Number1–5, Flag1–10, Cost1–3, Duration1–3, and task Start/Finish1–5; other fields require explicit omission permission. An empty default baseline record has no MPX representation |
| Calendars | Base/resource calendars, weekly intervals, date exceptions, and bounded work-week projection | At most 250 base calendars, 250 exceptions per calendar, and three intervals per day. Assessment reports calendar identity changes, absent-kind defaults, missing base-weekday materialization, and omitted labels/structure |
