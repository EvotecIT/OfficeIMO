...
Assigned resources require an explicit type before scheduling. Resource rates and per-use charges must be nonnegative. Work-week overrides and exceptions at the same calendar precedence cannot overlap inclusive dates; a child calendar can override its parent. Manual assignment calculations start from the stored task start and must fit inside the stored task span.
