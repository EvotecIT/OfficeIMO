...
internal static class AsciiDocEvidenceRunner {
    ...
    internal static int RunEvidence(string[] args, bool verifyBudgets) {
        try {
            ...
            string[] scales = string.IsNullOrWhiteSpace(scaleFilter)
                ? AsciiDocBenchmarkCorpus.Scales.ToArray()
                : [AsciiDocBenchmarkCorpus.Get(scaleFilter!).Scale];
            AsciiDocEvidenceBudgetManifest? manifest = verifyBudgets ? LoadBudgetManifest() : null;
            var measurements = new List<AsciiDocEvidenceMeasurement>();
            var failures = new List<string>();
            foreach (string scale in scales) {
                AsciiDocBenchmarkValidation.Validate(AsciiDocBenchmarkCorpus.Get(scale));
                foreach (string operation in operations) {
                    for (int iteration = 1; iteration <= repeat; iteration++) {
                        AsciiDocEvidenceMeasurement measurement = RunChildProbe(operation, scale) with { Iteration = iteration };
            ...
            var report = new AsciiDocEvidenceReport(
    ...
    private static AsciiDocEvidenceMeasurement Measure(string operation, string scale) {
        AsciiDocBenchmarkFixture fixture = AsciiDocBenchmarkCorpus.Get(scale);
        AsciiDocBenchmarkValidation.Validate(fixture);
        ...
        GC.KeepAlive(result);
        return new AsciiDocEvidenceMeasurement(
            operation,
            fixture.Scale,
            1,
    ...
    private static void ValidateMeasuredResult(string operation, AsciiDocBenchmarkFixture fixture, object result) {
        if (result is AsciiDocParseResult parsed) {
            if (parsed.HasErrors || !parsed.IsLossless
                || parsed.Document.BlocksOfType<AsciiDocHeading>().Count() != fixture.SectionCount + 1
                || parsed.Document.BlocksOfType<AsciiDocTableBlock>().Count() != fixture.SectionCount) {
    ...
    private static AsciiDocEvidenceMeasurement RunChildProbe(string operation, string scale) {
        ...
        using Process child = Process.Start(startInfo) ?? throw new InvalidOperationException("Unable to start AsciiDoc evidence probe.");
        ...
        return JsonSerializer.Deserialize<AsciiDocEvidenceMeasurement>(output, JsonOptions)
            ?? throw new InvalidOperationException($"Probe {operation}/{scale} returned no measurement.");
    }

    private static AsciiDocEvidenceBudgetManifest LoadBudgetManifest() {
        string path = Path.Combine(AppContext.BaseDirectory, "asciidoc-performance-budgets.json");
        return JsonSerializer.Deserialize<AsciiDocEvidenceBudgetManifest>(File.ReadAllText(path), JsonOptions)
            ?? throw new InvalidOperationException("AsciiDoc performance budget manifest is invalid.");
    }

    private static void EvaluateBudget(
        AsciiDocEvidenceBudgetManifest manifest,
        AsciiDocEvidenceMeasurement measurement,
        ICollection<string> failures) {
        AsciiDocEvidenceBudget? budget = manifest.Budgets.FirstOrDefault(item =>
    ...
    private static string SelectOperation(string value) => Operations.FirstOrDefault(
        ...
        ?? throw new ArgumentException("Unknown AsciiDoc evidence operation: " + value);
...
internal sealed record AsciiDocEvidenceMeasurement(
    string Operation,
    string Scale,
    int Iteration,
    long InputBytes,
...
internal sealed record AsciiDocEvidenceReport(
    ...
    int ProcessorCount,
    int Repeat,
    IReadOnlyList<AsciiDocEvidenceMeasurement> Measurements,
    IReadOnlyList<string> Failures);

internal sealed class AsciiDocEvidenceBudgetManifest {
    public int Version { get; set; }
    public string Description { get; set; } = string.Empty;
    public List<AsciiDocEvidenceBudget> Budgets { get; set; } = new();
}

internal sealed class AsciiDocEvidenceBudget {
...
internal sealed class AsciiDocManagedHeapSampler : IDisposable {
    private readonly Thread _thread;
    private readonly ManualResetEventSlim _stop = new(false);
    private long _peakBytes = GC.GetTotalMemory(forceFullCollection: false);
    private int _stopped;

    internal AsciiDocManagedHeapSampler() {
        _thread = new Thread(SampleUntilStopped) { IsBackground = true, Name = "OfficeIMO.AsciiDoc heap sampler" };
        _thread.Start();
    }
