...
This opt-in project compares equivalent image work without adding third-party dependencies to the OfficeIMO product or normal solution. The lanes cover PNG and photographic JPEG decode into a managed RGBA byte array, lossless PNG encode from one identical RGBA buffer, 2048x1619-to-800x632 linear/bilinear resize, static lossless VP8L WebP, and classic TIFF LZW. TIFF and WebP interchange is checked in both directions before timing; the TIFF validator also selects an independently written second page.
...
./Build/Run-LibraryComparisonBenchmarks.ps1 -RunMode full -Workload imagetifflzw -AffinityMask 0xFFFF
./Build/Run-LibraryComparisonBenchmarks.ps1 -RunMode full -Workload imagewebplossless -AffinityMask 0xFFFF
```
