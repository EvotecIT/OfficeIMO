        ...
        private static readonly IReadOnlyList<LegacyPptCapability> CapabilityRows =
            new ReadOnlyCollection<LegacyPptCapability>(new[] {
                ...
                new LegacyPptCapability(LegacyPptFeature.Layouts, "Design",
                    ...
                    "All 14 SlideLayoutType values and their eight-slot placeholder signatures decode to typed metadata and distinct native Open XML layouts under the referenced master. Supported PPTX layouts write native binary hints for fresh and incrementally appended slides. Layout placeholder geometry and supported decorative shapes are materialized into each owning binary slide when not overridden, preserving the visible layout despite the classic format having no independent PPTX-style layout persist object; unsupported layout objects are loss-blocked. Imported relationship or layout edits and richer layout styling remain planned."),
                new LegacyPptCapability(LegacyPptFeature.Themes, "Design",
                    ...
                    LegacyPptCapabilityState.Native,
                    "PowerPoint 2007+ RoundTripTheme12Atom OPC packages and RoundTripColorMapping12Atom XML decode to a typed full-theme model and project to native theme or theme-override parts on main, title, notes, handout, slide, layout, and notes-page scopes. Fresh PPT/POT/PPS writing emits deterministic full DrawingML theme packages and color mappings, preserving all twelve colors, font schemes, fill/line/effect schemes, object defaults, and theme overrides. The classic eight-color scheme remains the PowerPoint 97-2003 fallback. Unmodified imported theme records are byte-preserved; imported main-, notes-, and handout-master theme edits append preservation-aware master persist objects while retaining unrelated source records and synchronizing changed classic fallback colors."),
                new LegacyPptCapability(LegacyPptFeature.ColorSchemes, "Design",
                    ...
                    "All eight classic scheme colors decode and map consistently between binary background/text/shadow/title/fill/accent slots and Open XML Light1/Dark1/Accent4/Dark2/Light2/Accent1/Accent2/Accent3. Fresh PPT/POT/PPS writing emits a distinct native classic scheme for every referenced master, and no-op imported saves preserve original scheme records. The DrawingML round-trip theme extension preserves PPTX Accent5, Accent6, hyperlink, and followed-hyperlink colors exactly while retaining the classic eight-color fallback. Imported main-master theme color edits append synchronized native classic and DrawingML scheme records; direct title-, notes-, handout-, slide-, and notes-page scheme editing remains planned."),
                ...
                new LegacyPptCapability(LegacyPptFeature.Backgrounds, "Design",
                    ...
                    LegacyPptCapabilityState.Preserved,
                    LegacyPptCapabilityState.Planned,
                    "OfficeArt background shapes are distinguished from normal drawing content. Solid, no-fill, inherited, multi-stop linear/path gradient, texture, and picture backgrounds project to native Open XML fills on slides and all master types. Fresh PPT/POT/PPS writing now emits explicit solid, no-fill, and opaque multi-stop linear or scaled-gradient backgrounds on slides, main masters, notes masters, and handout masters; layout backgrounds are materialized onto their slides when the classic format has no separate layout persist object. Gradient focus/path/stop opacity, image and pattern authoring, exact texture semantics, and imported background edits remain planned or explicitly loss-blocked."),
