        ...
        private static readonly IReadOnlyList<LegacyPptCapability> CapabilityRows =
            new ReadOnlyCollection<LegacyPptCapability>(new[] {
                ...
                Planned(LegacyPptFeature.Masters, "Design", "Main, title, notes, and handout masters.",
                    "Main and title master records, supported shapes, and base title/body/other text styles through five levels are projected into native master/layout parts. Fresh binary writing emits the referenced Open XML slide-master topology, exact supported main-master text/rectangle/ellipse/line drawings, master-specific placeholder kinds, and matching OfficeArt drawing-cluster counts; unsupported master objects, rich shape styling, interactions, transitions, and timing are loss-blocked. The embedded writer scaffold currently bounds fresh authoring to eleven masters. DocumentAtom notes and handout master references, schemes, shapes, placeholders, and connector rules project into native notesMaster and handoutMaster parts. Notes/handout drawing authoring, later-version text-style extensions, and imported binary master editing remain planned."),
