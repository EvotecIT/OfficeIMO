        ...
        private static readonly IReadOnlyList<LegacyPptCapability> CapabilityRows =
            new ReadOnlyCollection<LegacyPptCapability>(new[] {
                ...
                new LegacyPptCapability(LegacyPptFeature.Layouts, "Design",
                    ...
                    "All 14 SlideLayoutType values and their eight-slot placeholder signatures decode to typed metadata and distinct native Open XML layouts under the referenced master. Supported PPTX layouts write native binary hints for fresh and incrementally appended slides. Layout placeholder geometry, supported decorative shapes, backgrounds, and theme overrides are materialized into each owning binary slide when not overridden, preserving the visible layout despite the classic format having no independent PPTX-style layout persist object; imported layout background or theme edits likewise append one updated persist object per affected slide. Slide, title-layout, and notes-page master-shape visibility maps to schema-valid root attributes and native inheritance flags, including preservation-aware imported edits. Unsupported layout objects are loss-blocked. Exact layouts projected from independent binary title-master persist objects support preservation-aware shape, theme, color-scheme, and background edits. Imported relationship changes, ordinary layout shape or placeholder-contract edits, and richer layout styling remain planned."),
                ...
                new LegacyPptCapability(LegacyPptFeature.Backgrounds, "Design",
                    ...
                    LegacyPptCapabilityState.Preserved,
                    LegacyPptCapabilityState.Planned,
                    "OfficeArt background shapes are distinguished from normal drawing content. Solid, no-fill, inherited, multi-stop linear/path gradient, texture, and picture backgrounds project to native Open XML fills on slides and all master types. Fresh PPT/POT/PPS writing emits explicit solid, no-fill, multi-stop linear, scaled, centered-path, and shape-path gradients on slides, notes pages, main masters, notes masters, and handout masters; the classic foreground/background opacity ramp preserves linearly interpolated gradient-stop opacity. Layout backgrounds are materialized onto their slides when the classic format has no separate layout persist object. Supported imported slide, notes-page, main-, title-, notes-, and handout-master background edits append preservation-aware drawing records, while ordinary PPTX-layout background edits materialize into every affected imported slide. Gradient focus, custom path rectangles, non-linear stop opacity, image and pattern authoring, and exact texture semantics remain planned or explicitly loss-blocked."),
                ...
                new LegacyPptCapability(LegacyPptFeature.EmbeddedOle,
                    ...
                    LegacyPptCapabilityState.Preserved,
                    LegacyPptCapabilityState.Native,
                    "ExOleEmbedContainer metadata, ExObjRefAtom shape references, and compressed or uncompressed ExOleObjStg records decode to an editable PowerPointOleObject with exact compound-storage bytes. Fresh PPT/POT/PPS writing emits native embedded-object metadata, storage, and shape references. Imported geometry, ProgID, icon/content view, color-follow, and storage replacement use preservation-aware incremental records while untouched metadata, preview pictures, compound streams, and compressed source records remain exact. Malformed storage, unsupported related parts, explicit new preview images, linked OLE, and ActiveX controls are loss-blocked rather than silently discarded."),
