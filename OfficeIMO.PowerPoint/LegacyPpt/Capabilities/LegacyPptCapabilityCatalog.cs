        ...
        private static readonly IReadOnlyList<LegacyPptCapability> CapabilityRows =
            new ReadOnlyCollection<LegacyPptCapability>(new[] {
                ...
                Planned(LegacyPptFeature.Masters, "Design", "Main, title, notes, and handout masters.",
                    "Main and title master records, supported shapes, and base title/body/other text styles through five levels are projected into native master/layout parts. Fresh binary writing emits the referenced Open XML slide-master topology, exact supported main-master text/rectangle/ellipse/line drawings, master-specific placeholder kinds, base title/body/other text-style atoms through five levels, referenced font-collection entries, and matching OfficeArt drawing-cluster counts; unsupported master objects, rich shape styling, interactions, transitions, timing, and later-version style extensions are loss-blocked. The embedded writer scaffold currently bounds fresh authoring to eleven masters. DocumentAtom notes and handout master references, schemes, shapes, placeholders, and connector rules project into native notesMaster and handoutMaster parts. Notes/handout drawing authoring and imported binary master editing remain planned."),
                ...
                new LegacyPptCapability(LegacyPptFeature.ColorSchemes, "Design",
                    ...
                    "All eight classic scheme colors decode and map consistently between binary background/text/shadow/title/fill/accent slots and Open XML Light1/Dark1/Accent4/Dark2/Light2/Accent1/Accent2/Accent3. Fresh PPT/POT/PPS writing emits a distinct native scheme for every referenced master, and no-op imported saves preserve original scheme records. PPTX themes have twelve color slots, so Accent5, Accent6, hyperlink, and followed-hyperlink colors require the later-version theme round-trip extension for exact conversion; scheme edits on imported binary masters remain planned."),
                new LegacyPptCapability(LegacyPptFeature.Placeholders, "Design",
                    ...
                    LegacyPptCapabilityState.Native,
                    "PlaceholderAtom position, all legacy kinds, full/half/quarter size, and vertical orientation project to native placeholder index/type/size/orientation on masters, generated layouts, and slides. Fresh main-master drawings and fresh or incrementally appended binary slides encode the corresponding master/slide placeholder kinds and fields; edits to imported placeholder contracts are loss-blocked. Object payloads are tracked by their separate capability rows."),
