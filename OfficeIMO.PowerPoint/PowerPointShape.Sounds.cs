...
namespace OfficeIMO.PowerPoint {
    public abstract partial class PowerPointShape {
        /// <summary>Gets the sound name played when this shape is clicked.</summary>
        public string? ClickSoundName => GetInteractionSound(mouseOver: false)?
            .Name?.Value;

        /// <summary>Gets the sound name played when the pointer enters this shape.</summary>
        public string? MouseOverSoundName => GetInteractionSound(mouseOver: true)?
            .Name?.Value;

        /// <summary>Sets an embedded WAV or AIFF sound played when this shape is clicked.</summary>
        ...
        /// <summary>Sets an embedded WAV or AIFF sound played when the pointer enters this shape.</summary>
        public void SetMouseOverSound(Stream audio, string name,
            string contentType = "audio/wav", string extension = ".wav") =>
            SetInteractionSound(mouseOver: true, audio, name, contentType,
                extension);

        /// <summary>Controls whether clicking this shape stops the currently playing sound.</summary>
        public void SetClickStopsSound(bool value) =>
            SetInteractionStopsSound(mouseOver: false, value);

        /// <summary>Controls whether entering this shape stops the currently playing sound.</summary>
        public void SetMouseOverStopsSound(bool value) =>
            SetInteractionStopsSound(mouseOver: true, value);

        /// <summary>Removes the sound played when this shape is clicked.</summary>
        public void ClearClickSound() => ClearInteractionSound(mouseOver: false);

        /// <summary>Removes the sound played when the pointer enters this shape.</summary>
        ...
        private void SetInteractionSound(bool mouseOver, Stream audio,
            ...
            if (OwnerSlide == null) {
                throw new InvalidOperationException(
                    "Action sounds require a shape attached to a slide.");
            }
        ...
        private byte[]? GetInteractionSoundBytes(bool mouseOver) {
            ...
            string? relationshipId = GetInteractionSound(mouseOver)?.Embed?.Value;
            return PowerPointEmbeddedSound.Read(OwnerSlide.SlidePart,
                relationshipId);
        }
        ...
        private A.HyperlinkType? GetInteraction(bool mouseOver) {
            ...
            return mouseOver
                ? properties?.GetFirstChild<A.HyperlinkOnHover>()
                : properties?.GetFirstChild<A.HyperlinkOnClick>();
        }

        private A.HyperlinkType GetOrCreateInteraction(bool mouseOver) {
            NonVisualDrawingProperties properties =
                GetNonVisualDrawingProperties(create: true)
                ?? throw new NotSupportedException(
                    "This shape type does not expose interactive drawing properties.");
            A.HyperlinkType? existing = mouseOver
                ? properties.GetFirstChild<A.HyperlinkOnHover>()
                : properties.GetFirstChild<A.HyperlinkOnClick>();
            if (existing != null) return existing;
            A.HyperlinkType created = mouseOver
                ? new A.HyperlinkOnHover { Id = string.Empty }
