...
internal static class TesseractTsvParser {
    internal static OcrResult Parse(string tsv, string? language) {
        ...
        foreach (IGrouping<(int Page, int Block, int Paragraph, int Line), WordRow> group in groups) {
            ...
            foreach (WordRow word in lineWords) {
                spans.Add(new OcrTextSpan {
                    ...
                    ParagraphId = paragraphId,
                    LineId = lineId,
                Region = new OcrRegion { X = word.Left, Y = word.Top, Width = word.Width, Height = word.Height },
                    CoordinateUnit = OcrCoordinateUnit.Pixels
                });
            }
        }

        IReadOnlyList<OcrDiagnostic> diagnostics = invalidRowCount == 0
    ...
    private static OcrRegion Union(IReadOnlyList<WordRow> words) {
        ...
        return new OcrRegion { X = left, Y = top, Width = right - left, Height = bottom - top };
    }
