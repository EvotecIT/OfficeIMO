...
public sealed partial class TesseractOcrEngine {
    ...
    public async Task<string> GetVersionAsync(CancellationToken cancellationToken = default) {
        OcrProcessResult result = await RunDiscoveryAsync(new[] { "--version" }, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<IReadOnlyList<string>> GetLanguagesAsync(CancellationToken cancellationToken = default) {
        ...
        OcrProcessResult result = await RunDiscoveryAsync(arguments, cancellationToken).ConfigureAwait(false);
    ...
    private Task<OcrProcessResult> RunDiscoveryAsync(IReadOnlyList<string> arguments, CancellationToken cancellationToken) {
        return OcrProcessRunner.RunAsync(new OcrProcessCommand {
