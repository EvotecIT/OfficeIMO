...
if ($null -ne $browserCase) {
    foreach ($failure in @('missing-browser-reference', 'changed-browser-reference')) {
        $target = Join-Path $OutputPath $failure
        New-Item -ItemType Directory -Path $target | Out-Null
        Copy-Item -LiteralPath (Join-Path $BundlePath $browserCase.contract.id) -Destination $target -Recurse
        $candidate = $bundle | ConvertTo-Json -Depth 100 | ConvertFrom-Json -Depth 100
        $candidate.cases = @($candidate.cases | Where-Object { $_.contract.id -eq $browserCase.contract.id })
        $entry = $candidate.cases[0]
        if ($failure -eq 'missing-browser-reference') {
            $entry.browserReference.pdfPath = $entry.contract.id + '/missing-browser-reference.pdf'
        } else {
            [IO.File]::AppendAllText((Join-Path $target $entry.browserReference.pdfPath), 'tampered')
        }
        $candidate | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath (Join-Path $target 'bundle.json') -Encoding utf8
        & dotnet $ToolPath verify --output $target *> (Join-Path $target 'verification.log')
        if ($LASTEXITCODE -ne 1) { throw "$failure did not return a structured verification failure." }
