...
loaded `srcdoc` frame with `document.open/write/close`, checks the parent realm's
mutation record for the removed root, and preserves a paragraph across writes
that split a character reference. The four PNGs show the checked
...
the Navigation API, or general browser compatibility.
