...
At clean renderer head `079e50f3d`, an offline replay of the same WAI archive and Chromium reference found no operation failures. The page-one W3C logo's managed PDF link expanded from 4.4 × 4.4 pt to its full 34.5 × 34.5 pt SVG viewport. Chromium's corresponding link is 34.5 × 46.5 pt because its anchor includes line-box space; this improvement does not establish link-geometry parity. Both print PDFs remain three pages with 31 distinct link destinations; the managed PDF still has 89 link rectangles versus Chromium's 63. The full HTML suite passed 3,499/3,499 on .NET 8 and .NET 10, the PDF suite passed 7,937/7,937 on .NET 10, the HTML PDF netstandard2.0 build passed, and clean-source H4 advanced-held-out acceptance passed 8/8. The exact-head WAI and H4 reports are under `Ignore/HtmlUnknownPageQualification/h10-wai-svg-link-clean-079e50f3d/` and `h10-h4-svg-link-clean-079e50f3d/` respectively. Remaining print resources, portable fonts, visual differences and cross-platform budgets are still open.

PDFKit inspection of those saved print PDFs found 35 page-and-destination groups in each. On page one, the W3C logo annotation is 121.5 × 34.5 pt in OfficeIMO and 122.25 × 46.5 pt in Chromium. The adjacent WAI breadcrumb has one 174.75 × 33.75 pt Chromium annotation but two 13.06-pt-high OfficeIMO text-fragment annotations. The page-three footer shows the same pattern: a single 393 × 33 pt Chromium accessibility-statement link becomes two 12.19-pt-high managed fragments. Four first-page tutorial cards also have a separate 0.75-pt-wide, image-height managed annotation beside their image links. These are hit-area differences despite matching destinations and page counts. The SVG drawing currently creates its own PDF link at its viewport, while inline text creates links per painted run; the outer anchor's CSS line box and fragments do not own one shared hit-area description. A probe that enlarged the atomic-run link shape left the logo at 34.5 pt because that shape does not own the SVG drawing's link, so it was discarded. The next renderer change must carry anchor identity and per-line box fragments through layout and pagination, then emit PDF annotations from those fragments without merging unrelated anchors that happen to share a URL.

## W3C anchor-owned PDF hit areas

Source `86278bc13` carries anchor-owned, paint-neutral rectangles through inline layout, SVG and block boxes, pagination, and PDF output. The frozen WAI archive has the same SHA-256 as above; the offline replay used clean, exact-source assemblies and finished all 13 operations without failure. Chromium and OfficeIMO's zero-margin, installed-Trebuchet print both produced three pages. All three print page pairs were inspected at 96 dpi. The OfficeIMO rasters are pixel-identical to the preceding `079e50f3d` print, so this change affects hit areas rather than page paint.

PDFKit URL annotations were grouped by page and destination, then compared as unions of rectangles in PDF points. All 35 page-and-destination groups and 31 distinct destinations remain present. Chromium has 63 rectangles; OfficeIMO now has 51, down from 89 before anchor ownership. Mean group intersection-over-union rises from 0.638 to 0.853, and union-weighted overlap rises from 0.596 to 0.865. The page-one W3C logo link is now 121.5 × 46.5 pt against Chromium's 122.25 × 46.5 pt; the WAI breadcrumb is one 177.14 × 33.37 pt rectangle against 174.75 × 33.75 pt. The page-three accessibility-statement link is one 395.03 × 33 pt rectangle against Chromium's 393 × 33 pt, replacing two short text-run rectangles. This is a substantial hit-area correction, not exact geometry parity: the page-two WCAG link has only 0.393 overlap because its list/text placement still differs. Remaining print layout differences, 26 archive-missing resources, and portable-font policy remain open.

The full .NET 10 HTML and PDF suites passed 3,511/3,511 and 7,937/7,937; the seven focused link regressions passed on .NET 8, and the HTML PDF netstandard2.0 build was clean. H4 advanced-held-out acceptance passed 8/8 on the clean source. The current macOS H4 static budget failed at 11,425.5 ms cold, 4,017.9 ms slowest warm, and 703,037,440 bytes sampled peak versus ceilings of 8,000 ms, 3,000 ms, and 700,000,000 bytes. Output fingerprints matched across all 76 outputs. A clean pre-change `1ecd56883` measurement during the same period of concurrent CasaRay/Xcode work also missed both time ceilings (13,052.7 ms cold, 4,663.7 ms warm) but peaked at 464,781,312 bytes. Cold managed allocation changed by 70,200 bytes and the three warm iterations by 0.5–1.2 MB. The time failure is therefore not isolated to the link change; the current peak-memory failure remains unqualified and requires an idle-host repeat. Windows and Linux budgets are still open. Exact replay PDFs, page rasters, reports, and PDFKit rectangle dumps are retained in `Ignore/HtmlUnknownPageQualification/h10-wai-anchor-clean-86278bc13/`; H4 acceptance and both budget reports are in adjacent `h10-h4-anchor-*` directories.

## W3C outside-marker list alignment

At clean source `36a016a16`, an inline list item no longer reserves its outside-marker width twice: the list's authored or default padding supplies the marker gutter, and inline text begins at the same content edge as a block-child list item. Left-to-right and right-to-left regressions passed on .NET 8 and .NET 10. The full .NET 10 HTML suite passed 3,512/3,512, the HTML PDF netstandard2.0 build had no warnings, and exact-source H4 advanced-held-out acceptance passed 8/8.

The unchanged WAI MHTML archive replayed offline from that clean source with no failures across 13 operations. Chromium and OfficeIMO's zero-margin, installed-Trebuchet print remain three pages with 31 distinct destinations and 35 page-and-destination groups. All three page pairs were rasterized and inspected at 96 dpi. Managed pages one and three are pixel-identical to `86278bc13`; page two moves the list marker and WCAG link left. PDFKit measures the WCAG link's first rectangle at X=54.75 pt in both Chromium and OfficeIMO, instead of 67.65 pt in the previous OfficeIMO print. Its managed Y coordinate is still 9.02 pt lower, so that page/URL group's rectangle overlap rises only from 0.393 to 0.426. The remaining vertical flow is open. Rechecking the saved PDFs confirmed that the footer hit-area height was already 33 pt in both engines at `86278bc13`; the earlier 15 pt value in this note was a transcription error. The exact replay and PDFKit rectangles are retained under `Ignore/HtmlUnknownPageQualification/h10-wai-list-gutter-clean-36a016a16/`; H4 acceptance is under `h10-h4-list-gutter-clean-36a016a16/` in the same ignored parent. The macOS static budget was not rerun at this head while eight concurrent Xcode compiler processes were each consuming most of a CPU core; the preceding failed comparison and Windows/Linux budgets remain unresolved.

## W3C nested-margin and page-entry alignment

At clean renderer source `a227c6269`, collapsed margins retain their positive and negative components through nested block layout. The WAI archive's `p + ul { margin-top: -8px }` rule now reaches a list whose items contain paragraphs. A focused browser probe and a mutation of the frozen stylesheet established the expected signed-margin behavior before the renderer change. The change also prevents a bordered flex navigation box from leaving an empty border fragment on the preceding PDF page. Direct and nested list regressions, and the page-entry case at two lead heights, pass.

The frozen offline WAI replay completed all 13 operations without failures. Chromium and OfficeIMO's matched zero-margin, installed-Trebuchet print still have three pages, 31 distinct link destinations and 35 page-and-destination URL groups; PDFKit counts 63 and 51 link rectangles respectively. All three page pairs were inspected at 96 dpi. OfficeIMO pages one and three are pixel-identical to the preceding clean print, and the navigation box starts intact on page three. On page two, the managed WCAG text baseline is about 1 pt from Chromium; its first link rectangle starts at X=54.75 pt in both PDFs and Y=62.11 pt versus Chromium's Y=65.13 pt. The earlier vertical gap was 9.02 pt. The rectangles also differ in height, so this is closer alignment, not exact hit-area or visual parity.

The .NET 10 HTML suite passed 3,516/3,516, focused .NET 8 margin/page-entry tests passed 10/10, the HTML PDF netstandard2.0 build had no warnings, and clean-source H4 advanced-held-out acceptance passed 8/8. The exact-head WAI PDF, report, PDFKit measurements and page rasters are under `Ignore/HtmlUnknownPageQualification/h10-wai-margin-clean-a227c6269/`; H4 evidence is under `h10-h4-margin-clean-a227c6269/` in the same ignored parent. WAI print qualification still needs the missing archived resources, a portable-font policy, remaining visual and annotation alignment, and supported-platform budgets.

## W3C inline-anchor PDF hit areas

At clean source `8c041e507`, inline text anchors use the selected face height for their paint-neutral PDF link rectangles instead of inheriting the surrounding line's extra leading. A Chromium print-mode probe of the same 16 px Trebuchet and 1.5 line-height combination measured a 24 px list line box and a 19 px inline anchor box. The frozen WAI archive replayed offline with no runner failures across 13 operations. Chromium and OfficeIMO's matched zero-margin, installed-Trebuchet print remain three pages; all three managed page rasters are byte-identical to `a227c6269` at 96 dpi. The change therefore improves hit areas without changing page paint.

PDFKit still finds the same 31 destinations and 35 page-and-destination groups, with 63 Chromium and 51 OfficeIMO rectangles. Mean group intersection-over-union rises from 0.864 to 0.901 and union-weighted overlap from 0.881 to 0.917. The page-two WCAG group's overlap rises from 0.788 to 0.877: the first managed rectangle is now X=54.75, Y=66.18, H=13.93 pt against Chromium's X=54.75, Y=65.13, H=14.25 pt. The page-three W3C footer group rises from 0.718 to 0.874. Exact geometry is still open: Chromium splits the WCAG link into four rectangles while OfficeIMO emits two, and their positions and widths differ. This evidence uses a macOS installed font, not a portable-font qualification; the 26 resources absent from the MHTML archive and other visible differences remain.

A focused face-metric regression passed on .NET 8 and .NET 10. The full .NET 10 HTML suite passed 3,517/3,517, the HTML PDF netstandard2.0 build had no warnings, and exact-head H4 advanced-held-out acceptance passed 8/8. The exact-head WAI PDFs, report, PDFKit rectangles and page rasters are under `Ignore/HtmlUnknownPageQualification/h10-wai-inline-box-clean-8c041e507/`; H4 evidence is under `h10-h4-inline-box-clean-8c041e507/` in the same ignored parent. A current macOS time/memory budget is still needed on an idle host; this replay ran alongside concurrent Xcode/CasaRay work. Windows and Linux budgets remain open.
