...
try {
    ...
    foreach (HtmlPublicResourceResult asset in assets)
        await File.WriteAllBytesAsync(Path.Combine(outputDirectory, "assets", asset.Sha256), asset.Resource.Content);
    ...
    await File.WriteAllTextAsync(Path.Combine(outputDirectory, "acquisition.json"), JsonSerializer.Serialize(
        new { document = Provenance(document), assets = assets.Select(Provenance).ToArray() }, jsonOptions));
    ...
    var origins = assets.SelectMany(asset => new[] { asset.Resource.Url, asset.Resource.FinalUrl })
        .Select(assetUrl => new Uri(assetUrl.GetLeftPart(UriPartial.Authority)))
        .Where(origin => origin.GetLeftPart(UriPartial.Authority) != document.Resource.FinalUrl.GetLeftPart(UriPartial.Authority))
    ...
    var page = new HtmlScriptRequest {
        ...
        ResourcePolicy = new HtmlRuntimeResourcePolicy {
            AllowNetwork = false, AllowedOrigins = origins,
    ...
    try {
        ...
        response = await HtmlRuntimeProtocol.ReadAsync<HtmlPublicRenderResponse>(
            ...
            ?? throw new HtmlScriptRuntimeException("The isolated renderer returned no response.");
        ...
        if (!response.RendererSha256.Equals(expectedRenderer, StringComparison.OrdinalIgnoreCase) ||
            !response.WorkerSha256.Equals(expectedWorker, StringComparison.OrdinalIgnoreCase) ||
            !response.RendererFilesSha256.Equals(expectedRendererFiles, StringComparison.OrdinalIgnoreCase) ||
            !response.WorkerFilesSha256.Equals(expectedWorkerFiles, StringComparison.OrdinalIgnoreCase))
    ...
    await File.WriteAllTextAsync(Path.Combine(outputDirectory, "failure.json"), JsonSerializer.Serialize(new {
        ...
        documentSha256 = document?.Sha256, assets = assets.Count
    }, jsonOptions));
