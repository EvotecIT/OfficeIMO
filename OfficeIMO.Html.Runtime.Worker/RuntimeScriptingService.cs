...
internal sealed class RuntimeScriptingService(JsScriptingService scripting, Func<RuntimeModuleLoader> modules,
    Func<IDocument, Engine?> ensureEngine, object sessionSync, HtmlScriptRequest request, Action<string> report) : IScriptingService, ISynchronousScriptingService, IDisposable {
    ...
    public object EvaluateScript(IDocument document, string source, string type, string sourceUrl) {
        ...
        bool child = document.Context.Parent != null;
        if (child && (type?.Equals("importmap", StringComparison.OrdinalIgnoreCase) == true
            || type?.Equals("module", StringComparison.OrdinalIgnoreCase) == true)) return null!;
    ...
    public async Task EvaluateScriptAsync(IResponse response, ScriptOptions options, CancellationToken cancel) {
        ...
        if (options.Document.Context.Parent != null && (isModule || string.Equals(options.PreparedType, "importmap", StringComparison.OrdinalIgnoreCase))) return;
        if (ensureEngine(options.Document) == null) return;
        if (!isModule) {
