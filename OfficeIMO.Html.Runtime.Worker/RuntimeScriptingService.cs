...
internal sealed class RuntimeScriptingService(JsScriptingService scripting, Func<RuntimeModuleLoader> modules, HtmlScriptRequest request) : IScriptingService {
    ...
    public object EvaluateScript(IDocument document, string source, string type, string sourceUrl) {
        if (type?.Equals("importmap", StringComparison.OrdinalIgnoreCase) == true) {
            modules().ImportMap.Load(source, new Uri(document.BaseUri));
            return null!;
        }
        ...
        string location = string.IsNullOrEmpty(sourceUrl) ? document.BaseUri : new Uri(new Uri(document.BaseUri), sourceUrl).AbsoluteUri;
        lock (engine) return engine.Evaluate(source, location).ToObject()!;
    }
    ...
    public async Task EvaluateScriptAsync(IResponse response, ScriptOptions options, CancellationToken cancel) {
        ...
        if (!isModule) {
            string sourceUrl = options.Element?.HasAttribute("src") == true ? response.Address.Href : options.Document.BaseUri;
        ...
        ModuleImportOperation operation = await options.EventLoop.EnqueueAsync(_ => {
            ...
            var baseUrl = new Uri(external ? response.Address.Href : options.Document.BaseUri);
            ...
            string identity = modules().Register(source, baseUrl, external ? new Uri(new Uri(options.Document.BaseUri), options.Element.Source!).AbsoluteUri : null);
