...
internal static class RuntimeStructuredClone {
    internal static JsValue Create(Engine engine, int maximumBytes) {
        ...
        return engine.Invoke(engine.Evaluate(reader.ReadToEnd()), new JsValue[] { Brand(engine), maximumBytes, Errors(engine) });
    }
    ...
    internal static ObjectInstance CreateTransport(Engine engine, int maximumCharacters) {
        ...
        using var reader = new StreamReader(stream);
        return engine.Invoke(engine.Evaluate(reader.ReadToEnd()), new JsValue[] { Brand(engine), maximumCharacters, Errors(engine) }).AsObject();
    }
