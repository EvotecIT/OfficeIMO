...
""";
        ...
        Assert.Equal(new MarkdownSourceSpan(17, 1, 17, 31), details.SummarySourceSpan);
        Assert.Equal(new MarkdownSourceSpan(17, 1, 17, 9), details.SummaryOpeningTagSourceSpan);
        Assert.Equal(new MarkdownSourceSpan(17, 10, 17, 21), details.SummaryTextSourceSpan);
        Assert.Equal(new MarkdownSourceSpan(17, 22, 17, 31), details.SummaryClosingTagSourceSpan);
        ...
        Assert.Equal(1, detailsSnapshot.FieldSourceSpans["summary"]!.StartColumn);
        Assert.Equal(31, detailsSnapshot.FieldSourceSpans["summary"]!.EndColumn);
        Assert.Equal(1, detailsSnapshot.FieldSourceSpans["summaryOpeningTag"]!.StartColumn);
        Assert.Equal(9, detailsSnapshot.FieldSourceSpans["summaryOpeningTag"]!.EndColumn);
        Assert.Equal(10, detailsSnapshot.FieldSourceSpans["summaryText"]!.StartColumn);
        Assert.Equal(21, detailsSnapshot.FieldSourceSpans["summaryText"]!.EndColumn);
        Assert.Equal(22, detailsSnapshot.FieldSourceSpans["summaryClosingTag"]!.StartColumn);
        Assert.Equal(31, detailsSnapshot.FieldSourceSpans["summaryClosingTag"]!.EndColumn);
...
""";
        ...
        Assert.Equal("Heads up", calloutTitle.Value);
        Assert.Equal("Heads up", title.Value);
        Assert.Equal(title.SourceSpan, calloutTitle.SourceSpan);
        ...
        AssertEquivalentField(calloutTitle, native.FindBlockSourceFieldAtPosition(3, 13));
        AssertEquivalentField(calloutBody, native.FindBlockSourceFieldAtPosition(4, 3));
        AssertEquivalentField(detailsOpeningTag, native.FindBlockSourceFieldAtPosition(6, 3));
        AssertEquivalentField(summaryOpeningTag, native.FindBlockSourceFieldAtPosition(7, 3));
        AssertEquivalentField(summaryText, native.FindBlockSourceFieldAtPosition(7, 10));
        AssertEquivalentField(summaryClosingTag, native.FindBlockSourceFieldAtPosition(7, 15));
        AssertEquivalentField(detailsBody, native.FindBlockSourceFieldAtPosition(9, 3));
        AssertEquivalentField(detailsClosingTag, native.FindBlockSourceFieldAtPosition(10, 3));
        ...
        Assert.Contains("> [!NOTE] Renamed", native.CreateReplaceEdit(calloutTitle, "Renamed").Apply(native.SourceMarkdown), StringComparison.Ordinal);
        ...
        Assert.Contains("<summary>Short</summary>", native.CreateReplaceEdit(summaryText, "Short").Apply(native.SourceMarkdown), StringComparison.Ordinal);
...
""";
        ...
        var tableRows = table.EnumerateSourceFields("tableRow").ToArray();
        Assert.Equal(2, tableRows.Length);
        Assert.Equal(new[] { 0, 1 }, tableRows.Select(field => field.Index).ToArray());
        Assert.Equal(new[] { "Name | Value", "CPU | 42" }, tableRows.Select(field => field.Value).ToArray());
        Assert.Equal(new MarkdownSourceSpan(1, 3, 1, 14), tableRows[0].SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(3, 3, 3, 10), tableRows[1].SourceSpan);
        ...
        var selectedTableRow = native.FindBlockSourceFieldAtPosition(3, 6);
        Assert.NotNull(selectedTableRow);
        Assert.Equal("tableRow", selectedTableRow!.Name);
        Assert.Equal(tableRows[1].SourceSpan, selectedTableRow.SourceSpan);
        ...
        var snapshotRows = snapshot.EnumerateSourceFields("tableRow").ToArray();
        Assert.Equal(2, snapshotRows.Length);
        Assert.Equal("CPU | 42", snapshotRows[1].Value);
        Assert.Equal(1, snapshotRows[1].Index);
        Assert.Equal(3, snapshotRows[1].SourceSpan.StartLine);
        Assert.Equal(10, snapshotRows[1].SourceSpan.EndColumn);
        ...
        var editedTableRow = native.CreateReplaceEdit(tableRows[1], "CPU | 84").Apply(native.SourceMarkdown);
        Assert.Equal("| CPU | 84 |", editedTableRow.Split('\n')[2]);
    }
...
""";
        ...
        Assert.Equal(11, snapshot.FieldSourceSpans["calloutTitle"]!.StartColumn);
        Assert.Equal(18, snapshot.FieldSourceSpans["calloutTitle"]!.EndColumn);
        Assert.Equal(11, snapshot.FieldSourceSpans["title"]!.StartColumn);
        Assert.Equal(18, snapshot.FieldSourceSpans["title"]!.EndColumn);

        var calloutTitle = Assert.Single(native.EnumerateBlockSourceFields("calloutTitle"));
        Assert.Equal("Heads up", calloutTitle.Value);
        Assert.Equal(callout.TitleSourceSpan, calloutTitle.SourceSpan);
