...
namespace OfficeIMO.Tests.MarkdownSuite {
    public class Markdown_Reader_Roundtrip_Tests {
        ...
        public void Reader_Preserves_Text_After_Inline_Image(string source, string image) {
            ...
            Assert.IsType<ParagraphBlock>(Assert.Single(parsed.Blocks));
        ...
        [Theory]
        [InlineData("![Survey photo](photo.png){title=\"a}b\"}{width=320 height=180} Credit: NOAA Fisheries.", false)]
        [InlineData("[![Survey photo](photo.png)](https://example.com/survey){width=320 height=180}{#survey} Credit: NOAA Fisheries.", true)]
        public void Reader_Preserves_Inline_Image_Size_Attributes_And_Credit_Source_Position(string source, bool linked) {
            ...
            var native = MarkdownNativeDocument.Parse(source, options);
            var nativeParagraph = Assert.IsType<MarkdownNativeParagraphBlock>(Assert.Single(native.Blocks));
            var nativeImage = Assert.Single(nativeParagraph.InlineRuns, inline => inline.Kind == (linked ? MarkdownNativeInlineKind.ImageLink : MarkdownNativeInlineKind.Image));
            int creditColumn = source.IndexOf("Credit:", StringComparison.Ordinal) + 1;
            Assert.True(nativeImage.SourceSpan.HasValue);
            Assert.True(nativeImage.SourceSpan.Value.EndColumn < creditColumn);
