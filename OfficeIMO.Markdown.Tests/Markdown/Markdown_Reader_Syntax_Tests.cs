...
""";
    ...
    public void ParseWithSyntaxTree_Captures_Html_Comment_Block() {
        ...
        Assert.Equal(NormalizeLineEndings(markdown), NormalizeLineEndings(Assert.IsType<string>(comment.Literal)));
...
""";
        ...
        Assert.Collection(comment.Children,
            ...
            node => {
                Assert.Equal(MarkdownSyntaxKind.HtmlCommentBody, node.Kind);
                Assert.Equal(" keep\nthis comment", NormalizeLineEndings(Assert.IsType<string>(node.Literal)));
