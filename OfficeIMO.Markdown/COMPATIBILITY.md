...
- Extension model
  ...
  - Markdig-style abbreviations are available as an opt-in reader feature through `MarkdownReaderOptions.Abbreviations`. Definitions are consumed, visible abbreviation text renders as HTML `<abbr>`, syntax/native metadata exposes visible text plus definition-title source edits, and parsed documents preserve abbreviation definitions when written back to Markdown; broader Markdig edge coverage remains partial.
- Spec breadth
