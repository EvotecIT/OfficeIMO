...
- Extension model
  ...
  - Markdig-style abbreviations are available as an opt-in reader feature through `MarkdownReaderOptions.Abbreviations`. Definitions are consumed, including empty-title definitions, visible abbreviation text renders as HTML `<abbr>`, syntax/native metadata exposes visible text plus definition-title source edits, and parsed documents preserve abbreviation definitions when written back to Markdown. Markdig-compatible inline boundaries cover trailing dashes and avoid expansion after opening punctuation such as quotes, slash, dot, and opening parentheses; bracket-text expansion, list-item abbreviation definitions, and Unicode HTML escaping remain named partial gaps.
- Spec breadth
