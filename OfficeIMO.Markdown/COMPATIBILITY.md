...
- Autolinks
  - Literal autolinks cover common cases (`http(s)://...`, `www.*`, plain emails, selected bare schemes such as `mailto:`, `ftp://`, `tel:`, and `xmpp:`, and angle-bracket absolute URIs like `mailto:`, `ftp://`, `tel:`, and `urn:`) but do not aim for full spec coverage. Compatibility profiles can narrow the bare scheme prefix list and opt into Markdig-style bracket, trailing-semicolon, trailing-quote, host-underscore rejection, apostrophe-started bare-scheme literal fallback, or bare-`mailto:` semicolon/colon/dash behavior, for example when matching Markdig `UseAutoLinks`.
  - Compatibility profiles can also reject URL-shaped bare autolinks with user-info authorities such as `https://user@example.com/path`, matching Markdig's literal-text behavior while preserving OfficeIMO's default permissive behavior.
  - HTML URL attribute rendering preserves `~` by default. Set `HtmlOptions.PercentEncodeTildeInUrlAttributes` when matching Markdig-style URL attribute output.
