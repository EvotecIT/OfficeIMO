...
- Extension model
  - The parser/renderer architecture is much cleaner than before, with public block parser, fenced block, inline parser, inline transform, context-aware type-targeted renderer/writer override seams, syntax-kind-targeted renderer/writer overrides, and override-aware nested rendering through the built-in block containers. Parsed-document render contexts can resolve semantic nodes or matched syntax nodes back to normalized/original source slices. It is still not as broad as mature dedicated markdown engines because full lossless token/trivia extension points are not first-class yet.
- Spec breadth
- We now cover a much larger compatibility set than the earlier subset reader, and the test suite now includes 242 pinned CommonMark 0.31.2 fixtures, 36 cmark-gfm smoke fixtures, and focused upstream non-render regression coverage with selected AST path/span assertions in addition to curated Markdig 1.3.2 parity cases. A package guardrail keeps the Markdig test and benchmark baselines aligned, but that is still not the same thing as full CommonMark/GFM conformance.
- Syntax-backed parse results now expose normalized source slices for span-backed nodes, including heading level/text tokens with multiline setext text and underline source-edit coverage, fenced-code opening/info/content/closing tokens with nested blockquote/list source-map coverage, code-span content and delimiter marker tokens, backslash escape markers and escaped characters, decoded entity source text, hard-break marker spellings, inline footnote reference delimiters, inline/reference-style link and image opening/separator/closing delimiters, linked-image outer opening/separator/closing delimiters, source-backed autolink targets/angle markers, and supported inline HTML tag opening/closing marker tokens with nested inline source-map coverage. Render/write extensions can also ask their active context to materialize source slices directly from token-level `MarkdownSourceSpan` values. Rebuilt table-cell, blockquote, and details body syntax now invalidates stale parsed child projections when their public child block collections are replaced, so syntax-to-semantic ownership follows the current typed AST. `MarkdownReaderOptions.PreserveTrivia` can retain the raw reader input as parse-result metadata for future lossless work. Line-ending-equivalent original input, including CRLF and standalone CR, can also materialize original source slices through line/column coordinates. `MarkdownRoundtripWriter` can preserve unchanged trivia-backed parse results byte-for-byte, apply explicit native source edits to original input when every edit remaps safely, and report diagnostics when it falls back to generated or normalized markdown; source-edit and transform fallback diagnostics now carry the source span that caused the fallback when one is available. Transform diagnostics also expose whether top-level block fingerprints changed, which individual input-block spans were affected, deepest original/final syntax-node anchors, and precise original child-node anchors for one-to-one single-block transforms when syntax fingerprints prove them; native diagnostics/snapshots and roundtrip fallback diagnostics prefer the most specific proven source span and preserve related spans for editor hosts. Full trivia capture, generalized below-block changed-node diffing, and general byte-preserving edit writing are still not implemented.
