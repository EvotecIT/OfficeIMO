...
- Extension model
  - The parser/renderer architecture is much cleaner than before, with public block parser, fenced block, inline parser, inline transform, context-aware type-targeted renderer/writer override seams, and syntax-kind-targeted renderer/writer overrides. Parsed-document render contexts can resolve semantic nodes or matched syntax nodes back to normalized/original source slices. It is still not as broad as mature dedicated markdown engines because nested block override traversal and lossless token/trivia extension points are not first-class yet.
- Spec breadth
