...
- Extension model
  ...
  - `HtmlOptions.EscapeNonAsciiText` keeps the historical .NET encoder behavior by default and lets Markdig/GFM-style profiles keep non-ASCII visible text literal while still escaping HTML-sensitive characters. Inline text, link display text, code block text, captions, simple quote text, and abbreviation output use this policy; broader attribute, TOC/helper, raw-HTML escape, and renderer-extension paths still need explicit policy routing before `UseNonAsciiNoEscape` can be promoted.
- Spec breadth
