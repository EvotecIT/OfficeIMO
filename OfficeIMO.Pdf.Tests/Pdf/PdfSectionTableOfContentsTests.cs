...
public class PdfSectionTableOfContentsTests {
    ...
    public void TableOfContents_StabilizesSectionPagesAndBuildsInternalNavigation() {
        ...
        PdfDocumentReadResult logical = PdfDocument.Load(bytes).Read();
        IReadOnlyList<string> textByPage = PdfDocument.Load(bytes).Reader.TextByPage();
    ...
    public void TableOfContents_RespectsHierarchyAndCustomPageFormatting() {
        ...
        string firstPage = PdfDocument.Load(bytes).Reader.TextByPage()[0];
    ...
    public void DeferredSection_GeneratedDestinationIsStableAcrossRepeatedRendering() {
        ...
        Assert.NotEmpty(PdfDocument.Load(second).Read().GetLinksByDestinationName(secondDestination));
    }
}
