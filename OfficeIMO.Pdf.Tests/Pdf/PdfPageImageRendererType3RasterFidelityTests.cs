...
public partial class PdfPageImageRendererTests {
    ...
    [Fact]
    public void RenderPage_FailsClosedForOversizedMinifiedType3TilingPatternTile() {
        string type3Font = "5 0 obj\n<< /Type /Font /Subtype /Type3 /PaintType 1 /FontBBox [0 0 500 700] /FontMatrix [0.001 0 0 0.001 0 0] /CharProcs << /A 6 0 R >> /Encoding << /Differences [65 /A] >> /FirstChar 65 /LastChar 65 /Widths [500] /Resources << /Pattern << /P1 7 0 R >> >> >>\nendobj";
        string glyph = BuildStreamObject(6, "<<", "500 0 d0 /Pattern cs /P1 scn 0 0 500 700 re f");
        string pattern = BuildStreamObject(7, "<< /Type /Pattern /PatternType 1 /PaintType 1 /TilingType 1 /BBox [0 0 10000 10000] /XStep 1000000 /YStep 1000000 /Matrix [0.001 0 0 0.001 0 0] /Resources << >>", "0 0 1 rg 0 0 10000 10000 re f");
        byte[] pdf = BuildSingleStreamPdf("BT /FType3 18 Tf 20 100 Td (A) Tj ET", "<< /Font << /FType3 5 0 R >> >>", type3Font, glyph, pattern);

        IReadOnlyList<PdfRenderCapabilityDiagnostic> diagnostics =
            PdfReadDocument.Open(pdf).Pages[0].GetRenderCapabilityDiagnostics();

        Assert.Contains(diagnostics, diagnostic => diagnostic.Code == PdfRenderCapabilities.Type3FontSubstitutionId);
    }
}
