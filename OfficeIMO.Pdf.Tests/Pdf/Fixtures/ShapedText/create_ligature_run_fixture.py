...
used = sorted({name for word in WORDS for name, _ in word} | {"space"}, key=lambda name: gid[name])
to_unicode_entries = [(gid[name], text) for word in WORDS for name, text in word] + [(gid["space"], " ")]
...
line = " ".join("<" + "".join(f"{gid[name]:04X}" for name, _ in word) + "> Tj " + space + " Tj" for word in WORDS)
