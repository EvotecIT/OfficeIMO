...
`OfficeIMO.Email.AddressBook` reads Outlook Offline Address Book component sets without Outlook, native code, or a third-party OAB parser. It inventories legacy components and lazily decodes uncompressed OAB v4 Full Details files into shared `OfficeIMO.Email` address, contact, and MAPI models.

The package is read-only. It does not modify an Outlook cache or contact Exchange.

## Install

```powershell
dotnet add package OfficeIMO.Email.AddressBook
```
...
- `OfficeIMO.Email.AddressBook` owns OAB file-set discovery, v4 schema/record decoding, search, and validation.
...
- `OfficeIMO.Email.Store` remains the PST/OST/OLM/EMLX mailbox owner; an OAB entry is not an `EmailDocument`.
...
Direct first-party dependency: `OfficeIMO.Email`. External parser dependencies: none.
