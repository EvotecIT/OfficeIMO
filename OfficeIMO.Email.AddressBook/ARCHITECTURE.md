...
The OAB package is a sibling of the mailbox-store package:

```text
OfficeIMO.Email                         shared address, contact, MAPI, diagnostics
├── OfficeIMO.Email.Store               PST/OST/OLM/EMLX mailbox containers
└── OfficeIMO.Email.AddressBook         OAB directory snapshots
    └── OfficeIMO.Reader.EmailAddressBook  optional Reader projection
```

An OAB record represents a directory object, not a message. Keeping it outside `OfficeIMO.Email.Store` avoids coupling address-book users to PST/OST traversal and avoids projecting directory entries as artificial `EmailDocument` instances.
