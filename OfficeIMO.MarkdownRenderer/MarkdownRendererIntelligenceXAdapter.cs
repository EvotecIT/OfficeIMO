...
public static class MarkdownRendererIntelligenceXAdapter {
    ...
    private static readonly Regex LegacyToolHeadingBulletRegex = new Regex(
        @"^(?<indent>\s*)-\s+(?<tool>[a-z0-9_.-]+):\s*(?<heading>#{2,6}\s+[^\r\n]+)\s*$",
        RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase | RegexOptions.Multiline);

    private static readonly Regex LegacyToolHeadingLeadRegex = new Regex(
        @"^(?<indent>\s*)-\s+(?<tool>[a-z0-9_.-]+):\s*$",
        RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase | RegexOptions.Multiline);

    private static readonly Regex LegacyToolHeadingSplitBulletRegex = new Regex(
        @"^(?<indent>\s*)-\s+(?<tool>[a-z0-9_.-]+):\s*(?<fragment>#{1,5})\s*$",
        RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase | RegexOptions.Multiline);

    private static readonly Regex LegacyToolSlugHeadingRegex = new Regex(
        @"^(?<indent>\s*)#{2,6}\s+(?<tool>[a-z0-9_.-]+)\s*$",
        RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase | RegexOptions.Multiline);

    private static readonly Regex CachedToolEvidenceMarkerLineRegex = new Regex(
    ...
    private static bool TryDetectLegacyIxVisualLanguageFromJson(string candidate, out string targetLanguage) {
        ...
        try {
            ...
            if (root.TryGetProperty("nodes", out var nodes)
                ...
                targetLanguage = "ix-network";
                return true;
            }

            if (LooksLikeLegacyIxDataViewPayload(root)) {
                targetLanguage = "ix-dataview";
                return true;
            }

            if (LooksLikeLegacyIxChartPayload(root)) {
                targetLanguage = "ix-chart";
                return true;
            }
    ...
    private static bool LooksLikeLegacyIxDataViewPayload(JsonElement root) {
        if (TryReadJsonString(root, "kind", "schema", "viewKind") is string kind
            && kind.IndexOf("dataview", StringComparison.OrdinalIgnoreCase) >= 0) {
            return true;
        }
        ...
        return TryGetPropertyIgnoreCase(root, out var columnsElement, "columns", "headers")
               && columnsElement.ValueKind == JsonValueKind.Array;
    }

    private static bool LooksLikeLegacyIxChartPayload(JsonElement root) {
        if (!TryGetPropertyIgnoreCase(root, out var typeElement, "type") || typeElement.ValueKind != JsonValueKind.String) {
            return false;
        }

        if (TryGetPropertyIgnoreCase(root, out var dataElement, "data")
            && (dataElement.ValueKind == JsonValueKind.Object || dataElement.ValueKind == JsonValueKind.Array)) {
            return true;
        }

        return TryGetPropertyIgnoreCase(root, out var optionsElement, "options")
    ...
    private static bool TryGetPropertyIgnoreCase(JsonElement element, out JsonElement value, params string[] candidateNames) {
        if (element.ValueKind != JsonValueKind.Object || candidateNames == null || candidateNames.Length == 0) {
            value = default;
            return false;
        }
        ...
        foreach (var property in element.EnumerateObject()) {
            for (var i = 0; i < candidateNames.Length; i++) {
                ...
                if (!string.IsNullOrWhiteSpace(candidate)
                    && string.Equals(property.Name, candidate, StringComparison.OrdinalIgnoreCase)) {
    ...
    private static string? TryReadJsonString(JsonElement element, params string[] candidateNames) {
        if (!TryGetPropertyIgnoreCase(element, out var value, candidateNames)) {
            return null;
        }
