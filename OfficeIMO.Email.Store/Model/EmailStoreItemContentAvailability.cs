...
public sealed class EmailStoreItemContentAvailability {
    ...
    internal static EmailStoreItemContentAvailability Create(
        ...
        int messageStatus = GetInteger(document.MapiProperties, 0x0E17) ?? 0;
    ...
    internal static bool? TryGetHeaderOnly(IEnumerable<MapiProperty> properties) {
        MapiProperty? property = properties.FirstOrDefault(item =>
            item.Name != null &&
            item.Name.PropertySet == PsetidCommon &&
            item.Name.LocalId == 0x8578);
        return ToBoolean(property?.Value);
    }

    internal static int? GetMessageStatus(IEnumerable<MapiProperty> properties) =>
        GetInteger(properties, 0x0E17);
    ...
    private static int? GetInteger(IEnumerable<MapiProperty> properties, ushort propertyId) {
        object? value = properties.FirstOrDefault(item => item.PropertyId == propertyId)?.Value;
        if (value is int integer) return integer;
        if (value is short shortInteger) return shortInteger;
        if (value is uint unsignedInteger && unsignedInteger <= int.MaxValue) return (int)unsignedInteger;
        return null;
    }
}
