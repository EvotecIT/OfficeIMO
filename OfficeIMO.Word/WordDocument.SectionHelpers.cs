...
namespace OfficeIMO.Word {
    ...
    public partial class WordDocument {
        ...
        /// </summary>
        public WordHeader? GetHeaderForSection(int sectionIndex, WordHeaderFooterType type) {
        ...
        public WordHeader? GetHeaderForSection(int sectionIndex = -1) => GetHeaderForSection(sectionIndex, WordHeaderFooterType.Default);

        /// <summary>
        /// Returns the footer for a specific section index (use -1 to target the last section).
        /// </summary>
        public WordFooter? GetFooterForSection(int sectionIndex, WordHeaderFooterType type) {
            if (this.Sections.Count == 0) throw new InvalidOperationException("Document has no sections.");
            if (sectionIndex < 0) sectionIndex = this.Sections.Count - 1;
            if (sectionIndex >= this.Sections.Count) throw new ArgumentOutOfRangeException(nameof(sectionIndex));
        ...
        /// </summary>
        /// <param name="sectionIndex">Index of the section or -1 for the last section.</param>
        public WordFooter? GetFooterForSection(int sectionIndex = -1) => GetFooterForSection(sectionIndex, WordHeaderFooterType.Default);
    }
}
