...
namespace OfficeIMO.Word {
    public partial class WordParagraph {
        public WordParagraph AddInsertedText(string text, string author, DateTime? date = null) {
            ...
            var ins = new InsertedRun() { Author = author, Date = date.Value, Id = WordHeadersAndFooters.GenerateRsid() };
            ins.Append(run);
        ...
        public WordParagraph AddDeletedText(string text, string author, DateTime? date = null) {
            ...
            run.Append(new Text(text) { Space = SpaceProcessingModeValues.Preserve });
            var del = new DeletedRun() { Author = author, Date = date.Value, Id = WordHeadersAndFooters.GenerateRsid() };
            del.Append(run);
