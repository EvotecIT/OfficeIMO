...
Legacy `.doc` support is first-party and dependency-free at runtime. The current reader projects supported Word 97-2003 body paragraphs, common run and paragraph formatting, built-in and custom paragraph styles, simple tables, paragraph-boundary sections, page setup, simple header/footer stories with supported direct run formatting, simple footnote/endnote bodies with supported direct run formatting, section note numbering and placement settings, and document properties into the normal `WordDocument` model. Native `.doc` saving is available for the supported simple subset: paragraphs, common run and paragraph formatting, tabs, line/page breaks, simple body tables, supported table-style border, shading, layout, and default-cell expansion, paragraph-boundary sections, page setup, simple header/footer stories with supported direct run formatting, simple footnote/endnote bodies with supported direct run formatting, supported section note settings, and scalar document properties. Unsupported features such as macros, embedded OLE objects, comments, text boxes, images, conditional or richer visual table styles, nested tables, richer note body structures, and richer header/footer or section shapes are diagnosed or blocked rather than silently flattened.
