...
Legacy `.doc` support is first-party and dependency-free at runtime. The current reader projects supported Word 97-2003 body paragraphs, simple zero-length body bookmarks, simple external and internal bookmark hyperlink fields with supported text, tab, and break display runs, common run and paragraph formatting, built-in and custom paragraph styles, simple tables, paragraph-boundary sections, page setup, simple header/footer stories with tabs, text-wrapping breaks, supported direct run formatting, and supported paragraph formatting, simple footnote/endnote bodies with supported direct run and paragraph formatting, section note numbering and placement settings, and document properties into the normal `WordDocument` model. Native `.doc` saving is available for the supported simple subset: paragraphs, simple zero-length body bookmarks, simple external and internal bookmark hyperlinks with supported text, tab, and break display runs, common run and paragraph formatting, tabs, line/page breaks, simple body tables, supported table-style border, shading, layout, paragraph formatting, run formatting, default-cell expansion, and conditional table/cell border, shading, paragraph formatting, run formatting, and cell-layout expansion, paragraph-boundary sections, page setup, simple header/footer stories with tabs, text-wrapping breaks, supported direct run formatting, and supported paragraph formatting, simple footnote/endnote bodies with supported direct run and paragraph formatting, supported section note settings, and scalar document properties. Unsupported features such as macros, embedded OLE objects, comments, text boxes, images, richer bookmark ranges, richer visual table style effects, nested tables, richer note body structures, and richer header/footer or section shapes are diagnosed or blocked rather than silently flattened.
