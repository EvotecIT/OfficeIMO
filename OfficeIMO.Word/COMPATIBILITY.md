...
| Legacy binary DOC | Partial | Supported Word 97-2003 `.doc` files route through the normal `WordDocument.Load(...)` path and explicit `LoadLegacyDocWithReport(...)` diagnostics are available. The dependency-free importer projects body paragraphs/runs, common direct character and paragraph formatting, built-in and custom paragraph styles, simple tables with common row/cell formatting and valid merged cells, single-section page setup including columns, page numbering, right-to-left gutter state, and vertical page alignment, paragraph-boundary multi-section page setup and break kinds, simple header/footer stories, and OLE document properties into the normal Word model. Native `.doc` saving is available for the supported simple subset: paragraphs, common run/paragraph formatting, supported built-in and custom paragraph style records, tabs, line/page breaks, simple body tables with common row/cell formatting and valid merged cells, single-section and paragraph-boundary multi-section page setup including equal-width columns, simple page numbering, right-to-left gutter state, and vertical page alignment, simple header/footer stories, and scalar document properties. Unsupported or preserve-only features such as macros, embedded OLE objects, fast/quick-save state, footnotes, endnotes, comments, text boxes, images, visual table styles, nested tables, invalid or conflicting table descriptors, unsupported style formatting, and richer multi-section or header/footer shapes are reported or blocked before silent data loss. |
