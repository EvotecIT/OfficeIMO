...
public sealed partial class PdfInvoiceDocument {
    ...
    private void ComposeDetails(PdfContentBuilder content) {
        ...
        if (references.Count != 0) TableGroup(content, Label(InvoicePdfText.References), references);
        ...
        if (_invoice.Payee != null) DetailGroup(content, Label(InvoicePdfText.Payee), Party(_invoice.Payee));
        if (_invoice.TaxRepresentative != null) DetailGroup(content, Label(InvoicePdfText.TaxRepresentative), Party(_invoice.TaxRepresentative));
        if (_invoice.Payments.Count != 0 || _invoice.PaymentTerms != null) {
            content.H2(Label(InvoicePdfText.Payment));
        ...
        if (_invoice.Notes.Count != 0) {
            content.H2(Label(InvoicePdfText.Notes));
