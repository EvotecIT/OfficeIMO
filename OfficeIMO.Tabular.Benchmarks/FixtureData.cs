...
internal static class FixtureData {
    ...
    internal static string Root =>
        Environment.GetEnvironmentVariable("OFFICEIMO_TABULAR_BENCHMARK_DATA")
        ?? Path.Combine(Path.GetTempPath(), "OfficeIMO", "Benchmarks", "Tabular", SourceCommit);
    ...
    internal static void WriteProvenance(string artifactsPath) {
        Directory.CreateDirectory(artifactsPath);
        var packages = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) {
            ["OfficeIMO.Tabular"] = typeof(TabularReader).Assembly.GetName().Version?.ToString() ?? string.Empty,
            ["Sylvan.Data.Csv"] = typeof(Sylvan.Data.Csv.CsvDataReader).Assembly.GetName().Version?.ToString() ?? string.Empty,
            ["Sylvan.Data.Excel"] = typeof(Sylvan.Data.Excel.ExcelDataReader).Assembly.GetName().Version?.ToString() ?? string.Empty,
            ["BenchmarkDotNet"] = typeof(BenchmarkDotNet.Running.BenchmarkSwitcher).Assembly.GetName().Version?.ToString() ?? string.Empty
        };
        var document = new {
            schemaVersion = 1,
            workload = "markpflug-65k-sales-v1",
            sourceCommit = SourceCommit,
            expectedRows = ExpectedRows,
            expectedColumns = ExpectedColumns,
            fixtures = Hashes.Select(item => new {
                name = item.Key,
                source = SourceRoot + item.Key,
                sha256 = item.Value
            }),
            packages
        };
        ...
        File.WriteAllText(
            Path.Combine(artifactsPath, "tabular-benchmark-provenance.json"),
