...
public class PdfDocIOTests {
    [Fact]
    public void Save_WritesPdfToWritableStream() {
        using var stream = new MemoryStream();

        PdfDoc returned = BuildDocument("Stream save")
            .Save(stream);
    ...
    public async Task Save_WritesPdfToPathOutputs() {
        ...
        try {
            PdfDoc returned = BuildDocument("Path save").Save(syncPath);
    ...
    private static PdfDoc BuildDocument(string title) {
