...
namespace OfficeIMO.Tests {            Assert.Matches(@"0 [0-9.]+ [0-9.]+ \.[0-9]+ re", strikeLine);
        ...
        public void Test_WordDocument_SaveAsPdf_ParagraphFormatting() {
            ...
            int etIndex = Array.IndexOf(lines, "ET");
            Assert.True(etIndex >= 0, "ET marker not found");
            string strikeLine = etIndex + 1 < lines.Length ? lines[etIndex + 1] : string.Empty;
            Assert.False(string.IsNullOrEmpty(strikeLine), "strikethrough line not found");
            Assert.Matches(@"0 [0-9.]+ [0-9.]+ \.889[0-9]* re", strikeLine);
        }
    }
}
