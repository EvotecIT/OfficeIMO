    ...
    public partial class Excel {
        ...
        public void LegacyXls_NativeSave_WritesAggregateConditionalFormattingRuleTypesAsExpressions() {
            ...
            try {
                ...
                Assert.All(legacySheet.ConditionalFormattings, rule => Assert.Equal(LegacyXlsConditionalFormattingType.Formula, rule.Type));
                Assert.Contains(legacySheet.ConditionalFormattings, rule => rule.Ranges.Single() == "A1:A5" && rule.Formula1 == "COUNTIF(A1:A5,A1)>1");
                Assert.Contains(legacySheet.ConditionalFormattings, rule => rule.Ranges.Single() == "B1:B5" && rule.Formula1 == "COUNTIF(B1:B5,B1)=1");
                Assert.Contains(legacySheet.ConditionalFormattings, rule => rule.Ranges.Single() == "C1:C5" && rule.Formula1 == "C1>AVERAGE(C1:C5)");
                Assert.Contains(legacySheet.ConditionalFormattings, rule => rule.Ranges.Single() == "D1:D5" && rule.Formula1 == "D1<=AVERAGE(D1:D5)-1*STDEV(D1:D5)");
                Assert.Contains(legacySheet.ConditionalFormattings, rule => rule.Ranges.Single() == "E1:E5" && rule.Formula1 == "E1>=LARGE(E1:E5,2)");
                Assert.Contains(legacySheet.ConditionalFormattings, rule => rule.Ranges.Single() == "F1:F5" && rule.Formula1 == "F1<=SMALL(F1:F5,2)");
                Assert.Contains(legacySheet.ConditionalFormattings, rule => rule.Ranges.Single() == "G1:G5" && rule.Formula1 == "G1>=LARGE(G1:G5,ROUNDUP(COUNT(G1:G5)*40/100,0))");
                Assert.Contains(legacySheet.ConditionalFormattings, rule => rule.Ranges.Single() == "H1:H5" && rule.Formula1 == "H1<=SMALL(H1:H5,ROUNDUP(COUNT(H1:H5)*40/100,0))");

                ExcelSheet projectedSheet = result.Document.Sheets[0];
                AssertProjectedExpressionRule(projectedSheet, "A1:A5", "COUNTIF(A1:A5,A1)>1");
                AssertProjectedExpressionRule(projectedSheet, "B1:B5", "COUNTIF(B1:B5,B1)=1");
                AssertProjectedExpressionRule(projectedSheet, "C1:C5", "C1>AVERAGE(C1:C5)");
                AssertProjectedExpressionRule(projectedSheet, "D1:D5", "D1<=AVERAGE(D1:D5)-1*STDEV(D1:D5)");
                AssertProjectedExpressionRule(projectedSheet, "E1:E5", "E1>=LARGE(E1:E5,2)");
                AssertProjectedExpressionRule(projectedSheet, "F1:F5", "F1<=SMALL(F1:F5,2)");
                AssertProjectedExpressionRule(projectedSheet, "G1:G5", "G1>=LARGE(G1:G5,ROUNDUP(COUNT(G1:G5)*40/100,0))");
                AssertProjectedExpressionRule(projectedSheet, "H1:H5", "H1<=SMALL(H1:H5,ROUNDUP(COUNT(H1:H5)*40/100,0))");
            } finally {
                TryDelete(openXmlPath);
                TryDelete(xlsOutputPath);
            }
