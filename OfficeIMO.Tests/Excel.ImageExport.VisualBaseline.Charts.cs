...
namespace OfficeIMO.Tests {
    public partial class ExcelImageExportVisualBaselineTests {
        ...
        private const string ChartDataLabelBoxesBaselineName = "officeimo-excel-image-chart-data-label-boxes";
        ...
        [Fact]
        public void ChartDataLabelBoxesImageExportMatchesApprovedBaselines() {
            using ExcelBaselineFixture fixture = CreateChartDataLabelBoxesBaselineWorkbook();
            ExcelRange range = fixture.Sheet.Range("A1:H10");
            ExcelImageExportOptions options = CreateBaselineOptions();
            ...
            Assert.DoesNotContain(png.Diagnostics, item => item.Code == ExcelImageExportDiagnosticCodes.ChartSeriesStyleApproximation);
            Assert.DoesNotContain(png.Diagnostics, item => item.Code == ExcelImageExportDiagnosticCodes.ChartTextStyleApproximation);
            Assert.DoesNotContain(png.Diagnostics, item => item.Severity == OfficeImageExportDiagnosticSeverity.Error);
            Assert.DoesNotContain(svg.Diagnostics, item => item.Severity == OfficeImageExportDiagnosticSeverity.Error);
            Assert.Contains(">120<", svgText, StringComparison.Ordinal);
            Assert.Contains(">180<", svgText, StringComparison.Ordinal);
            Assert.Contains(">160<", svgText, StringComparison.Ordinal);
            Assert.Contains("#FDE68A", svgText, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("#B45309", svgText, StringComparison.OrdinalIgnoreCase);
            Assert.Contains("#7C2D12", svgText, StringComparison.OrdinalIgnoreCase);
            AssertRasterBaseline(ChartDataLabelBoxesBaselineName + ".png", png.Bytes);
            AssertTextBaseline(ChartDataLabelBoxesBaselineName + ".svg", svgText);
        }
        ...
        [Fact]
        public void ApprovedChartDataLabelBoxesBaselinesAreRenderableAndNonBlank() {
            using ExcelBaselineFixture fixture = CreateChartDataLabelBoxesBaselineWorkbook();
            ExcelRange range = fixture.Sheet.Range("A1:H10");
            ExcelImageExportOptions options = CreateBaselineOptions();
            string pngPath = Path.Combine(BaselineDirectory, ChartDataLabelBoxesBaselineName + ".png");
            string svgPath = Path.Combine(BaselineDirectory, ChartDataLabelBoxesBaselineName + ".svg");

            Assert.True(File.Exists(pngPath), "Approved chart data-label box PNG baseline is missing.");
            Assert.True(File.Exists(svgPath), "Approved chart data-label box SVG baseline is missing.");
            OfficeRasterImage image = VisualBaselineTestSupport.DecodePng(File.ReadAllBytes(pngPath), "Approved chart data-label box PNG baseline is not a supported PNG file.");
            Assert.Equal(range.ExportImage(OfficeImageExportFormat.Png, options).Width, image.Width);
            Assert.Equal(range.ExportImage(OfficeImageExportFormat.Png, options).Height, image.Height);
            Assert.True(VisualBaselineTestSupport.CountNonBackgroundPixels(image, OfficeColor.White) > 1500, "Approved chart data-label box PNG baseline should contain a visible rendered chart.");
            string svg = File.ReadAllText(svgPath);
            Assert.Contains("Data Label Box Fidelity", svg, StringComparison.Ordinal);
        ...
        private static ExcelBaselineFixture CreateChartDataLabelBoxesBaselineWorkbook() {
            ...
            sheet.CellValue(4, 1, "Feb");
            sheet.CellValue(4, 2, 180);
            sheet.CellValue(5, 1, "Mar");
            sheet.CellValue(5, 2, 160);
            sheet.Range("A2:B2").SetFillColor("E2E8F0").SetBold();
            sheet.Range("A3:B5").SetFillColor("F8FAFC");

            ExcelChart chart = sheet.AddChartFromRange("A2:B5", row: 2, column: 4, widthPixels: 340, heightPixels: 220, type: ExcelChartType.ColumnClustered, title: "Data Label Boxes");
            chart.SetDataLabels(
                showValue: true,
