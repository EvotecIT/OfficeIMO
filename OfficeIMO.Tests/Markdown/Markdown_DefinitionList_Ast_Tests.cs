...
""";
    ...
    [Fact]
    public void DefinitionList_NestedListBody_Keeps_MixedListTails_AsDefinitionChildren() {
        const string markdown = """
Term
:   First paragraph
    - item
...
""";
        ...
        Assert.Equal("sibling item", Assert.Single(ordered.Items).Content.RenderMarkdown());
        Assert.Equal(
            new[] {
                MarkdownSyntaxKind.Paragraph,
                MarkdownSyntaxKind.UnorderedList,
            ...
            definitionValue.Children.Select(child => child.Kind).ToArray());
        Assert.Equal(new MarkdownSourceSpan(2, 5, 4, 15), definitionValue.SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(3, 3, 3, 10), unorderedSyntax.SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(4, 1, 4, 15), orderedSyntax.SourceSpan);
        Assert.Equal(NormalizeHtml(markdig), NormalizeHtml(office));
        ...
        var nativeDefinitionList = Assert.IsType<MarkdownNativeDefinitionListBlock>(Assert.Single(native.Blocks));
        var nativeDefinition = Assert.Single(Assert.Single(nativeDefinitionList.Groups).Definitions);
        Assert.IsType<MarkdownNativeParagraphBlock>(nativeDefinition.Children[0]);
        Assert.IsType<MarkdownNativeListBlock>(nativeDefinition.Children[1]);
        Assert.IsType<MarkdownNativeListBlock>(nativeDefinition.Children[2]);
        Assert.Equal("First paragraph\n\n- item\n\n1. sibling item", definitionBody.Value!.Replace("\r\n", "\n"));
...
""";
    ...
    [Fact]
    public void DefinitionList_NestedListBody_Stops_Before_UnindentedBlockquote() {
        const string markdown = """
Term
:   First
    - item
...
""";
        ...
        Assert.Equal(new MarkdownSourceSpan(4, 1, 4, 7), result.SyntaxTree.Children[1].SourceSpan);
        Assert.Equal("Term\n:   First\n    - item\n\n> quote", written);
        Assert.Equal(NormalizeHtml(markdig), NormalizeHtml(office));
...
""";
    ...
    [Fact]
    public void DefinitionList_NestedListLazyParagraph_Preserves_SetextLookingText() {
        const string markdown = """
Term
:   First
...
""";
        ...
        Assert.Equal("First", paragraph.Inlines.RenderMarkdown());
        Assert.Equal("item\nlazy title\n===\ntext", item.Content.RenderMarkdown().Replace("\r\n", "\n"));
        Assert.Equal(
            new[] {
                MarkdownSyntaxKind.Paragraph,
                MarkdownSyntaxKind.UnorderedList
        ...
        Assert.Equal(new MarkdownSourceSpan(4, 5, 7, 4), listSyntax.SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(4, 5, 7, 4), listItemSyntax.SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(4, 7, 7, 4), itemParagraphSyntax.SourceSpan);
        Assert.Contains("\\===", written, StringComparison.Ordinal);
...
""";
    ...
    [Fact]
    public void DefinitionList_NestedBlockquoteLazyParagraph_Stops_Before_UnindentedList() {
        const string markdown = """
Term
:   First
    > quote
...
""";
        ...
        Assert.Equal("First", paragraph.Inlines.RenderMarkdown());
        Assert.Equal("quote\nlazy one\nlazy two", quoteParagraph.Inlines.RenderMarkdown().Replace("\r\n", "\n"));
        Assert.Equal("tail", Assert.Single(trailingList.Items).Content.RenderMarkdown());
        Assert.Equal(
            new[] {
                MarkdownSyntaxKind.Paragraph,
            ...
            definitionValue.Children.Select(child => child.Kind).ToArray());
        Assert.Equal(new MarkdownSourceSpan(2, 5, 5, 8), definitionValue.SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(3, 3, 5, 8), quoteSyntax.SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(3, 7, 5, 8), quoteParagraphSyntax.SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(6, 1, 6, 6), result.SyntaxTree.Children[1].SourceSpan);
        Assert.Equal("Term\n:   First\n    > quote\n    > lazy one\n    > lazy two\n\n- tail", written);
        Assert.Equal(NormalizeHtml(markdig), NormalizeHtml(office));
        Assert.Equal(NormalizeHtml(markdig), NormalizeHtml(reparsedOffice));
        ...
        var nativeTrailingList = Assert.IsType<MarkdownNativeListBlock>(native.Blocks[1]);
        Assert.Equal("First\n\n> quote\n> lazy one\n> lazy two", definitionBody.Value!.Replace("\r\n", "\n"));
        Assert.Equal(new MarkdownSourceSpan(2, 5, 5, 8), definitionBody.SourceSpan);
        Assert.Equal("quote\nlazy one\nlazy two", nativeParagraph.Text.Replace("\r\n", "\n"));
        Assert.Equal("tail", Assert.Single(nativeTrailingList.Items).Text);
        MarkdownInvariantAssert.MappedAssociatedObjectsAreConsistent(result);
    }
...
""";
    ...
    [Fact]
    public void DefinitionList_NestedBlockquoteLazyParagraph_Preserves_SetextLookingText() {
        const string markdown = """
Term
:   First
...
""";
        ...
        Assert.Equal("First", paragraph.Inlines.RenderMarkdown());
        Assert.Equal("quote\nlazy title\n===\ntext", quoteParagraph.Inlines.RenderMarkdown().Replace("\r\n", "\n"));
        Assert.Equal(
            new[] {
                MarkdownSyntaxKind.Paragraph,
                MarkdownSyntaxKind.Quote
    ...
    [Fact]
    public void DefinitionList_NestedBlockquoteLazyParagraph_Preserves_Markdig_SoftBreak() {
        const string markdown = """
Term
:   First
    > quote
...
""";
        ...
        Assert.Equal("First", paragraph.Inlines.RenderMarkdown());
        Assert.Equal("quote\nlazy", quoteParagraph.Inlines.RenderMarkdown());
        Assert.IsType<SoftBreakInline>(quoteParagraph.Inlines.Nodes[1]);
        Assert.Equal(
            new[] {
                MarkdownSyntaxKind.Paragraph,
                MarkdownSyntaxKind.Quote
        ...
        Assert.Equal(new MarkdownSourceSpan(2, 5, 4, 4), definitionValue.SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(3, 3, 4, 4), quoteSyntax.SourceSpan);
        Assert.Equal(new MarkdownSourceSpan(3, 7, 4, 4), quoteParagraphSyntax.SourceSpan);
        Assert.Equal("Term\n:   First\n    > quote\n    > lazy", written);
        Assert.Equal(NormalizeHtml(markdig), NormalizeHtml(office));
        Assert.Equal(NormalizeHtml(markdig), NormalizeHtml(reparsedOffice));
        ...
        var nativeParagraph = Assert.IsType<MarkdownNativeParagraphBlock>(Assert.Single(nativeQuote.Children));
        Assert.Equal("First\n\n> quote\n> lazy", definitionBody.Value!.Replace("\r\n", "\n"));
        Assert.Equal(new MarkdownSourceSpan(2, 5, 4, 4), definitionBody.SourceSpan);
        Assert.Contains(nativeParagraph.InlineRuns, inline => inline.Kind == MarkdownNativeInlineKind.SoftBreak);
        MarkdownInvariantAssert.MappedAssociatedObjectsAreConsistent(result);
    }
...
""";
        ...
        Assert.Equal("First", paragraph.Inlines.RenderMarkdown());
        Assert.Equal("quote\n\\| A \\|\n\\|---\\|\n\\| B \\|", quoteParagraph.Inlines.RenderMarkdown());
        Assert.Equal(
            new[] {
                MarkdownSyntaxKind.Paragraph,
                MarkdownSyntaxKind.Quote
