...
public class Markdown_GenericAttributes_Syntax_Tests {
    ...
    [Fact]
    public void Blockquote_Standalone_GenericAttributes_Attach_To_Following_PipeTable_With_Source_Backup() {
        const string markdown = "> {#tbl .wide}\n> | A |\n> |---|\n> | B |\n";
        var options = MarkdownReaderOptions.CreateGitHubFlavoredMarkdownProfile();
        options.GenericAttributes = true;
        options.PreserveTrivia = true;
        ...
        var quoteNode = Assert.Single(result.FinalSyntaxTree.Children);
        var table = Assert.Single(quoteNode.Children, node => node.Kind == MarkdownSyntaxKind.Table);
        var attributes = Assert.Single(table.Children, node => node.Kind == MarkdownSyntaxKind.GenericAttributeBlock);
        ...
        var native = MarkdownNativeDocument.Parse(markdown, options);
        var nativeQuote = Assert.IsType<MarkdownNativeQuoteBlock>(Assert.Single(native.Blocks));
        var nativeTable = Assert.IsType<MarkdownNativeTableBlock>(Assert.Single(nativeQuote.Children));
        var field = Assert.Single(native.EnumerateBlockSourceFields("attributes"));
    ...
    [Fact]
    public void List_Standalone_GenericAttributes_Attach_To_Following_PipeTable_With_Source_Backup() {
        ...
        var listNode = Assert.Single(result.FinalSyntaxTree.Children);
        var itemNode = Assert.Single(listNode.Children);
        var table = Assert.Single(itemNode.Children, node => node.Kind == MarkdownSyntaxKind.Table);
        var attributes = Assert.Single(table.Children, node => node.Kind == MarkdownSyntaxKind.GenericAttributeBlock);

        Assert.Equal("{#tbl .wide}", attributes.Literal);
        Assert.Equal(new MarkdownSourceSpan(3, 3, 3, 14), attributes.SourceSpan);
        Assert.True(result.TryCreateOriginalSourceSlice(attributes, out var slice));
        Assert.Equal("{#tbl .wide}", slice.Text);

        var native = MarkdownNativeDocument.Parse(markdown, options);
        var nativeList = Assert.IsType<MarkdownNativeListBlock>(Assert.Single(native.Blocks));
        var nativeTable = Assert.IsType<MarkdownNativeTableBlock>(Assert.Single(nativeList.Items[0].Children.OfType<MarkdownNativeTableBlock>()));
        var field = Assert.Single(native.EnumerateBlockSourceFields("attributes"));

        Assert.Same(nativeTable, field.Block);
        Assert.Equal("{#tbl .wide}", field.Value);
        Assert.Equal(new MarkdownSourceSpan(3, 3, 3, 14), field.SourceSpan);
    ...
    [Fact]
    public void InlineHtmlWrapper_GenericAttributes_Are_Consumed_Without_Metadata_And_Preserve_Trailing_Text_Source() {
        const string markdown = "<u>hi</u>{#under .wide} tail\n";
        var options = new MarkdownReaderOptions {
            GenericAttributes = true,
            PreserveTrivia = true
        };
        ...
        var paragraph = Assert.Single(result.FinalSyntaxTree.Children, node => node.Kind == MarkdownSyntaxKind.Paragraph);
        var htmlTag = Assert.Single(paragraph.Descendants(), node => node.Kind == MarkdownSyntaxKind.InlineHtmlTag);
        var trailingText = Assert.Single(
            paragraph.Descendants(),
            node => node.Kind == MarkdownSyntaxKind.InlineText
                && node.SourceSpan.HasValue
                && node.SourceSpan.Value.Equals(new MarkdownSourceSpan(1, 24, 1, 28)));

        Assert.True(htmlTag.Attributes.IsEmpty);
        Assert.Equal("u", htmlTag.Literal);
        Assert.Equal(new MarkdownSourceSpan(1, 1, 1, 9), htmlTag.SourceSpan);
        Assert.True(result.TryCreateOriginalSourceSlice(htmlTag, out var htmlTagSlice));
        Assert.Equal("<u>hi</u>", htmlTagSlice.Text);
        Assert.Equal(new MarkdownSourceSpan(1, 24, 1, 28), trailingText.SourceSpan);
        Assert.True(result.TryCreateOriginalSourceSlice(trailingText, out var trailingTextSlice));
        Assert.Equal(" tail", trailingTextSlice.Text);

        var native = MarkdownNativeDocument.Parse(markdown, options);
        var nativeParagraph = Assert.IsType<MarkdownNativeParagraphBlock>(Assert.Single(native.Blocks));
        var nativeHtmlTag = Assert.Single(nativeParagraph.InlineRuns, inline => inline.Kind == MarkdownNativeInlineKind.HtmlTag);
        ...
        Assert.Equal("hi", nativeHtmlTag.Text);
        Assert.DoesNotContain(nativeHtmlTag.Metadata, metadata => metadata.Name == "attributes");
        Assert.DoesNotContain(native.EnumerateInlineMetadata(), metadata => metadata.Name == "attributes");
        Assert.Equal(new MarkdownSourceSpan(1, 24, 1, 28), nativeTrailingText.SourceSpan);
        Assert.True(native.TryCreateOriginalSourceSlice(nativeTrailingText, out var nativeTrailingTextSlice));
        Assert.Equal(" tail", nativeTrailingTextSlice.Text);
        Assert.Equal(
            "<p><u>hi</u> tail</p>",
