...
internal static class MarkdigExtensionCompatibilityMatrixWriter {
    public static string Write(MarkdigExtensionInventoryReport report) {
        ...
        sb.AppendLine("- [ ] Continue `UsePreciseSourceLocation` through the broader lossless AST/source model; the current native block, semantic HeadingBlock level/text source spans, semantic LinkInline/ImageInline/ImageLinkInline source spans, semantic TextRun escape source spans, semantic decoded entity source-text spans, semantic HardBreakInline marker source spans, semantic CodeSpanInline content source spans, semantic AbbreviationInline text/title source spans, semantic ImageBlock source spans, semantic CodeBlock and SemanticFencedBlock info/content source spans, semantic CustomContainerBlock name source spans, structured details and summary semantic/syntax/native tag and text fields, editable list-item paragraph, editable native table row, native block source-field, list item/list-item paragraph, table row/table cell, inline metadata, reference-definition field, abbreviation-definition field, and source-trivia snapshot raw normalized/original text plus failure reasons, editable document source trivia including tab-expanded columns and line endings, tab-aware line/column source slices, inline, metadata, source-edit target, generated-node source-slice and source-edit failure metadata, machine-readable roundtrip fallback reasons, custom block parser context, custom inline parser context, inline transform context, and document-transform context source-slice APIs improve editor-grade source addressing but do not close full trivia parity.");
