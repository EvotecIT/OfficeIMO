    ...
    private static MatrixCells CreateCells(MarkdigExtensionInventoryRow row) {
        ...
        switch (row.MethodName) {
            ...
            case "UsePreciseSourceLocation":
                return new MatrixCells("Cross-cutting source architecture", "Partial parser spans", "Has native block/snapshot field accessors, semantic ImageBlock source spans for standalone and linked image alt/path/title/link target/link title tokens, semantic CodeBlock and SemanticFencedBlock info/content source spans, structured details opening/closing tag and summary opening/text/closing semantic/syntax/native fields, native list-item paragraph projections/source slices/source-backed canonical reconciliation/original-preserving source edits, document-level abbreviation-definition source fields/snapshots/source slices/source edits, tab-aware line/column source slices, document-level blank-line, horizontal-whitespace with tab-expanded columns, and line-ending trivia source slices/edits, native inline/metadata/source-edit-target source slices, generated-node source-slice and source-edit failure metadata, custom block parser context source slices, custom inline parser context source slices, inline transform context source slices, document-transform context source slices, and reason-aware original mapping failures carried by native source edits; still needs full lossless trivia and mapping", "Has explicit source edits and fallback diagnostics; needs broader roundtrip behavior", "Needs broader source-edit and original-mapping proof", row.NextAction);
