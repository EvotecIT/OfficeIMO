...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        ...
        Row("UseAutoLinks", "Extended autolinks", "Bare URL, www, email, and selected scheme autolinks", MarkdigExtensionInventoryStatus.Partial, "Core parser, profile-gated", "Full extended URL/email/scheme/boundary corpus, writer preservation, and native/source metadata.", "OfficeIMO has profile-sensitive bare URL/email autolinks with Markdig-style previous-character, domain-without-period, query/fragment special-character, balanced-parenthesis trailing-punctuation, punctuation-before-closing-parenthesis preservation, single trailing punctuation/underscore trimming, lowercase www. prefix matching, optional www-host underscore rejection, optional user-info authority rejection for Markdig-compatible http/www/ftp literals, lowercase bare scheme matching, profile-selectable bare scheme prefixes for Markdig-compatible mailto:, ftp://, and tel: behavior while OfficeIMO/GFM can keep xmpp:, bare mailto path/query/fragment targets with address-only display, default/Markdig-style HTML URL rendering that can normalize Unicode hostnames to IDNA or opt into Markdig-compatible tilde percent-encoding in URL attributes, a GFM HTML profile that keeps cmark-gfm-style percent-encoded Unicode hosts, GFM/table-cell coverage, source-backed target and angle-marker metadata, and Markdown writer preservation for parsed bare and angle autolink spelling, but broader Markdig/GFM edge breadth and general text escaping differences are not complete.", "Broaden remaining GFM/Markdig autolink edge cases and separate any non-autolink renderer-policy deltas before promotion."),
