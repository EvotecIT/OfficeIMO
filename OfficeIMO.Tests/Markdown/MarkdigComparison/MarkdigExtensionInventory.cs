...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        ...
        Row("UseGenericAttributes", "Generic attributes", "Attribute blocks/spans on Markdown elements", MarkdigExtensionInventoryStatus.Partial, "Core AST/source architecture", "Arbitrary attribute block/span parsing, renderer/writer propagation, and source-edit coverage across blocks/inlines.", "OfficeIMO now has generic attribute storage on semantic MarkdownObject nodes and MarkdownSyntaxNode nodes, with fenced-code id/classes/attributes projected from MarkdownCodeFenceInfo through ordinary CodeBlock and SemanticFencedBlock parser paths. It still does not parse Markdig-style generic attributes on arbitrary blocks/inlines.", "Use the shared attribute storage as the target for future generic attribute parsers, then prove renderer/writer/source-edit propagation across blocks and inlines."),
