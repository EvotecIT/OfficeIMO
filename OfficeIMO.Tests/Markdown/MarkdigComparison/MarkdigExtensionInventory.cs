...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        Row("UseAbbreviations", "Abbreviations", "Inline abbreviation definitions and expansion", MarkdigExtensionInventoryStatus.Partial, "Core parser, opt-in", "Remaining Markdig abbreviation renderer policy plus promotion-level source/native/writer coverage.", "OfficeIMO has opt-in abbreviation definitions through MarkdownReaderOptions.Abbreviations, case-sensitive/later-wins document-wide definition collection, consumed definition syntax nodes including empty-title and list-item-contained definitions, AbbreviationInline semantic nodes, HTML <abbr> rendering, Markdig comparison cases for top-level text, unresolved bracket text, emphasis, link labels, blockquotes, lists, list-item definitions, dash/opening-punctuation boundaries, and pipe-table cells when UsePipeTables is also enabled, syntax/native metadata for visible text plus definition title source edits, nested container/table-cell AST propagation, and definition-preserving Markdown writing for parse-owned definitions with front matter, empty-title, and reparse-stability coverage. The previously named bracket-text and list-item-definition parser residuals are closed; the remaining targeted residuals are Unicode HTML entity rendering policy differences.", "Separate Unicode HTML escaping as renderer policy, then decide whether list-contained definition source-location writing needs byte/location preservation before promotion."),
        ...
        Row("UseNonAsciiNoEscape", "Non-ASCII no-escape rendering", "Renderer escaping policy", MarkdigExtensionInventoryStatus.Intentional, "Renderer escaping policy", "Document profile differences when output claims broaden.", "OfficeIMO keeps escaping behavior profile/renderer-owned instead of mirroring this Markdig switch.", "Document any renderer escaping profile differences when output claims broaden."),
