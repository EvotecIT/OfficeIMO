...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        ...
        Row("UseGenericAttributes", "Generic attributes", "Attribute blocks/spans on Markdown elements", MarkdigExtensionInventoryStatus.Partial, "Core AST/source architecture", "Remaining arbitrary block/span parsing, broader Markdown writer preservation, and source-edit coverage across blocks/inlines.", "OfficeIMO now has generic attribute storage on semantic MarkdownObject nodes and MarkdownSyntaxNode nodes, with fenced-code id/classes/attributes projected from MarkdownCodeFenceInfo through ordinary CodeBlock and SemanticFencedBlock parser paths. Default fenced-code HTML rendering projects those attributes onto the HTML pre wrapper, and semantic fenced-block code fallback renderers receive the attributed CodeBlock. Opt-in MarkdownReaderOptions.GenericAttributes now parses Markdig-style trailing attribute blocks for ATX headings and paragraphs into the same semantic/syntax attribute storage, renders those attributes to heading/paragraph HTML, and writes normalized trailing attribute blocks that reparse stably. It still does not parse generic attributes for arbitrary block families or inline spans, and source-edit/token-level attribute coverage is not complete.", "Extend the shared attribute parser/writer to more block families and inline spans, then prove source-edit and writer propagation across arbitrary blocks and inlines."),
