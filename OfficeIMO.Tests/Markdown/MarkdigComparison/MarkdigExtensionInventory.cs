...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        Row("UseAbbreviations", "Abbreviations", "Inline abbreviation definitions and expansion", MarkdigExtensionInventoryStatus.Partial, "Core parser, opt-in", "Remaining Markdig abbreviation edge breadth plus promotion-level source/native coverage.", "OfficeIMO has opt-in abbreviation definitions through MarkdownReaderOptions.Abbreviations, case-sensitive/later-wins document-wide definition collection, consumed definition syntax nodes including empty-title definitions, AbbreviationInline semantic nodes, HTML <abbr> rendering, Markdig comparison cases for top-level text, emphasis, link labels, blockquotes, lists, dash/opening-punctuation boundaries, and pipe-table cells when UsePipeTables is also enabled, syntax/native metadata for visible text plus definition title source edits, nested container/table-cell AST propagation, and definition-preserving Markdown writing for parse-owned definitions with front matter, empty-title, and reparse-stability coverage. A 449-case disposable UseAbbreviations sweep now leaves 11 named residual mismatches: Unicode HTML entity rendering, bracket-text expansion, and list-item abbreviation definitions.", "Close the remaining bracket-text/list-item definition parser deltas or document them as intentional, and separate Unicode HTML escaping as renderer policy before promotion."),
