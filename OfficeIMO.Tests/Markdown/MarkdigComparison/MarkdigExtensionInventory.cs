...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        ...
        Row("UsePreciseSourceLocation", "Precise source location", "Line/column source location precision", MarkdigExtensionInventoryStatus.Partial, "Cross-cutting core source architecture", "Complete lossless trivia/original mapping, generated-node roundtrip semantics, and source-edit coverage before claiming parity.", "OfficeIMO has syntax/source/native spans, source slices, original-source slices when trivia is preserved, reason-aware original-source slice failure reporting, source edits, roundtrip diagnostics, addressable native block/snapshot source fields including repeated fields by occurrence index, source-order inline metadata snapshots, native inline/inline-metadata source-slice APIs for source-backed link targets, titles, link/image delimiters, formatting and emphasis-extra delimiters, footnote definition markers, front-matter fences, code and semantic fenced block markers, formatting content, escaped-character markers, decoded entity source text, hard-break markers, raw inline HTML fragments, raw HTML block comment/tag/CDATA/declaration/processing-instruction markers, per-column table alignment markers, pipe-table row delimiters, and similar metadata, source-slice APIs aligned with native source-edit targets for blocks, list item content, list-item paragraphs, table cells, definition-list objects, reference definitions, reference-definition fields, and document-level source trivia, paragraph-level native projections, source slices, and source edits for list-item paragraphs, custom block parser context normalized source-slice APIs for parser-created spans and relative line ranges, custom inline parser context normalized source-slice APIs for claimed inline ranges, inline transform context normalized source-slice APIs for source-backed inline nodes and spans, emphasis sequence semantic node source spans, document-transform context normalized/original source-slice APIs for parsed model objects and syntax spans, block/inline HTML renderer contexts and Markdown writer contexts with normalized/original source-slice APIs plus original-source failure reasons, parse-result generated syntax diagnostics with syntax paths, index paths, source fallback anchors, and associated semantic object details, generated syntax-node original-source slice rejection with a dedicated generated-node failure reason, native source edits that carry generated-node original-source failure metadata for generated block, inline, list-item, list-item paragraph, table-cell, definition-list, block-field, and metadata targets, plus document-level source trivia, snapshots, source-order enumeration, position lookup, and normalized/original source slices for blank lines, whitespace-only lines, leading/trailing horizontal whitespace, tabs, and line endings. Document-level trivia columns and generic line/column source-slice fallback now expand tabs with the same tab-stop model as source maps. Original-source slices now map line-ending-equivalent normalized spans back to the original CRLF, LF, or standalone CR spelling through a shared mapper used by parse results and transform contexts, and line-ending trivia source edits preserve original source bytes around the changed trivia when possible. Full lossless trivia/original mapping is still partial.", "Continue Phase 3 source-map and trivia work before claiming parity."),
