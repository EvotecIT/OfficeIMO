...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        Row("UseAbbreviations", "Abbreviations", "Inline abbreviation definitions and expansion", MarkdigExtensionInventoryStatus.Partial, "Core parser, opt-in", "Broader Markdig abbreviation edge breadth plus promotion-level source/native coverage.", "OfficeIMO has opt-in abbreviation definitions through MarkdownReaderOptions.Abbreviations, case-sensitive/later-wins definition collection, consumed definition syntax nodes, AbbreviationInline semantic nodes, HTML <abbr> rendering, Markdig comparison cases, syntax/native metadata for visible text plus definition title source edits, and definition-preserving Markdown writing for parse-owned definitions with front matter and reparse-stability coverage, but broader Markdig edge breadth is not complete.", "Broaden Markdig abbreviation edge cases and promote only after the parser/source/writer deltas are explicit."),
