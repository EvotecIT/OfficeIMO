...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        ...
        Row("UseNonAsciiNoEscape", "Non-ASCII no-escape rendering", "Renderer escaping policy", MarkdigExtensionInventoryStatus.Partial, "Renderer escaping policy", "Complete renderer escape-policy coverage across attributes, security-owned escape paths, and custom renderer extension output.", "OfficeIMO exposes HtmlOptions.EscapeNonAsciiText so Markdig/GFM-style HTML output can keep non-ASCII visible text literal while preserving the historical .NET encoder behavior by default. The GitHub-flavored HTML profile enables literal non-ASCII text rendering, and inline text, link display text, code block text, captions, simple quote text, abbreviation output, TOC labels/titles, heading helper text, page titles, raw-HTML escape output, image-blocked placeholder text, and portable fallback helper text use the explicit policy. Attribute, sanitizer/security-owned escape, and renderer-extension callback output still need explicit policy alignment before this row can be promoted.", "Route or document remaining attribute/security/custom-renderer output policy before promotion."),
