...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        ...
        Row("UsePreciseSourceLocation", "Precise source location", "Line/column source location precision", MarkdigExtensionInventoryStatus.Partial, "Cross-cutting core source architecture", "Complete lossless trivia/original mapping, generated-node diagnostics, and source-edit coverage before claiming parity.", "OfficeIMO has syntax/source/native spans, source slices, original-source slices when trivia is preserved, reason-aware original-source slice failure reporting, source edits, roundtrip diagnostics, addressable native block/snapshot source fields including repeated fields by occurrence index, native inline/inline-metadata source-slice APIs for source-backed link targets, titles, formatting content, and similar metadata, source-slice APIs aligned with native source-edit targets for blocks, list item content, table cells, definition-list objects, reference definitions, and reference-definition fields, paragraph-level native projections and source slices for list-item paragraphs, plus document-level source trivia, snapshots, source-order enumeration, position lookup, and normalized/original source slices for blank lines, whitespace-only lines, leading/trailing horizontal whitespace, tabs, and line endings. Document-level trivia columns now expand tabs with the same tab-stop model as source maps. Original-source slices now map line-ending-equivalent normalized spans back to the original CRLF, LF, or standalone CR spelling. Full lossless trivia/original mapping is still partial.", "Continue Phase 3 source-map and trivia work before claiming parity."),
