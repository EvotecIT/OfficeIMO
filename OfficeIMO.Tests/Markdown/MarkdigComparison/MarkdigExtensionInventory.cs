...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        ...
        Row("UseAutoLinks", "Extended autolinks", "Bare URL, www, and email autolinks", MarkdigExtensionInventoryStatus.Partial, "OfficeIMO has profile-sensitive bare URL/email autolinks with Markdig-style previous-character, domain-without-period, and query/fragment special-character options, GFM coverage, source-backed target and angle-marker metadata, and Markdown writer preservation for parsed bare and angle autolink spelling, but broader Markdig/GFM edge breadth is not complete.", "Broaden remaining GFM/Markdig autolink edge cases before promotion."),
        ...
        Row("UseEmphasisExtras", "Emphasis extras", "Strikethrough, inserted, marked, superscript, and subscript-style extras", MarkdigExtensionInventoryStatus.Partial, "OfficeIMO has strikethrough, inserted-text, and highlight/mark-style inlines with source-backed delimiter metadata for the parser-owned forms, but not the full Markdig emphasis-extra set.", "Decide profile semantics for Markdig single-tilde subscript versus GFM single-tilde strikethrough, then add superscript/subscript parser, AST/source, renderer, and writer evidence before promotion."),
