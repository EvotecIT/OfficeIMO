...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        ...
        Row("UsePreciseSourceLocation", "Precise source location", "Line/column source location precision", MarkdigExtensionInventoryStatus.Partial, "Cross-cutting core source architecture", "Complete lossless trivia/original mapping, generated-node diagnostics, and source-edit coverage before claiming parity.", "OfficeIMO has syntax/source/native spans, source slices, original-source slices when trivia is preserved, reason-aware original-source slice failure reporting, source edits, roundtrip diagnostics, addressable native block/snapshot source fields including repeated fields by occurrence index, native inline/inline-metadata source-slice APIs for source-backed link targets, titles, formatting content, and similar metadata, source-slice APIs aligned with native source-edit targets for blocks, list item content, table cells, definition-list objects, reference definitions, and reference-definition fields, plus paragraph-level native projections and source slices for list-item paragraphs. Full lossless trivia/original mapping is still partial.", "Continue Phase 3 source-map and trivia work before claiming parity."),
