...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        ...
        Row("UseEmphasisExtras", "Emphasis extras", "Strikethrough, inserted, marked, superscript, and subscript-style extras", MarkdigExtensionInventoryStatus.Partial, "OfficeIMO has strikethrough, inserted-text, superscript, and highlight/mark-style inlines with source-backed delimiter metadata for the parser-owned forms, but not the full Markdig emphasis-extra set.", "Decide profile semantics for Markdig single-tilde subscript versus GFM single-tilde strikethrough, then add subscript parser, AST/source, renderer, and writer evidence before promotion."),
