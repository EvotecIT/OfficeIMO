...
internal static class MarkdigExtensionInventory {
    ...
    private static IReadOnlyList<MarkdigExtensionInventoryRow> CreateRows() => [
        Row("UseAbbreviations", "Abbreviations", "Inline abbreviation definitions and expansion", MarkdigExtensionInventoryStatus.Partial, "Core parser, opt-in", "Promotion-level source/native/lossless writer coverage.", "OfficeIMO has opt-in abbreviation definitions through MarkdownReaderOptions.Abbreviations, case-sensitive/later-wins document-wide definition collection, consumed definition syntax nodes including empty-title and list-item-contained definitions, AbbreviationInline semantic nodes, HTML <abbr> rendering, Markdig comparison cases for top-level text, Unicode text, unresolved bracket text, emphasis, link labels, blockquotes, lists, list-item definitions, dash/opening-punctuation boundaries, and pipe-table cells when UsePipeTables is also enabled, syntax/native metadata for visible text plus definition title source edits, nested container/table-cell AST propagation, definition-preserving Markdown writing for parse-owned definitions with front matter, empty-title, and reparse-stability coverage, and Markdig-style literal non-ASCII text rendering through HtmlOptions.EscapeNonAsciiText = false. The previously named bracket-text, list-item-definition, and Unicode text-rendering residuals are closed.", "Decide whether list-contained definition source-location and lossless writer breadth need byte/location preservation before promotion."),
        ...
        Row("UseAutoLinks", "Extended autolinks", "Bare URL, www, email, and selected scheme autolinks", MarkdigExtensionInventoryStatus.Partial, "Core parser, profile-gated", "Full extended URL/email/scheme/boundary corpus, writer preservation, and native/source metadata.", "OfficeIMO has profile-sensitive bare URL/email autolinks with Markdig-style previous-character, domain-without-period, query/fragment special-character, balanced-parenthesis trailing-punctuation, punctuation-before-closing-parenthesis preservation, single trailing punctuation/underscore trimming, optional trailing semicolon retention, optional trailing quote retention with paired single-quote literal fallback, lowercase www. prefix matching, optional www-host and url-host underscore rejection, optional user-info authority rejection for Markdig-compatible http/www/ftp literals, optional closing-bracket URL consumption, lowercase bare scheme matching, profile-selectable bare scheme prefixes for Markdig-compatible mailto:, ftp://, and tel: behavior while OfficeIMO/GFM can keep xmpp:, apostrophe-started bare scheme literal fallback, bare mailto path/query/fragment targets with address-only display, optional Markdig-compatible mailto semicolon and address-only colon/dash handling, default/Markdig-style HTML URL rendering that can normalize Unicode hostnames to IDNA or opt into Markdig-compatible tilde percent-encoding in URL attributes, percent-encoded single quotes in URL attributes, a GFM HTML profile that keeps cmark-gfm-style percent-encoded Unicode hosts, GFM/table-cell coverage, source-backed target and angle-marker metadata, and Markdown writer preservation for parsed bare and angle autolink spelling, but broader Markdig/GFM edge breadth and general text escaping differences are not complete.", "Broaden remaining GFM/Markdig autolink edge cases and separate any non-autolink renderer-policy deltas before promotion."),
        ...
        Row("UseNonAsciiNoEscape", "Non-ASCII no-escape rendering", "Renderer escaping policy", MarkdigExtensionInventoryStatus.Partial, "Renderer escaping policy", "Complete renderer escape-policy coverage across visible text, attributes, TOC/helper text, raw-HTML escape paths, and renderer extension output.", "OfficeIMO exposes HtmlOptions.EscapeNonAsciiText so Markdig/GFM-style HTML output can keep non-ASCII visible text literal while preserving the historical .NET encoder behavior by default. The GitHub-flavored HTML profile enables literal non-ASCII text rendering, and inline text, link display text, code block text, captions, simple quote text, and abbreviation output use the explicit policy. Attribute, TOC/helper, raw-HTML escape, and renderer-extension paths still need policy alignment before this row can be promoted.", "Route remaining human-visible renderer text paths through the explicit escape policy or document why a path remains attribute/security-owned."),
